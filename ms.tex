...
\kevin{Suggested clarification to (A3) and (A5).  The total derivative
in (A3) is, by the chain rule,
\[
\left.\nabla_\theta^{\rm tot}\log g_n^\theta\right|_{\theta=\phi}
=
\left.\partial_\theta\log f_{Y_n\mid X_n}(y_n^*\mid X_n;\theta)
\right|_{\theta=\phi}
+
\left.\partial_x\log f_{Y_n\mid X_n}(y_n^*\mid X_n;\theta)
\,\partial_\theta X_n^\theta\right|_{\theta=\phi}.
\]
It is the time-$n$ total measurement gradient on the simulated particle.
The log-weight gradient satisfies
\[
...
\left.\nabla_\theta^{\rm tot}\log g_k^\theta\right|_{\theta=\phi}.
\]
Thus every time-$k$ simulator contribution is discounted by
$\alpha^{n-k}$.  Section~S7 applies the historical particle estimates of
\citet{chanDelMoral18} to analytical lag truncations and then recovers the
finite discounted sum by an exact geometric-mixture identity.}
...
\end{aligned}
\]
Here $\varrho=1-\underbar M/(2\bar M)\in[\tfrac12,1)$ is unchanged.
The sum beginning at $q=1$ is empty for $N\leq2$; the population
terms are zero at $\varrho=\tfrac12$.  The notation $\lesssim$ hides the same
particle-system mixing and logarithmic-threshold factor as the preceding variance argument;
apart from that factor, the suppressed constants are independent of
$N,J$, and $\alpha$.  The finite geometric sum is
...
\kevin{Interpretation.  The algorithm uses geometric discounting and no
hard lag cutoff.  Lag-truncated scores are used only as analytical devices.
If
\[
\beta_k=(1-\alpha)\alpha^k\quad(0\leq k<N-1),
\qquad \beta_{N-1}=\alpha^{N-1},
\]
then $\sum_k\beta_k=1$, $\sum_{k=r}^{N-1}\beta_k=\alpha^r$, and hence
the implemented discounted score satisfies the exact identity
\[
\nabla_\theta\hat\ell^\alpha(\theta)
=\sum_{k=0}^{N-1}\beta_k\nabla_\theta\hat s_k^1(\theta).
\]
For $N\geq2$, $\beta_0=1-\alpha$ is the mixture weight on the entire hard-zero-lag
score, whereas the coefficient of the zero-lag contribution is
$\sum_{k=0}^{N-1}\beta_k=1$, because every hard-$k$ score contains it.
Thus $\nabla_\theta\hat\ell_n^0(\theta)=\Delta_{n,0}^J$ exactly.
For $N=1$, the only mixture weight is $\beta_0=1$.
Thus DMOP-$\alpha$ is exactly a geometric average of the analytical lag
truncations and, in this precise sense, approximates a hard lag cutoff.  A
contribution of age $r$ is multiplied by $\alpha^r$, and its effective
memory is
\[
\sum_{k=0}^{N-1}\beta_k(k+1)
=\sum_{r=0}^{N-1}\alpha^r.
\]
Applying the finite-window variance and weak-bias bounds before taking this
geometric average gives the particle terms in the theorem.  This order of
argument avoids applying a particle theorem once to the complete
unnormalized score.
For the full-score population term, posterior backward contraction over
$q$ steps and the first future transition give
\[
...
\]
The factor $\varrho^{q+1}$ therefore remains in the bound; discounting and
model forgetting enter together rather than as unrelated approximations.
The sharper population term in the proposed theorem retains the
transition constants through the same $\varrho$.
S14 also retains the signed first-order coefficient
$\sum_{n=1}^{N}\sum_{r=1}^{n-1}r\Delta_{n,r}$, with an explicit
$O\{N(1-\alpha)^2\}$ remainder.  Cancellation of this coefficient can
improve the population order without an estimator correction, but is
not imposed by the mixing assumptions.}
...
Update the supplement-section cross-reference in this paragraph from
Section~S6 to Section~S7.
...
For the particle term, use the lag-truncated score
$\nabla_\theta\hat s_k^1(\theta)$ only inside the proof.  Its terms telescope
by creation time, so the time-$s$ contribution is evaluated after age $k$,
not after age $s+k$.  For one historical input over $q$ subsequent
selection--mutation steps, Theorems~3.3 and~3.5 of
\citet{chanDelMoral18} are applied to the prediction genealogies.
Conditional multinomial resampling and the exact bounded Bayes-ratio
expansion then pass those estimates to the filtered averages; Lemma~2.3
of \citet{karjalainen23} controls the prediction cloud at the beginning
of the window.  Together these steps give
\[
...
\]
The terminal block is treated as one additive historical functional; its
second moment and bias are bounded by
$C(k+1)^3G'(\theta)^2/J$ and $C(k+1)^2G'(\theta)/J$,
respectively.  Cauchy--Schwarz controls overlapping windows.  For
nonoverlapping windows, Davydov's inequality is applied after the correct
$t-s-k$ separation.  Summing these covariances, with $\lesssim$ retaining
the manuscript's convention of hiding the particle-system mixing and
logarithmic-threshold factor, gives
\[
\Var\{\nabla_\theta\hat s_k^1(\theta)\}
...
\]
The exact geometric-mixture identity
\[
\nabla_\theta\hat\ell^\alpha(\theta)
=\sum_{k=0}^{N-1}\beta_k\nabla_\theta\hat s_k^1(\theta)
\]
then recovers DMOP-$\alpha$ itself.  It is an algebraic identity, not a
fixed-lag implementation or an added randomization.  Minkowski's inequality
and $\sum_k\beta_k(k+1)=\sum_{r=0}^{N-1}\alpha^r$ give the displayed
particle bounds.}
...
\left(\sum_{r=0}^{N-1}\alpha^r\right)^2.
\]
Consequently DMOP-$0$ and every fixed $\alpha<1$ have leading variance
order $N/J$ when $J\geq N$, with the constant increasing with the effective
discount memory.  At $\alpha=1$, this conservative argument gives
$N^3/J+N^4/J^2$; it does not claim that this endpoint dependence is sharp.
The population bias moves in the opposite direction and vanishes at
$\alpha=1$.
...
In the supplement's proof, each bounded one-time contribution is controlled
before the discounted lag sum is taken, so no additional $N^2$ rescaling is
appropriate.}
...
\kevin{Suggested clarification.  DMOP-$\alpha$ itself is geometrically
discounted; the proof does not replace it by a fixed-lag algorithm.  The
auxiliary lag truncations are used because their geometric average is
exactly DMOP-$\alpha$.  A separately implemented MOP-$(1,k)$ method may
still be mentioned here, but it is not required for Theorem~
\ref{thm:mop-biasvar}.}
