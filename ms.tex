...
\kevin{Section~S7 contains the new bias--variance analysis. Change the preamble references for the Bayesian figures, baseline justification, and algorithmic parameters from S7, S8, S9 to S8, S9, S10.}
...
\begin{enumerate}[label=(A\arabic*),itemsep=-1ex, , font=\small] 
    ...
    \item \textbf{Differentiable Density Ratios and Simulator.} The draws have a law independent of $\theta$. For fixed draws and reference-filter resampling indices, the simulated path and its log measurement densities have more than two continuous derivatives in $\theta$.\label{assump:diff-meas-and-sim}
\end{enumerate}
\arxiv{}{\vspace*{-3ex}}

\kevin{Add for the particle-error bounds:
\textbf{(A6) Measurement-sensitivity decay.}
The simulator is $C^1$ in its incoming state. For a continuation
$X_n^\theta(x_t,\omega_{t+1:n})$ from $x_t$, with $X_t^\theta(x_t)=x_t$,
\[
\left\|D_{x_t}\log f_{Y_n\mid X_n}
\bigl(y_n^*\mid X_n^\theta(x_t,\omega_{t+1:n});\theta\bigr)\right\|_2
...
\textbf{Alternative (A6-J).} The log measurement density is $C^1$ in
...
Either condition suffices for the particle bounds; see S7 for the
