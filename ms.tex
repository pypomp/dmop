...
\kevin{Suggested assumption edits for the revised supplement: clarify in (A3) that $\nabla_\theta$ is the total derivative through the reparameterized simulator and enlarge $G'(\theta)$ if necessary to bound the actual term $\nabla_\theta\log g_{n,j}^{A,F,\theta}$ uniformly. Add (A6), ``First-Order Model Regularity'': for fixed simulator random numbers, the simulated path and log measurement densities are continuously differentiable near $\phi$; differentiation may pass through the expectations defining the filter; and the realized total DMOP mark is bounded by $B_1G'(\theta)$. The supplement derives the standard pathwise derivative identity directly by differentiating the simulator integral. A primitive sufficient condition for the mark bound is that the fixed-seed simulator Jacobian in its parent state has norm at most $\kappa<1$, its parameter Jacobian and initial-state derivative are bounded, and the state and parameter derivatives of the log measurement density are bounded; the tangent recursion then gives a geometric series bounded by $(1-\kappa)^{-1}$. The supplement also proves deterministic lagwise forgetting from first principles: the posterior backward kernel has minorization constant $\underbar M/\bar M$ under (A2), so its oscillation contracts geometrically. Thus (A6) no longer assumes a bounded transition-density score or the desired forgetting conclusion. For the $O(J)$ particle genealogy, Theorem~2 of \citet{chanDelMoral18} gives $O(n/J)$ weak bias for each bounded lag coefficient on the full genealogy. The proof cannot restart only $r+1$ steps in the past because the total AD derivative at time $n-r$ already depends on the earlier genealogy. Discounting therefore gives $O\{n/[J(1-\alpha)]\}$ bias for the $n$th conditional score. A uniform $O(J^{-1})$ conditional-score bias would require an additional integrated particle-genealogy estimate with geometric lag decay; it is not implied by (A6), by deterministic filter-derivative stability, or by the time-uniform $O(J^2)$ backward-estimator results of \citet{delMoral15derivative,tadic18}. State separately as (A7) the one-offspring continuation estimate needed for variance. The proof uses multinomial resampling; systematic resampling remains the practical default.}
...
\kevin{Suggested replacement for Theorem~\ref{thm:mop-biasvar}, under (A2), (A3), the proposed first-order model-regularity assumption (A6), the separate finite-particle continuation assumption (A7), multinomial resampling, $\theta=\phi$, and $\alpha\in[0,1)$: $\E\|\nabla_\theta\hat\ell^\alpha-\nabla_\theta\ell\|_2^2\lesssim pG'(\theta)^2\big[N/\{J(1-\alpha)^2\}+N^2\{\varrho(1-\alpha)/((1-\varrho)(1-\alpha\varrho))+N/[J(1-\alpha)]\}^2\big]$, and $\Var(\nabla_\theta\hat\ell^\alpha)\lesssim NpG'(\theta)^2/\{J(1-\alpha)^2\}$. The constant is independent of $N$, $J$, and $\alpha$. Only the variance term uses (A7); the population-discount bias follows from deterministic derivative forgetting, while the particle term follows from the full-genealogy path-space weak-error bound.}
