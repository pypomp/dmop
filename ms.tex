...
\kevin{Suggested correction to this endpoint discussion: under their nondegeneracy and mixing conditions, \citet{poyiadjis11} show that the asymptotic variance grows at least quadratically with the time horizon; they do not give a sharp $N^4/J$ variance law. A generic path-functional upper bound can be $O(N^4/J)$: rescaling an oscillation-one result to an $O(N)$ additive score contributes $N^2$ to MSE, and the path-space expansion can contribute another $N^2$. Thus oscillation normalization alone does not create the quartic power. Separately, the proved full-genealogy weak-bias term can contribute $N^4/\{J^2(1-\alpha)^2\}$ to squared bias. The proved bound does not establish a rate below DMOP-$1$. Such a comparison is available only if one additionally proves integrated particle-genealogy bias $O(\varrho^r/J)$ and assumes both non-cancellation of the DMOP-$0$ population bias and a uniform Poyiadjis-type nondegeneracy lower bound; under those qualifications, $1-\alpha=(NJ)^{-1/4}$ is strictly better in rate than both endpoints when $1\ll J\ll N$. DMOP-$1$'s pathwise reparameterization mark is not generally particlewise identical to the complete-data likelihood-ratio score of \citet{poyiadjis11}; the two have the same population score target only when the required differentiability and derivative--expectation interchanges are valid. Their lower bound is therefore motivation for the DMOP-$1$ endpoint assumption, not a proved lower bound for the DMOP-$1$ particle estimator.}
