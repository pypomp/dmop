...
suppose additionally that the Markov transition of the current
differentiable particle has Dobrushin coefficient uniformly smaller than
one.  This last condition is automatic if (A2) is imposed on the full
differentiable-particle state, but it is not asserted to follow from a
minorization of the physical-location marginal alone.  Then
\[
...
it is absorbed by the $N/J$ term.  A sharper variance upper bound of order
$N/\{J(1-\alpha^2)\}$ before horizon saturation would require the
centered two- and four-lineage estimate stated in Section~S7 and is not
claimed under the displayed conditions.}
...
whose squared contributions sum to $N/\{J(1-\alpha)^2\}$; the quadratic ratio
remainders sum to $N^2/\{J^2(1-\alpha)^2\}$.  The separate accumulated
weight-gradient recursion contracts at rate $\alpha$ modulo common additive
shifts, and those shifts cancel from the normalized score.  Its effective memory
therefore depends jointly on the current-particle contraction and $\alpha$,
through the factors $1-\alpha\varrho$ already present in the proof, rather than
through an additional independent mixing-time factor.  Uniform filter-derivative
stability results of Tadi\'c--Doucet and Del Moral--Doucet--Singh provide standard
sufficient conditions for the extra differentiable-particle contraction, but
physical-location minorization alone does not prove it.  The uniform $J^{-1}$
current-filter weak-bias lemma is not applied directly to a historical
genealogical functional.}
...
$\{N+(k+1)^2\}/J$.  A differentiable particle does include its location and
gradient; the unresolved issue is not the particle's semantics, but covariance
among score contributions that share a resampling genealogy.  Current-cloud
mixing and suppression of its mixing-time factor do not remove that covariance.}
...
For $\alpha<1$, the direct theorem above gives a finite-particle tradeoff without
the unproved centered genealogy estimate.  Balancing its displayed upper bound
gives
\[
1-\alpha\asymp\left(\frac1{NJ}+\frac1{J^2}\right)^{1/4},
\qquad
\operatorname{MSE}\lesssim
pG'(\theta)^2N\left(\frac{N}{J}+\frac{N^2}{J^2}\right)^{1/2}.
\]
This is only the minimizer of the proved upper bound.  The signed population
