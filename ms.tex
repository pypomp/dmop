...
We test IFAD with $\alpha = 0, 0.97, 1$ against IF2 on a global search problem for the Dhaka cholera model using our implementation in \texttt{pypomp} 0.4.6.0 \citep{abkemeier25}.
For each method, we perform 100 searches, initialized with 100 starting parameter vectors drawn uniformly from the same wide bounding box used in \cite{ionides15}. 
...
For each method, we choose algorithmic parameters that maximize the log-likelihood best for that method while finishing within the allotted time. 
...
To assess whether other methods can catch up to IFAD-0.97 given more time, we separately run the IF2-only approach for 4 times as many iterations and the IFAD variants with 4 times as many Adam iterations, slowing the cooling rate for each as well.
...
We summarize our findings in Table \ref{table:mle} and Figures \ref{fig:boxplot}, \ref{fig:density}, and \ref{fig:optim}. 
...
Based on Figure \ref{fig:optim}, IFAD-0 and IF2-only plateau heavily after being run for long enough and ostensibly would not catch up to IFAD-0.97 without excessive time and effort. 
...
Using the improved computational efficiency of \texttt{pypomp} (elaborated upon in Section \ref{sec:efficiency}), IF2 alone can easily surpass all of these previous results within a wall clock time of 15 minutes, and IFAD-0.97 holds the record at $-3744.17$ within a slightly shorter time. 
\aja{It wouldn't actually take that long to run the IF2 searches in R, so I don't think that really explains why -3747.82 hadn't been easily hit previously.}
...
That said, these are also the most biased estimates, and IFAD-1 tends to agree more closely with IFAD-0.97. 
Because multiple searches are employed in large part to mitigate the effects of Monte~Carlo variance, the lower Monte~Carlo variance of IFAD-0.97 suggests that fewer searches are needed when running IFAD-0.97 to obtain a log-likelihood close to the maximum. 
...
\begin{tabular}{lll}
\toprule
Metric & \texttt{pomp} \citep{king16} & \texttt{pypomp} \citep{abkemeier25} \\
\midrule
Language & R (\texttt{C++} simulator) & Python (\texttt{JAX}) \\
Hardware& 3.0 GHz Intel Xeon Gold 6154 (1 core) & NVIDIA RTX6000 Pro Blackwell \\
IF2 Replications & 1 & 100 \\
Latency (s) & 935 & 90 \\
\bottomrule
\end{tabular}
\caption{Comparison of execution latency for running IF2 (60 iterations, 5000 particles, 1 versus 100 replications) between the CPU-only R \texttt{pomp} package and the GPU-accelerated \texttt{pypomp} package.}
\label{table:speedup}
\end{table}
