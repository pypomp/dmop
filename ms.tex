...
\]
For $0\leq\alpha<1$, the full-score MSE has the two-line bound
\[
\begin{aligned}
&\E\|\nabla_\theta\hat\ell^\alpha(\theta)
-\nabla_\theta\ell(\theta)\|_2^2\\
&\quad\lesssim pG'(\theta)^2\left[
\left(\frac NJ+\frac{N^2}{J^2}\right)
\left(\sum_{r=0}^{N-1}\alpha^r\right)^2
+\left\{
\varrho(1-\alpha)
...
\end{aligned}
\]
Here $\varrho\in(0,1)$.  The notation $\lesssim$ hides the same
...
S14 sharpens these constants using the existing
$\epsilon=\underbar M/\bar M$: for $0<\epsilon<1$ the population bias is
at most
\[
2(N-1)\sqrt p\,G'(\theta)
...
\]
and it is zero when $\epsilon=1$.
It also retains the signed first-order coefficient
