...
current DMOP object is the pair $(x_n,w_n)$,
where (A2) governs the transition of $x_n$ and the accumulated
weight-gradient component satisfies $w_n=\alpha w_{n-1}+h_n(x_n)$ along a
selected lineage.  Thus (A3) bounds the current input $h_n(x_n)$, while
...
the following bounds hold.  Here the current DMOP object is the pair
consisting of the particle location and its accumulated weight gradient:
the location transition is the one in (A2), the current measurement-gradient
input is the bounded function in (A3), and the second component obeys
$w_n=\alpha w_{n-1}+h_n(x_n)$.  No total-variation minorization of this
pair is assumed.  Then
\[
...
remainders sum to $N^2/\{J^2(1-\alpha)^2\}$.  The current state used in
this calculation is the pair $(x,w)$.  Assumption (A2) couples the location
component geometrically, while $w_n=\alpha w_{n-1}+h_n(x_n)$.  Full-score
...
accumulated weight gradient; the unresolved issue is covariance
