...
\kevin{Suggested assumption edits for the revised supplement: clarify in (A3) that $\nabla_\theta$ is the total derivative through the reparameterized simulator and enlarge $G'(\theta)$ if necessary to bound the actual term $\nabla_\theta\log g_{n,j}^{A,F,\theta}$ uniformly. A primitive sufficient condition for this bound is that the fixed-seed simulator Jacobian in its parent state has norm at most $\kappa<1$, its parameter Jacobian and initial-state derivative are bounded, and the state and parameter derivatives of the log measurement density are bounded; the tangent recursion then gives a geometric series bounded by $(1-\kappa)^{-1}$. Add (A6), ``First-Order Model Regularity'': the initial density and each transition--measurement density product are continuously differentiable near $\phi$; their first derivatives are bounded by $B_1G'(\theta)$ times the density; differentiation may pass under the filtering integrals; and the density derivative agrees with conditional expectation of the reparameterized-simulator derivative. No second parameter derivative is required. Theorem~2.2 of \citet{tadicHigher20} supplies deterministic lagwise forgetting. For the $O(J)$ particle genealogy, Theorem~2 of \citet{chanDelMoral18} gives $O(n/J)$ weak bias for each bounded lag coefficient on the full genealogy. The proof cannot restart only $r+1$ steps in the past because the total AD derivative at time $n-r$ already depends on the earlier genealogy. Discounting therefore gives $O\{n/[J(1-\alpha)]\}$ bias for the $n$th conditional score. A uniform $O(J^{-1})$ conditional-score bias would require an additional integrated particle-genealogy estimate with geometric lag decay; it is not implied by (A6), by deterministic filter-derivative stability, or by the time-uniform $O(J^2)$ backward-estimator results of \citet{delMoral15derivative,tadic18}. State separately as (A7) the one-offspring continuation estimate needed for variance. These density conditions are proof conditions only: DMOP remains plug-and-play. The proof uses multinomial resampling; systematic resampling remains the practical default.}
