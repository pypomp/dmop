...
A POMP model inference algorithm is said to be plug-and-play if it does not need to evaluate the transition density of the latent Markov process, enabling an arbitrary model simulator to be plugged into the algorithm, a convenient property that facilitates scientific discovery \citep{he10}.
...
Our methodology is included in \texttt{pypomp} \citep{abkemeier25}, an open-source software package for POMP modeling leveraging GPU acceleration and automatic differentiation.
...
A basic IFAD approach is presented in Algorithm~\ref{IFAD}.
This requires  a Hessian estimate $H(\theta)$ with minimum eigenvalue no smaller than $c$, but we allow for the possibility of a trivial estiamte, $H=I_p$, in which case the AD search becomes stochastic gradient descent (SGD).
Algorithm~\ref{IFAD} is convenient for theoretical analysis, but two further modifications are helpful in practice.
...
However, even when applying much larger amounts of Monte Carlo effort, IF2 fails to get within seven \eic{ADJUST FOR BEST IF2 AND NEW UNDERSTANDING OF MLE} log-likelihood units of the maximum on this problem (Figure~\ref{fig:boxplot}).
...
We tested IFAD with $\alpha = 0, 0.97, 1$ against IF2 on a global search problem for the Dhaka cholera model using our implementation in \texttt{pypomp} 0.4.6.0 \citep{abkemeier25}.
For each method, we performed 100 searches, initialized with 100 starting parameter vectors drawn uniformly from the same wide bounding box used in \cite{ionides15}. 
To ensure a fair comparison under comparable computational effort, we run each method for about as long as it takes for IFAD-0.97 to converge, as it converges to the highest log-likelihood in the shortest time. 
For each method, we choose algorithmic parameters that maximize the log-likelihood best for that method while running within the allotted time. 
We run standard IF2 for 650 iterations, while IFAD-0.97 is run using a short IF2 warm-start of 175 iterations followed by the Adam optimizer for 175 iterations.
IFAD-0 and IFAD-1 are run for 60 IF2 iterations and 225 Adam iterations.
To assess whether other methods can catch up to IFAD-0.97 given more time, we separately run the IF2-only approach for four times as many iterations and the IFAD variants with four times as many Adam iterations, slowing the cooling rate for each as well.
A summary of these optimization settings is provided in \arxiv{Section~\ref{appendix:algparams}}{Section~S9 of the supplement}, and the complete source code to reproduce the numerical results is available at \href{https://github.com/pypomp/dmop}{https://github.com/pypomp/dmop}.

We summarize our findings in Table \ref{table:mle} and Figures \ref{fig:boxplot} and \ref{fig:optim}. 
Because estimating the log-likelihoods at every iteration of IF2 with particle filtering for Figure \ref{fig:optim} lengthens the computation time and thus compromises the fair comparison between methods, we obtain the times in Table \ref{table:mle} and Figure \ref{fig:optim} by running the methods again without the extra calls to the particle filter. 
...
\aja{The statement about beating AK with IFAD is less impressive when we can also beat it with IF2. Maybe we shouldn't compare to past attempts?}
Meanwhile, \cite{ionides15} only achieve a maximum log-likelihood of $-3768.6$, and \cite{king08} only $-3793.4$.

Examining Table \ref{table:mle}, IFAD-0.97 can attain a log-likelihood of -3744.17, which surpasses IF2 even when IF2 runs for 4 times as long.
IFAD-1 comes close in the time it takes for IFAD-0.97 to hit its maximum, but it needs to be run for an extended time to reach that maximum and partly close the gap in median log-likelihood. 
IFAD-0 falls close to 4 log-units short of IFAD-0.97 even with extended time, ostensibly due to having high bias. 
Based on Figure \ref{fig:optim}, IFAD-0 and IF2-only plateau heavily after being run for long enough and ostensibly would not catch up to IFAD-0.97 in a reasonable amount of time. 

Notably, the estimates for IFAD-0.97 frequently display much less Monte~Carlo variance than that of the other methods, as can be seen in Figure~\ref{fig:boxplot}. 

\aja{Choose a parameter estimate density plot to display?}
...
While IF2 quickly approaches a neighborhood of the MLE (Figure \ref{fig:optim}), IF2 alone ultimately fails to achieve the last few log-likelihood units, as IF2 struggles to get much closer than about 3 log-likelihood units of the MLE even when run for an extended time (Table \ref{table:mle}).
Conversely, we have encountered many failed searches with gradient descent alone. 
...
IFAD, in comparison, approaches the MLE quickly due to the IF2 warm-start (Figure \ref{fig:optim}) and also succeeds at refining the coarse solution found by the warm-start with DMOP gradient steps to find the MLE.
...
Since replicated searches are inevitably needed for Monte~Carlo maximization of this challenging likelihood maximization task, it is necessary to focus on the right tail of the distributions in Figure~\ref{fig:boxplot} to compare the capabilities of the methods. 
...
\begin{figure}[ht]
    ...
    %\arxiv{}{\vspace*{-1mm}}
    \caption{Raincloud plot of all searches. IFAD-0.97 outperforms all other methods.
...
\begin{figure}[ht]
    ...
    \arxiv{}{\vspace*{-1mm}}
    \caption{Optimization progress of IFAD and IF2.
...
The source code for this paper is at \href{https://github.com/pypomp/dmop}{https://github.com/pypomp/dmop}.
...
\eic{REWRITE WITH LATEST PYPOMP RESULTS} Implementation of the particle filter, simulator, and DMOP-$\alpha$ in \texttt{JAX} \citep{jax} takes advantage of just-in-time compilation and GPU acceleration, even with a simulator written in Python.
