...
\kevin{Suggested assumption edits for the revised supplement: retain (A3) as a bound on the partial measurement-density derivative at fixed state. Add (A6), ``First-Order Model Regularity'': for fixed simulator random numbers, the simulated path and log measurement densities are continuously differentiable near $\phi$; differentiation may pass through the expectations defining the filter; and, separately, the realized total pathwise DMOP mark $\nabla_\theta\log g_{n,j}^{A,F,\theta}$ is bounded by $B_1G'(\theta)$, enlarging the existing scale $G'(\theta)$ if necessary. The supplement derives the pathwise derivative identity directly by differentiating the simulator integral. A sufficient condition for the total-mark bound is that the fixed-seed simulator Jacobian in its parent state has norm at most $\kappa<1$, its parameter Jacobian and initial-state derivative are bounded, and the state and parameter derivatives of the log measurement density are bounded; the tangent recursion then gives a geometric series bounded by $(1-\kappa)^{-1}$. The supplement proves deterministic lagwise forgetting from first principles using the posterior backward-kernel minorization $\underbar M/\bar M$. Theorem~2 of \citet{chanDelMoral18} gives $O(n/J)$ weak bias for each bounded full-genealogy lag coefficient, hence $O\{n/[J(1-\alpha)]\}$ bias for the $n$th discounted conditional score. A uniform $O(J^{-1})$ conditional-score bias would require the additional geometric integrated-genealogy estimate stated separately in Section~S7. Add (A7), ``Primitive-Innovation Continuation Stability,'' for the variance proof: order the independent initial-state simulator variables, propagation variables, and multinomial-resampling uniforms chronologically, and bound the change in the conditional continuation caused by replacing any one primitive block. This avoids treating after-resampling particles as independent before their shared prediction cloud has been revealed. The proof uses multinomial resampling; systematic resampling remains the practical default.}
...
\kevin{Suggested replacement for Theorem~\ref{thm:mop-biasvar}, under (A2), (A3), the proposed first-order model-regularity assumption (A6), the separate primitive-innovation continuation assumption (A7), multinomial resampling, $\theta=\phi$, and $\alpha\in[0,1)$: $\E\|\nabla_\theta\hat\ell^\alpha-\nabla_\theta\ell\|_2^2\lesssim pG'(\theta)^2\big[N/\{J(1-\alpha)^2\}+N^2\{\varrho(1-\alpha)/((1-\varrho)(1-\alpha\varrho))+N/[J(1-\alpha)]\}^2\big]$, and $\Var(\nabla_\theta\hat\ell^\alpha)\lesssim NpG'(\theta)^2/\{J(1-\alpha)^2\}$. The constant is independent of $N$, $J$, and $\alpha$. Only the variance term uses (A7); its proof reveals the independent initial, propagation, and resampling blocks one at a time. The population-discount bias follows from deterministic derivative forgetting, while the particle term follows from the full-genealogy path-space weak-error bound.}
