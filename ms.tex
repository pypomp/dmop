...
There is also a sharper result for correctly specified random data.  The
per-time difference between the exact and DMOP-$\alpha$ population scores
has conditional mean zero given the previous observations.  Its cross-time
inner products therefore vanish, and Section~S7 proves
\[
...
sandwich-efficiency loss rather than a loss of consistency rate.}
...
remainders sum to $N^2/\{J^2(1-\alpha)^2\}$.  The current object used in
...
the gradient of its log weight; the unresolved issue is covariance
