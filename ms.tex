...
\begin{enumerate}[label=(A\arabic*),itemsep=-1ex, , font=\small] 
    ...
    \kevin{For the finite-difference proof in Section~S5, (A4) should be strengthened locally as follows. For fixed $N$ and baseline $\phi$, there is an open neighborhood $U$ of $\phi$ on which the fixed-seed MOP-$1$ reconstruction is twice continuously differentiable in its evaluation parameter and $\sup_{J\geq1}\sup_{\theta\in U}\|\nabla_\theta^2\hat\ell_J^1(\theta)\|_{\mathrm{op}}<\infty$ almost surely. The baseline, random seed, and first-pass resampling indices are held fixed as the evaluation parameter varies.}
...
\kevin{Suggested assumption edits for the revised supplement: clarify in (A3) that $\nabla_\theta$ is the total derivative through the reparameterized simulator. Add an assumption (A6), ``Second-Order Tangent Stability'': the normalized Feynman--Kac flow and its differentiated MOP recursion admit first- and second-order perturbation expansions; a perturbation introduced $r$ filtering steps in the past contributes at most $C_0\rho^r$ times its size to the first-order conditional-score perturbation and at most $C_0\rho^r$ times its squared size to the second-order remainder, uniformly in time and $\alpha\in[0,1]$, for constants $C_0<\infty$ and $\rho\in(0,1)$. This is the differentiated analogue of strong filter stability. It can be verified by adapting the tangent-semigroup estimates of \citet{delMoral15derivative,tadic18}, for example under uniform bounds on the first two log-derivatives of the initial, transition, and measurement densities together with differentiation under the integral sign. The algorithm remains plug-and-play because it does not evaluate the transition density or its derivatives. The proof uses multinomial resampling; systematic resampling remains the practical default.}
...
\kevin{The Arzel\`a--Ascoli outline above should be replaced. The proof in Section~S5 uses coordinatewise finite differences: Taylor's theorem bounds the difference between $\partial_i\hat\ell_J^1(\phi)$ and $\{\hat\ell_J^1(\phi+h e_i)-\hat\ell_J^1(\phi)\}/h$ by $H^*h/2$. Strong consistency is needed only at the countable collection of points $\phi$ and $\phi+h_r e_i$. Taking $J\to\infty$ and then $h_r\downarrow0$ proves gradient consistency without interchanging a derivative and an unproved uniform limit.}
...
\kevin{Suggested replacement for Theorem~\ref{thm:mop-biasvar}, under (A2), (A3), the proposed second-order tangent-stability assumption (A6), multinomial resampling, $\theta=\phi$, and $\alpha\in[0,1)$: $\E\|\nabla_\theta\hat\ell^\alpha-\nabla_\theta\ell\|_2^2\lesssim pG'(\theta)^2\big[N/\{J(1-\alpha)^2\}+N^2\{\rho(1-\alpha)/((1-\rho)(1-\alpha\rho))+J^{-1}\}^2\big]$, and $\Var(\nabla_\theta\hat\ell^\alpha)\lesssim NpG'(\theta)^2/\{J(1-\alpha)^2\}$. The constant is independent of $N$, $J$, and $\alpha$.}
...
\kevin{Suggested replacement proof outline: expand the differentiated particle flow around its deterministic counterpart. Conditional centering removes the first-order sampling terms, while second-order tangent stability makes the remaining $J^{-1}$ contributions geometrically summable. This gives finite-particle bias $O(J^{-1})$ per conditional score, uniformly in time and $\alpha$. The other bias is the population error from discounting historical score contributions and vanishes as $\alpha\uparrow1$. Near one, the leading MSE terms are $N/\{J(1-\alpha)^2\}+N^2(1-\alpha)^2+N^2/J^2$; balancing suggests $1-\alpha\asymp(NJ)^{-1/4}$ and MSE $\lesssim N^{3/2}/\sqrt J+N^2/J^2$.}
...
\kevin{Suggested correction to this endpoint discussion: \citet{poyiadjis11} give an asymptotic-variance lower bound that is a positive quadratic polynomial in $N$ under their nondegeneracy condition, not a sharp $N^4/J$ variance law. A generic path-functional upper bound can be $O(N^4/J)$: rescaling an oscillation-one result to an $O(N)$ additive score contributes $N^2$ to MSE, and the path-space expansion can contribute another $N^2$. Thus oscillation normalization alone does not create the quartic power. Under non-cancellation of the DMOP-$0$ population bias and a uniform Poyiadjis nondegeneracy lower bound, the interior choice above is strictly better in rate than both endpoints when $1\ll J\ll N$; these lower-bound qualifications are essential.}
