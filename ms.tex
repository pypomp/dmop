...
\kevin{Suggested clarifications to the existing assumptions, rather than a new assumption. In (A3), specify that the displayed derivative is the total fixed-random-number derivative $\nabla_\theta\log g_{n,j}^{A,F,\theta}$ through the reparameterized simulated state, uniformly in $n,j$ and in a neighborhood of $\phi$; enlarge $G'(\theta)$ to a finite bound on that neighborhood if necessary. In (A5), specify that, for fixed base simulator random numbers and fixed reference-filter resampling indices, the complete recursively simulated path and the composed log measurement densities have the stated continuous derivatives in $\theta$, and that the base random-number law does not depend on $\theta$. The product rule then gives $\|\nabla_\theta\prod_{s=1}^nG_s(X_s)\|_\infty\leq n\bar G^nG'(\theta)$, so dominated convergence justifies differentiation under every finite-horizon simulator expectation. No primitive-innovation continuation assumption is needed for the revised theorem. Assumption (A2) gives geometric forgetting of the population lag coefficients through the posterior backward kernels, but it does not imply geometric mixing of the inherited simulator tangent. The finite-particle proof therefore uses the historical path-space bounds of \citet{delMoral04,chanDelMoral18}, which apply to the actual bounded ancestral mark. The resulting generic variance is $O(N^3pG'(\theta)^2/J)$ and the full particle bias is $O(N^2\sqrt p\,G'(\theta)/J)$. The proof uses multinomial resampling; systematic resampling remains the practical default.}
...
\kevin{Suggested replacement for Theorem~\ref{thm:mop-biasvar}, under (A2), the clarified total-derivative form of (A3), the clarified fixed-random-number form of (A5), multinomial resampling, $\theta=\phi$, and $\alpha\in[0,1]$: there are $C<\infty$ and $\varrho\in(0,1)$, independent of $N,J$, and $\alpha$, such that $\E\|\nabla_\theta\hat\ell^\alpha-\nabla_\theta\ell\|_2^2\leq CpG'(\theta)^2\big[\min\{N^2,N^3/J\}+N^2\{\varrho(1-\alpha)/((1-\varrho)(1-\alpha\varrho))\}^2+N^4/J^2\big]$ and $\Var(\nabla_\theta\hat\ell^\alpha)\leq CpG'(\theta)^2\min\{N^2,N^3/J\}$. In particular, the MSE is at most $CpG'(\theta)^2\{N^3/J+N^2(1-\alpha)^2+N^4/J^2\}$. For each fixed $N$, setting $\alpha=1$ recovers the ordinary $O(J^{-1})$ MSE rate. The $N^3/J$ term is the generic historical path-space upper bound, not a sharp Poyiadjis rate.}
...
\kevin{Suggested replacement proof outline: the clarified (A3) and (A5) justify differentiation under each finite-horizon simulator expectation and identify the population recursion with posterior averages of the realized marks. Under (A2), the posterior backward kernels contract in oscillation by $1-\underbar M/\bar M$, which gives the displayed population discount bias. Sum the conditional-score decomposition over time and perform summation by parts. This writes the full DMOP-$\alpha$ score as $(1-\alpha)$ times historical particle averages at times $1,\ldots,N-1$, plus one terminal historical average. The $O(n/J)$ historical weak-bias estimate of \citet{chanDelMoral18} then gives full particle bias $O(N^2\sqrt p\,G'(\theta)/J)$ uniformly in $\alpha$. The historical occupation-measure variance bound of \citet{delMoral04} gives $L^2$ error $O(\sqrt{n/J})$ for a norm-one path functional; after the same summation-by-parts calculation and the $O(N)$ scale of the unnormalized score, this gives variance $O(N^3pG'(\theta)^2/J)$. Combining variance, particle bias, and population discount bias proves the theorem. Section~S7 also records the precise additional centered two-lineage covariance estimate that would improve the variance to $O\{N/[J(1-\alpha)]\}$ and yield the conditional interior rate $O\{N^{4/3}/J^{2/3}+N^2/J^2\}$, but (A2)--(A5) do not prove that estimate for inherited simulator tangents. Replace the preceding reference to Section~\suppSecProofBiasVar by a reference to Section~S7, where the full replacement proof is given.}
\kevin{Audit of the other SI variance arguments: the current-cloud covariance calculation does not establish the missing finite-particle lag covariance. The particle-system contraction used there has blocks of order $\log J$, so the covariance sum retains a factor $1+\min(N,\log J)$ even for a genuinely current-cloud mark. More importantly, the total fixed-random-number derivative carries the inherited simulator tangent, and truncating the discounted weights does not truncate that tangent. Thus neither the DMOP-$0$ calculation nor the truncated-score calculation can be reused for the general theorem under (A2)--(A5). The uniform filter-derivative stability theorem of \citet{delMoral15derivative} concerns the distinct $O(J^2)$ backward derivative estimator and likewise does not prove this covariance for the $O(J)$ DMOP genealogy.}
