...
\aja{I might want to add results for how high the other methods get when allowed to run for longer.}
We summarize our findings in Table \ref{table:mle}. 
\aja{Should indicate that we ran separate runs for recording the times and recording the intermediate log-lik estimates.}
...
\caption{Maximum log-likelihood found by IF2 and IFAD. IFAD-0.97 performs the best among all methods in about the same time span.}
\arxiv{}{\vspace*{-7mm}}
\label{table:mle}
\end{table}
...
The results of Table~\ref{table:mle} and Figure~\ref{fig:boxplot}) indicate that the actual MLE is around -3744.17. 
IFAD has effectively solved a long-standing benchmark problem.
%%
...
While IF2 quickly approaches a neighborhood of the MLE within only 40 iterations (Figure \ref{fig:optim}), IF2 alone ultimately fails to achieve the last few log-likelihood units, as no IF2 search comes within 3.5 log-likelihood units of the MLE. 
Conversely, we have encountered many failed searches with gradient descent alone. 
This is a difficult, nonconvex, and noisy problem with $18$ parameters, and gradient descent without a warm-start gets stuck in local minima and saddle points.

IFAD, in comparison, approaches the MLE quickly due to the IF2 warm-start (Figure \ref{fig:optim}) and also succeeds at refining the coarse solution found by the warm-start with DMOP gradient steps to find the MLE (Figure \ref{fig:boxplot}).
...
In particular, Figure~\ref{fig:boxplot} shows that IFAD with $\alpha=0.97$ can maximize the challenging likelihood of \cite{king08} in a short time, whereas previously it was necessary to carry out an extensive customized search or to live with an incompletely maximized likelihood.
...
\begin{figure}[ht]
    \centering
    \arxiv{}{\vspace*{0.1ex}}
    \includegraphics[width=\arxiv{12cm}{\textwidth/\real{2.2}}]{imgs/precise_raincloud.pdf}
    %\arxiv{}{\vspace*{-1mm}}
...
\begin{figure}[ht]
    \centering
    \vspace{1mm}
    \includegraphics[width=\arxiv{14cm}{\textwidth/2}]{imgs/precise_traces.pdf}
    \arxiv{}{\vspace*{-1mm}}
