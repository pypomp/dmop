...
\begin{algorithm}[htbp!]
    ...
    \label{alg:ifad}
		...
		\hspace*{4mm} Using fresh random numbers, run Algorithm \ref{alg:mop} to obtain $\hat\loglik(\theta_m)$ and set $g_m := \nabla_{\theta}(-\hat\loglik(\theta_m))$ (for Theorem~\ref{thm:linear}, use $\alpha=1$, the after-resampling estimate, and multinomial resampling). \newline
...
\begin{thm}[Finite-horizon Linear Convergence of IFAD] \label{thm:linear}
Let $f=-\ell$ and suppose that, on a convex region containing the iterates and
the minimizer $\theta^*$,
\(
\gamma I_p\preceq\nabla^2 f(\theta)\preceq\Gamma I_p
\)
with $0<\gamma\leq\Gamma$.  Fix a maximum number
$M\in\mathbb N$, $M\geq1$, of second-stage iterations,
$\delta\in(0,1)$, and $\epsilon>0$.  At iteration $m$, use a fresh
particle-randomness block to compute the after-resampling DMOP-$1$ gradient
\(
g_m=-\nabla_\theta\hat\ell^1(\theta_m)
\)
with $J$ particles and multinomial resampling, and use a curvature matrix satisfying
\(
cI_p\preceq B_m\preceq CI_p
\)
for fixed $0<c\leq C$.  Assume the constants in the gradient-concentration
lemma of Section~\suppSecProofOptimizationConvergence{} are uniform on this
region, and choose $J$ according to that lemma with $\delta$ replaced by
$\delta/M$.

For any $\beta\in(0,1)$, choose
\begin{equation}\nonumber
0<\eta\leq \frac{c(1-\beta)}{\Gamma},
\qquad
\sigma\geq \frac{2C}{c(1-\beta)},
\end{equation}
and stop when $\|g_m\|_2\leq\sigma\epsilon$.  Then, with probability at least
$1-\delta$, simultaneously for every executed iteration $m<M$, either the algorithm
has stopped and
\begin{equation}\nonumber
\ell(\theta^*)-\ell(\theta_m)
\leq \frac{(\sigma+1)^2\epsilon^2}{2\gamma},
\end{equation}
or its next step satisfies
\begin{equation}\nonumber
\ell(\theta^*)-\ell(\theta_{m+1})
\leq
\left\{1-\frac{2\eta\beta\gamma}{C}
\left(\frac{\sigma}{\sigma+1}\right)^2\right\}
\big\{\ell(\theta^*)-\ell(\theta_m)\big\}.
\end{equation}
...
\begin{enumerate}[label=(A\arabic*),itemsep=-1ex, , font=\small] 
    ...
    \item \textbf{Bounded Measurement Model.} There exist $\underbar{G}, \bar{G}$ such that $0<\underbar{G} \leq f_{Y_n \mid X_n}\left(y_n^* \mid x_n; \theta\right) \leq \bar{G}<\infty$.  In a neighborhood of the parameter under consideration, the total fixed-random-number derivative through the reparameterized simulated state is uniformly bounded: for every $n,j$ and every realization of the fixed simulator variables and reference-filter resampling indices, $\|\nabla_\theta^{\mathrm{tot}}\log g_{n,j}^{A,F,\theta}\|_\infty\leq G'(\theta)<\infty$.\label{assump:bounded-measurement}
...
\kevin{Suggested clarifications to the existing assumptions, rather than a new assumption.
In (A3), specify that the displayed derivative is the total fixed-random-number derivative
$\nabla_\theta\log g_{n,j}^{A,F,\theta}$ through the reparameterized simulated state,
uniformly in $n,j$ and near $\phi$.
In (A5), specify that, for fixed simulator random numbers and reference-filter resampling
indices, the recursively simulated path and composed log measurement densities have the
stated derivatives, and that the base random-number law does not depend on $\theta$.
The product rule then gives
$\|\nabla_\theta\prod_{s=1}^nG_s(X_s)\|_\infty\leq n\bar G^nG'(\theta)$,
so dominated convergence justifies differentiation under finite-horizon expectations.
No new primitive-innovation or tangent-stability assumption is needed: a differentiable
particle includes its location and simulator and weight tangents.
Under (A2)--(A3), a first-principles particle-update telescoping calculation gives
time-uniform weak bias $C\operatorname{osc}(f)/J$ for bounded current-particle
functionals.
The finite-particle variance proof uses Lemma~2.3 of \citet{karjalainen23}
for the prediction empirical measure.  For the filtered empirical average,
the supplement separately adds the conditional multinomial-resampling error
and the Bayes-ratio error
$\Psi_n(\eta_n^J)(f)-\Psi_n(\eta_n)(f)$; both are
$O\{\operatorname{osc}(f)/\sqrt J\}$ in $L^4$.
Particle-system mixing then controls the covariance sum; as elsewhere,
$\lesssim$ hides its mixing-time factor.
The proof requires multinomial resampling and, for the quoted particle-system theorem,
$J\geq2$; systematic resampling remains the practical default.}
...
there are $C<\infty$ and $\varrho\in(0,1)$, independent of $N,J,\alpha$, such that
\[
\E\|\nabla_\theta\hat\ell^\alpha-\nabla_\theta\ell\|_2^2
\lesssim pG'(\theta)^2\left[
J^{-1}\sum_{n=1}^N\left(\sum_{r=0}^{n-1}\alpha^r\right)^2
+J^{-2}\left(\sum_{n=1}^N\sum_{r=0}^{n-1}\alpha^r\right)^2
+N^2\left\{\frac{\varrho(1-\alpha)}
...
\lesssim pG'(\theta)^2
\min\left\{N^2,\,
J^{-1}\sum_{n=1}^N\left(\sum_{r=0}^{n-1}\alpha^r\right)^2\right\}.
\]
For fixed $\alpha<1$, the variance contribution is
$O\{NpG'(\theta)^2/[J(1-\alpha)^2]\}$ and the MSE is bounded by
\[
pG'(\theta)^2\left\{
\frac{N}{J(1-\alpha)^2}
+N^2(1-\alpha)^2
...
\right\}.
\]
At $\alpha=1$, the finite sums give orders $N^3/J$ and $N^4/J^2$ for
the variance and squared particle-bias terms.}
...
For the particle bias, telescope the local empirical updates.
After centering the empirical numerator, the one-update bias is the expectation of
the product of a numerator fluctuation and a random-normalizer fluctuation; each is
$O(J^{-1/2})$, so the bias is $O(J^{-1})$.
Geometric filter contraction makes the sum of these local weak biases uniform in time.
Applying this result to the current filtered-weight tangent gives full particle bias
\[
\frac{\sqrt p\,G'(\theta)}{J}
\sum_{n=1}^N\sum_{r=0}^{n-1}\alpha^r.
\]
For the variance, the current differentiable particle includes the accumulated
weight tangent.
The centered $L^4$ particle bound, Davydov's inequality, and particle-system mixing
give
\[
\Var(\nabla_\theta\hat\ell^\alpha)
\lesssim
\frac{pG'(\theta)^2}{J}
\sum_{n=1}^N\left(\sum_{r=0}^{n-1}\alpha^r\right)^2.
\]
Combining these two bounds with the population discount bias proves the theorem.
Section~S7 contains the full proof; it does not claim the stronger
$(1-\alpha)^{-1}$ variance factor.}
\kevin{Audit of the other SI variance arguments.
The original DMOP-$0$ and fixed-lag calculations are retained in the supplement.
Their use of $\lesssim$ intentionally suppresses the particle-system mixing-time
factor.
Once particle is read in the differentiated sense used by the
algorithm---location, simulator tangent, and weight tangent---the current-particle
covariance argument applies directly; there is no separate augmented-state or
tangent-stability gap.
The full DMOP-$\alpha$ calculation yields
$N/[J(1-\alpha)^2]$ for fixed $\alpha<1$.
No stronger $(1-\alpha)^{-1}$ factor is asserted.}
...
The finite-sum upper envelope displays opposing $\alpha$ directions, but hidden
constants and one-sided bounds do not prove that the actual MSE has an interior
minimizer.}
