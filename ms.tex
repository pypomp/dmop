...
\kevin{Suggested clarification to (A3) and (A5), and the separate extra
premise needed for the sharper Section~S7 particle bound.  The total
derivative in (A3) is, by the chain rule,
\[
...
\]
It is therefore generally a function of the physical location and its
fixed-random-number simulator gradient, or equivalently a functional carried
by the fixed-random-number simulator genealogy.  It is not generally a function of the
physical location alone.  The algorithm also carries the distinct
log-weight gradient, which satisfies
\[
...
+\left.\nabla_\theta^{\rm tot}\log g_n^\theta\right|_{\theta=\phi}.
\]
Thus $\alpha$ contracts the inherited log-weight gradient, but not the
simulator gradient inside the new measurement input.  Assumptions (A3) and
(A5) give boundedness and differentiability; (A2) mixes the physical
location.  They do not by themselves imply that the total measurement input
is a current-location function or that the location together with its
simulator gradient is mixing.  The sharper Section~S7 covariance calculation
therefore requires the current-location property as an explicit additional
premise, or a separate contraction condition for the simulator gradient.}
...
multinomial resampling, $J\geq2$, $\theta=\phi$, $\alpha\in[0,1)$, and the
additional premise that the bounded one-time measurement-gradient input
can be written as the current function $h_n(X_n)$, the following bounds
hold.  This current-function premise should be stated explicitly: bounded
total fixed-random-number derivatives in (A3) and differentiability in
(A5) do not by themselves imply it for an unaugmented physical state.
The differentiated computation carries the physical location, its simulator
gradient, and the gradient of its log weight.  The location transition is the
one in (A2), and differentiating the log-space weight recursion gives
...
the three-component computational state is assumed.  Under the stated
current-location premise, the physical backward kernels suffice.  Then
\[
\begin{aligned}
&\max\left\{
...
it is absorbed by the $N/J$ term.
The population discount-bias bound does not require the additional
current-location premise: the Section~S7 proof projects each bounded
genealogical input onto its physical location at the time that input is
created before applying the physical backward kernels.
...
\right\}.
\]
The proof conditions on the particle locations but not on the realized
ancestral indices.  It uses the exact backward genealogy representation of
\citet{chanDelMoral18}.  Assumption (A2) then minorizes every backward kernel
and gives geometric lag covariance.}

\kevin{Interpretation of the third MSE term.  Its two finite-horizon
upper bounds combine to give the contribution
\[
pG'(\theta)^2
\min\left\{
\left[\sum_{r=1}^{N-1}(N-r)(1-\alpha^r)\varrho^r\right]^2,\,
(1-\alpha)^2
\left[\sum_{q=0}^{N-2}(N-1-q)(\alpha\varrho)^q\right]^2
\right\}.
\]
The second bound follows from the exact summation-by-parts identity
\[
\nabla_\theta\ell(\theta)-\nabla_\theta\ell^\alpha(\theta)
...
\leq2\sqrt p\,G'(\theta)\varrho^q.
\]
It yields the simpler full-score bound
\[
\|\nabla_\theta\ell(\theta)-\nabla_\theta\ell^\alpha(\theta)\|_2
\leq
\frac{2N\sqrt p\,G'(\theta)(1-\alpha)}
{1-\alpha\varrho}.
\]
This removes the extra explicit slow-mixing factor
$\varrho/(1-\varrho)$ from the earlier lagwise simplification.  It does
not improve the powers of $N$ or $1-\alpha$.  Indeed, the homogeneous
two-state example in Section~S7 has
\[
\Delta_{n,r}
=\frac1{20}\left(\frac25\right)^{r-1},
\qquad
\nabla_\theta\ell(0)-\nabla_\theta\ell^\alpha(0)
\geq\frac{(N-1)(1-\alpha)}{20}.
\]
Thus the $N^2(1-\alpha)^2$ squared-bias order cannot be improved uniformly
under fixed observations, although particular models can have signed
first-order cancellation.
The first finite bound also has a geometric-cutoff interpretation:
$\alpha^r$ is the survival probability of a cutoff with probabilities
$(1-\alpha)\alpha^k$, including probability $1-\alpha$ of cutoff zero.
Algebraically,
\[
...
$C$ independent of $\alpha$.  More strongly, for fixed $N$, the exact
fixed-data discrepancy is a finite polynomial in $1-\alpha$; an
exponentially small bound would force every coefficient of that polynomial
to vanish.  The comparison is nevertheless favorable when $\alpha$ is
calibrated to the mixing time.  Put
$k=\lfloor\alpha/(1-\alpha)\rfloor$.  The ratio of the geometric scalar
bound to the fixed-lag scalar bound lies between
$1/2$ and $\varrho^{-k}$ before horizon saturation.  Thus the two uncapped
bounds differ by at most a fixed factor when $k|\log\varrho|$ is bounded,
meaning that discount memory spans at most a fixed number of mixing
e-folding times.}
...
Under the additional current-location premise stated above, the exact
conditional backward representation of a sampled particle genealogy from
\citet{chanDelMoral18}, together with (A2), gives
\[
...
$N^2/\{J^2(1-\alpha^2)\}$.  This does not assert total-variation mixing of
the differentiated computational state.  For a general total measurement
derivative, the simulator gradient is genealogical and the displayed
covariance need not decay under (A2).  The factor $\alpha$ discounts that
input only after it enters the log-weight-gradient recursion; it does not
create the missing covariance decay.}
\kevin{Audit of the other SI variance arguments.
Under the current-location premise, the DMOP-$0$ calculation remains valid
because no historical score is carried across resampling times.  Without
that premise, the total one-time measurement derivative can itself carry
the simulator genealogy, so physical-cloud mixing alone does not prove the
same rate.  The earlier fixed-lag calculation does not remain valid:
...
$\{N+(k+1)^2\}/J$.  Under the additional current-location premise stated
above, the backward genealogy calculation in Lemma~S13 controls covariance
among single-time score contributions that share a resampling genealogy and
gives
\[
\max\{\operatorname{MSE}_{\rm particle},\operatorname{Var}\}
\lesssim
pG'(\theta)^2
\left\{\frac{N(k+1)}J+\frac{N^2(k+1)}{J^2}\right\}.
\]
Combining this with the population truncation bound adds
$pG'(\theta)^2N^2\varrho^{2(k+1)}/(1-\varrho)^2$ to the full MSE.
The proof does not identify an ancestral sum with a current-location
function; it applies location mixing to the individual inputs before
summing them.}
...
Under the additional current-location premise, DMOP-$0$ has variance
$O\{NpG'(\theta)^2/J\}$, and Lemma~S13 gives the corresponding sharpened
particle bound for $\alpha>0$.  Without that premise, (A2) mixes only the
physical cloud and does not control the simulator-genealogical total
...
Under the same premise, for $\alpha<1$, Lemma~S13 gives the sharper particle term proportional to
...
$N\lesssim J\lesssim N^2$.  Once $J\gtrsim N^2$, both the endpoint
$\alpha=1$ and every interior choice with
$1-\alpha\lesssim J^{-1/2}$ give the finite-horizon upper bound
$pG'(\theta)^2N^2/J$.  The exact finite-sum right side is nevertheless
strictly decreased by moving slightly left of one: its particle part has
positive derivative at $\alpha=1$, while its squared population-bias
part has derivative zero.  For $J\gg N^2$, its interior minimizer
satisfies $1-\alpha\asymp N/J$, but its improvement over $\alpha=1$ is
lower order.
The power $J^{-4/3}$ applies only in the joint regime $J\lesssim N$ and
is not a fixed-$N$ superparametric Monte Carlo rate: it is produced by
balancing the $N^2/J^2$ quadratic particle remainder against population
bias.  For fixed $N$, sufficiently large $J$ lies in the $N^2/J$ regime.
These are only bounds obtained by balancing the proved upper bound.  The signed population
