...
\begin{enumerate}[label=(A\arabic*),itemsep=-1ex, , font=\small] 
    ...
    for $1\leq a,b\leq p$. The derivatives include the dependence of the simulated state on $\theta$.\label{assump:bounded-measurement}
...
at $\varrho=\tfrac12$. As in the preceding variance argument,
$\lesssim$ hides the particle mixing and logarithmic factors; the other
constants depend on the model and stability bounds, independently of
$N,J,\alpha$. The finite geometric sum is
...
$NpG'(\theta)^2/J$, multiplied by a constant depending on $\alpha$.}
...
inequality for separated windows, with gap $t-s-k-r-1$ under (A6) or
$t-s-k$ under (A6-J). Summing covariances and then ages gives
\[
\Var\{\nabla_\theta\hat s_k^1(\theta)\}
\lesssim\frac{NpG'(\theta)^2(k+1)^2}{J},
\qquad
\|\E\nabla_\theta\hat s_k^1(\theta)
...
$N/J$ and, for $J\geq N$, the same particle-MSE order. The constant
