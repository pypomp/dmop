...
\kevin{Suggested clarifications to the existing assumptions, rather than a new first-order regularity assumption. In (A3), specify that the displayed derivative is the total fixed-random-number derivative $\nabla_\theta\log g_{n,j}^{A,F,\theta}$ through the reparameterized simulated state, uniformly in $n,j$ and in a neighborhood of $\phi$; enlarge $G'(\theta)$ to a finite bound on that neighborhood if necessary. In (A5), specify that, for fixed base simulator random numbers and fixed reference-filter resampling indices, the complete recursively simulated path and the composed log measurement densities have the stated continuous derivatives in $\theta$, and that the base random-number law does not depend on $\theta$. These clarifications imply the required differentiation under the finite-horizon simulator expectations: the product rule gives $\|\nabla_\theta\prod_{s=1}^n G_s(X_s)\|_\infty\leq n\bar G^nG'(\theta)$, so dominated convergence applies. Therefore no separate first-order regularity assumption is needed. Relabel Primitive-Innovation Continuation Stability as (A6): for the variance proof, order the independent initial-state simulator variables, propagation variables, and multinomial-resampling uniforms chronologically, and bound the change in the conditional continuation caused by replacing any one primitive block. The supplement proves deterministic lagwise forgetting from first principles using the posterior backward-kernel minorization $\underbar M/\bar M$. Theorem~2 of \citet{chanDelMoral18} gives $O(n/J)$ weak bias for each bounded full-genealogy lag coefficient, hence $O\{n/[J(1-\alpha)]\}$ bias for the $n$th discounted conditional score. A uniform $O(J^{-1})$ conditional-score bias still requires the additional geometric integrated-genealogy estimate stated separately in Section~S7. The proof uses multinomial resampling; systematic resampling remains the practical default.}
...
\kevin{Suggested replacement for Theorem~\ref{thm:mop-biasvar}, under (A2), the clarified total-derivative form of (A3), the clarified fixed-random-number form of (A5), the primitive-innovation continuation assumption (A6), multinomial resampling, $\theta=\phi$, and $\alpha\in[0,1)$: $\E\|\nabla_\theta\hat\ell^\alpha-\nabla_\theta\ell\|_2^2\lesssim pG'(\theta)^2\big[N/\{J(1-\alpha)^2\}+N^2\{\varrho(1-\alpha)/((1-\varrho)(1-\alpha\varrho))+N/[J(1-\alpha)]\}^2\big]$, and $\Var(\nabla_\theta\hat\ell^\alpha)\lesssim NpG'(\theta)^2/\{J(1-\alpha)^2\}$. The constant is independent of $N$, $J$, and $\alpha$. Only the variance term uses (A6); its proof reveals the independent initial, propagation, and resampling blocks one at a time. The population-discount bias follows from deterministic derivative forgetting, while the particle term follows from the full-genealogy path-space weak-error bound.}
...
\kevin{Suggested replacement proof outline: first, the clarified (A3) and (A5) imply differentiation under the finite-horizon simulator expectations because $\|\nabla_\theta\prod_{s=1}^n G_s(X_s)\|_\infty\leq n\bar G^nG'(\theta)$; applying dominated convergence identifies the differentiated population recursion with the posterior averages of the realized DMOP marks. Unroll that recursion by lag. Under (A2), the posterior backward kernels contract in oscillation by the factor $1-\underbar M/\bar M$; applying this contraction to the conditional expectation of the bounded realized DMOP mark proves that the deterministic lag contribution is geometrically small, giving population discount bias proportional to $1-\alpha$. For the particle term, apply Theorem~2 of \citet{chanDelMoral18} to the bounded total-derivative mark on the complete ancestral path. Its normalizer-ratio condition follows directly from (A2)--(A3), and it gives $O(n/J)$ bias for every lag coefficient of the $n$th conditional score. Summing the discounted lags gives $O\{n/[J(1-\alpha)]\}$ conditional-score bias. For the variance, order the $J(2N+1)$ independent initial, propagation, and multinomial-resampling blocks chronologically, form the associated Doob martingale, and use (A6) to bound the independent-copy difference for each block. The exact conditional independent-copy identity gives one increment variance equal to one half of that squared difference; summing the $J(2N+1)$ increments gives $NpG'(\theta)^2/\{J(1-\alpha)^2\}$. Near one, the proved leading MSE terms are $N/\{J(1-\alpha)^2\}+N^2(1-\alpha)^2+N^4/\{J^2(1-\alpha)^2\}$. Directly minimizing this bound gives $1-\alpha=\{1/(NJ)+N^2/J^2\}^{1/4}$ whenever this is at most one, and MSE $O\{pG'(\theta)^2(N^{3/2}/\sqrt J+N^3/J)\}$. The $N^3/J$ term is the square of the accumulated full-genealogy weak-bias bound, not the variance term. The sharper choice $1-\alpha=(NJ)^{-1/4}$ yields the conditional upper bound $O\{pG'(\theta)^2(N^{3/2}/\sqrt J+N^2/J^2)\}$ only if one additionally proves integrated particle-genealogy bias $O(\varrho^r/J)$; it does not establish dominance over either endpoint. Replace the preceding reference to Section~\suppSecProofBiasVar by a reference to Section~S7, where the full replacement proof is given.}
...
\kevin{Suggested correction to this endpoint discussion: under their mixing and nondegeneracy conditions, Theorem~1 of \citet{poyiadjis11} gives a quadratic lower bound for the central-limit asymptotic variance of their already unnormalized additive path estimator. In fixed-$N$, large-$J$ asymptotics the leading variance of the unscaled particle error corresponding to that central limit theorem is of order at least $N^2/J$; no additional $N^2$ normalization factor should be applied. The separate generic path-space $L^p$ upper bound cited there is $O(N^2/\sqrt J)$ in error, hence $O(N^4/J)$ after squaring, but it is not established as a sharp variance law. DMOP-$1$'s pathwise reparameterization marks are not generally identical particle by particle to the complete-data likelihood-ratio marks of \citet{poyiadjis11}, which include initial- and transition-density scores. Agreement of their population score targets therefore does not transfer the Poyiadjis lower bound to DMOP-$1$; a separate estimator-level asymptotic-variance argument is required. There is also no DMOP-$0$ MSE lower bound under the present assumptions, since its conditional population biases may cancel or vanish. Consequently, the proved and conditional upper bounds in Section~S7 do not establish that an interior $\alpha$ has lower MSE than either endpoint.}
