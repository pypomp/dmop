...
The bootstrap filter samples and scores one endpoint per month, applies the
death-count measurement density there, and resamples.  Its latent path thus
contains monthly endpoints only.  KSV bridge backward sampling is not needed
for these runs because the 19 internal states are not part of the SMC path.
...
iteration, the executable block method runs the monthly bootstrap filter,
draws one endpoint path from its forward genealogy, and takes a projected
stochastic Fisher-score step.  Because the Dacca complete pseudo-likelihood
...
$\widetilde p^{\rm block}_{\theta_{m-1}}(X\mid y)$ and one complete genealogy
