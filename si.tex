...
\frac{NpG'(\theta)^2}{J}
\begin{cases}
\left(\dfrac{1-\alpha^N}{1-\alpha}\right)^2,
&0\leq\alpha<1,\\[6pt]
N^2,
&\alpha=1.
\end{cases}
...
\citet{karjalainen23} applies to this predictor-cloud chain. We spell out
the passage from its Dobrushin bound to the score mixing coefficient.
Let $A$ be an event generated by the score history through time $m$, and
let $B$ be an event generated by scores from time $m+l$ onward. Let
$\mathcal F_{m+1}^P$ be the sigma-field generated by all variables through
the production of the prediction cloud $X_{m+1,1:J}^P$. The time-$m$
resampling indices enter both the time-$m$ score and the construction of
$X_{m+1,1:J}^P$, so $A\in\mathcal F_{m+1}^P$. Conditional on this
sigma-field, the Markov property gives, for a suitable future-tail kernel
$K$,
\begin{equation}
...
\end{equation}
For $l=1$, the same display follows from $a(1)\leq1$.
...
(\ref{eq:refined-dmop-alpha-mse}) can be minimized directly over
$\alpha\in[0,1]$ for given $N,J$, and $\varrho$.  There is no general
closed form for that finite-sum minimizer.  In the particle-rich regime
$J\geq N$, Cauchy--Schwarz gives
\begin{align}
&\frac1{J^2}
\left(
\sum_{n=1}^{N}\sum_{r=0}^{n-1}\alpha^r
...
\frac1J\sum_{n=1}^{N}
\left(\sum_{r=0}^{n-1}\alpha^r\right)^2.
\label{eq:particle-bias-absorbed-when-j-exceeds-n}
\end{align}
Thus the squared particle-bias term is absorbed into the particle-variance
term whenever $J\geq N$.  For $\alpha<1$, after omitting the fixed factors
in $\varrho$, the implicit constants, and the particle-system mixing-time
factor suppressed by $\lesssim$, the resulting $\alpha$-dependent envelope
is
\begin{equation}
\frac{N}{J(1-\alpha)^2}+N^2(1-\alpha)^2.
\label{eq:simplified-alpha-envelope}
\end{equation}
Differentiating Equation~(\ref{eq:simplified-alpha-envelope}) with respect
to $1-\alpha$ gives the rate-balancing choice
\begin{equation}
1-\alpha
\asymp
(NJ)^{-1/4}.
...
\end{equation}
At this choice, Equation~(\ref{eq:simplified-alpha-envelope}) is of order
\begin{equation}
\frac{N^{3/2}}{\sqrt J}.
\label{eq:rate-balanced-mse-envelope}
\end{equation}
At the choice in Equation~(\ref{eq:rate-balancing-alpha-choice}),
$N(1-\alpha)\asymp N^{3/4}J^{-1/4}$.  Hence this balance applies before
the geometric sums saturate at the horizon when
$N\lesssim J\lesssim N^3$.

For $J\gg N^3$, the geometric upper bounds used in
Equation~(\ref{eq:refined-dmop-alpha-mse-simplified}) are no longer sharp
at the minimizing value.  Indeed, when $N(1-\alpha)$ is small,
\begin{equation}
\sum_{r=0}^{n-1}\alpha^r
=
...
\end{equation}
Substitution into the exact finite-sum envelope shows that its first-order
decrease away from $\alpha=1$ is of order
$N^4(1-\alpha)/J$, whereas the population-discount term is of order
$N^2(1-\alpha)^2$.  For $J\gg N^3$, balancing these terms gives
\begin{equation}
1-\alpha\asymp\frac{N^2}{J},
\qquad
...
N^3/J, & J\gtrsim N^3.
\end{cases}
\label{eq:finite-horizon-alpha-regimes}
\end{equation}
When $J<N$, Equation~
(\ref{eq:particle-bias-absorbed-when-j-exceeds-n}) does not absorb the
squared particle-bias term.  Minimizing the unabsorbed simplified envelope
then gives $1-\alpha\asymp J^{-1/2}$ and MSE at most of order $N^2/J$, up
to the same common and suppressed factors.

For completeness, the unabsorbed finite-sum envelope also gives a
ridge-type endpoint calculation.  For $N>1$, the left derivative at
$\alpha=1$ is
\begin{equation}
\frac1J\sum_{n=1}^{N}n^2(n-1)
+\frac{N^2(N+1)^2(N-1)}{6J^2}
>0,
\label{eq:mse-envelope-alpha-one-derivative}
\end{equation}
so moving left from $\alpha=1$ decreases the displayed envelope.  Its
right derivative at $\alpha=0$ is
\begin{equation}
2\left\{
\frac{N-1}{J}
+\frac{N(N-1)}{J^2}
-\frac{N^2\varrho^2}{1-\varrho}
\right\}.
\label{eq:mse-envelope-alpha-zero-derivative}
\end{equation}
Consequently, if
\begin{equation}
\frac{N-1}{J}+\frac{N(N-1)}{J^2}
<\frac{N^2\varrho^2}{1-\varrho},
\label{eq:mse-envelope-interior-condition}
\end{equation}
neither endpoint minimizes the displayed envelope, and at least one
minimizer lies in $(0,1)$.  These calculations optimize only the proved
upper envelope.  The hidden constants, suppressed mixing-time factor, and
one-sided inequalities prevent them from identifying the minimizer of the
actual MSE or proving that the actual MSE is smaller than at both
endpoints.
\end{remark}
...
for the proved upper envelope.  That calculation can establish an interior
minimizer of the displayed envelope under
Equation~(\ref{eq:mse-envelope-interior-condition}), but it does not
determine either endpoint derivative of the actual MSE in
Equation~(\ref{eq:actual-mse-alpha-derivative}).
\end{remark}
