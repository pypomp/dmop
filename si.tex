...
expectations defining the filtering recursion. In addition to the partial
derivative bound in Assumption {\refAssumpBoundedMeasurement}, the total
pathwise derivative used by DMOP satisfies, uniformly in $n,j$,
\begin{equation}
\left\|
...
most $B_1G'(\theta)$. Here and in the following display, vector norms are
Euclidean and matrix norms are the induced operator norms. Differentiating
one simulation step gives
\begin{align}
...
$X_0$ be the simulated initial state, generate $X_s$ recursively by
Equation~(\ref{eq:reparameterized-simulator-step}), and carry the resulting
pathwise derivatives. For every integrable functional $h$ of these
quantities, define
\begin{equation}
\rho_t(h)
=
...
\end{proof}
\begin{assumptionA7}[Primitive-Innovation Continuation Stability]
...
Assumption~(A7) concerns the finite-$J$, $O(J)$ interacting genealogy and
separates the two sources of randomness within each filtering time: the
...
{\refAssumpBoundedProcess}, {\refAssumpBoundedMeasurement}, and
{\refAssumpTangentStability}. The time-uniform results of
...
{\refAssumpTangentStability}, and (A7) hold, and multinomial resampling is
used. Then
\begin{equation}
\Var\left(\nabla_\theta\hat\ell^\alpha(\theta)\right)
\lesssim
...
filtration, and conditional expectations in Assumption~(A7). Equations~
...
\left\{
B_1G'(\theta)
+2B_1G'(\theta)\sum_{r=1}^{n-1}\alpha^r
\right\}                                                       \notag\\
&\leq
NB_1G'(\theta)
...
We next relate each increment to the replacement in Assumption~(A7).
...
\end{align}
Assumption~(A7) and Equation~(\ref{eq:replacement-doob-identity}) therefore
imply, almost surely,
\begin{align}
...
{\refAssumpBoundedMeasurement}, {\refAssumpTangentStability}, and (A7)
hold, and multinomial resampling is used. Then there are
...
\end{equation}
Thus, conditionally on an $N^2$ DMOP-$0$ lower bound, the proved upper bound
vanishes relative to that lower bound when $J/N\to\infty$. It does not, by
itself, prove an improvement over an $N^2/J$ DMOP-$1$ rate.
...
$N,J\to\infty$. Suppose also that the constants in Assumptions
{\refAssumpBoundedProcess}, {\refAssumpBoundedMeasurement},
{\refAssumpTangentStability}, and (A7), and the constant in Equation~
(\ref{eq:conditional-integrated-genealogy-bound}), can be chosen uniformly
along this sequence, with
\begin{equation}
\sup_{N,J}pG'(\theta)^2<\infty.
\label{eq:bounded-score-scale-joint-sequence}
\end{equation}
These uniformity conditions hold, in particular, when the model, $p$, and
$\theta$ are fixed and the assumptions are uniform in time.
Suppose that the \(\varrho\) in
Equation~(\ref{eq:conditional-integrated-genealogy-bound}) is the same
\(\varrho\) supplied by Theorem~\ref{thm:refined-dmop-alpha-mse} through
Lemma~\ref{lem:derivative-measure-forgetting}.
...
Moreover, the pathwise DMOP-$1$ score used here is not generally identical
particle by particle to their complete-data likelihood-ratio score, which
also contains initial- and transition-density score terms. Under the
differentiability conditions needed to define that complete-data score, and
when the corresponding derivative--expectation interchanges are valid, the
two constructions have the same population score target. The cited
quadratic result nevertheless serves only as motivation for the endpoint
assumption above.
