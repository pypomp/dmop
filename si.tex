...
\newtheorem*{assumptionA6}{Assumption (A6)}
\newtheorem*{assumptionA7}{Assumption (A7)}
...
{\refAssumpTangentStability} hold, Assumption~(A7) in
Section~\ref{appendix:sharper-biasvar} holds, and multinomial resampling is
used.
Then, for a constant independent of $N,J$, and $\alpha$,
\begin{align}
&\E\left\|
\nabla_\theta\ell(\theta)-\nabla_\theta\hat\ell^\alpha(\theta)
\right\|_2^2                                                    \notag\\
...
Section~\ref{appendix:sharper-biasvar}, gives the sharper population-bias
factor
\begin{equation}
\frac{\varrho(1-\alpha)}
{(1-\varrho)(1-\alpha\varrho)}.
\end{equation}
Since $1-\alpha\varrho\geq1-\varrho$, this factor is at most a constant
multiple of $1-\alpha$. Equation~(\ref{eq:dmop-alpha-conservative-mse})
therefore follows from Equation~(\ref{eq:refined-dmop-alpha-mse}), and
Equation~(\ref{eq:dmop-alpha-conservative-variance}) follows from
Lemma~\ref{lem:dmop-alpha-variance}.
\end{proof}

\section{Sharper Bias-Variance Analysis}
\label{appendix:sharper-biasvar}

The preceding truncation argument bounds two terms separately that cancel
as $\alpha$ approaches one. We now analyze the conditional-score increments
...
\begin{assumptionA6}[First-Order Model Regularity]
The initial density and, for every $n$, the product
\begin{equation}
f_{X_n\mid X_{n-1}}(x_n\mid x_{n-1};\theta)
f_{Y_n\mid X_n}(y_n^*\mid x_n;\theta)
\label{eq:one-step-model-kernel}
\end{equation}
are continuously differentiable in a neighborhood of $\phi$. The norm of
the derivative of the product in
Equation~(\ref{eq:one-step-model-kernel}) is at most
$B_1G'(\theta)$ times that product, uniformly in
$n,x_{n-1},x_n$, and $\theta$ in this neighborhood; the same bound holds
for the derivative of the initial density. Here $G'(\theta)$ in Assumption
{\refAssumpBoundedMeasurement} may be enlarged to cover these derivative
bounds. Differentiation may be passed under every integral
used by the filtering recursion. The derivative obtained by differentiating
the transition kernel agrees with the conditional expectation of the
reparameterized-simulator derivative used by DMOP. The derivative bound in
Assumption {\refAssumpBoundedMeasurement} is understood as the total
derivative actually used by DMOP: uniformly in $n,j$,
\begin{equation}
\left\|
...
Equation~(\ref{eq:bounded-realized-measurement-mark}) bounds the actual
term in the existing differentiated-weight recursion. It follows from more
primitive conditions, for example, if for every fixed random seed the
simulator Jacobian with respect to its parent state has operator norm at most
$\kappa<1$, its parameter Jacobian and the derivative of the initial
simulated state have norm at most $B_1$, and the state and parameter
derivatives of the log measurement density have norm at most
$B_1G'(\theta)$. Indeed, differentiating one simulation step gives
\begin{align}
\left\|\nabla_\theta X_{n,j}^{P,\theta}\right\|
&\leq
\kappa
\left\|\nabla_\theta X_{n-1,j}^{F,\theta}\right\|+B_1             \notag\\
&\leq
B_1\sum_{s=0}^{n}\kappa^s
\leq
\frac{B_1}{1-\kappa}.
\label{eq:simulator-tangent-geometric-bound}
\end{align}
The middle inequality follows by induction from the bound on the initial
derivative. Multinomial resampling copies the selected state derivative, so
the same bound holds after resampling. The measurement-density chain rule
then gives
\begin{align}
\left\|\nabla_\theta\log g_{n,j}^{A,F,\theta}\right\|
&\leq
\left\|\partial_\theta\log
f_{Y_n\mid X_n}(y_n^*\mid X_{n,j}^{F,\theta};\theta)\right\|          \notag\\
&\quad+
\left\|\nabla_x\log
f_{Y_n\mid X_n}(y_n^*\mid X_{n,j}^{F,\theta};\theta)\right\|
...
&\leq
B_1G'(\theta)
\left(1+\frac{B_1}{1-\kappa}\right).
\label{eq:primitive-mark-bound}
\end{align}
Thus the stated primitive conditions imply
Equation~(\ref{eq:bounded-realized-measurement-mark}) after enlarging
$B_1$. They are sufficient, not necessary; the proofs below use only
Equation~(\ref{eq:bounded-realized-measurement-mark}).

Assumption~{\refAssumpTangentStability} is a primitive first-derivative
model condition, not a particle-error bound. The density and derivative
clauses of (A6) are chosen to imply the bounded-observation, first-order case of
Assumptions 2.1--2.2 of
\citet{tadicHigher20}; the simulator compatibility and bounded-mark clauses are
additional conditions connecting that density calculation to DMOP. The
transition density and its derivatives are used only in the proof; DMOP
remains plug-and-play because the algorithm does not evaluate them.
...
the ancestral paths at time \(t\). The manuscript's existing \(\rho_t\)
is the corresponding population posterior path measure.
...
\(w_{n,j}^{P,\theta}=(w_{n-1,j}^{F,\theta})^\alpha\). There is no
unrecorded ancestor index in this term. In the MOP-$\alpha$ recursion in
the main manuscript, the discounted-weight accumulation and process
...
\end{align}
The simulator-compatibility and differentiation-under-the-integral clauses
of Assumption {\refAssumpTangentStability} identify the right-hand
population quantities with the lag coefficients of the deterministic
differentiated recursion. Thus
\begin{equation}
...
Under Assumptions {\refAssumpBoundedProcess},
{\refAssumpBoundedMeasurement}, and {\refAssumpTangentStability}, there are
...
\begin{proof}[Proof.]
We prove the result directly from the posterior update and the contraction
of the posterior backward kernels. This avoids assuming the desired
lagwise conclusion as part of Assumption
{\refAssumpTangentStability}.
...
\frac{
\rho_{n-1}(dx_{0:n-1})
M_n(x_{n-1},dx_n)G_n(x_n)
}
{ho_{n-1}M_n(G_n)}.
\label{eq:posterior-path-update}
\end{align}
Because \(h_{k,i}\) is measurable with respect to the path through time
\(k\leq n-1\), integrating Equation~
(\ref{eq:posterior-path-update}) first with respect to \(x_n\) gives
\begin{align}
\rho_n(h_{k,i})
&=
\frac{
...
\end{align}
The same inequality holds with zero left-hand side if \(\epsilon=1\).
...
\end{equation}
The physical state process is Markov, and all auxiliary simulator seeds or
pathwise derivatives through time \(k\) are measurable with respect to the
past. Therefore repeated conditioning through the backward kernels gives
\begin{align}
...
\end{align}
Applying Equation~
(\ref{eq:posterior-backward-oscillation-contraction}) successively to
Equation~(\ref{eq:iterated-backward-conditioning}) therefore gives
\begin{align}
&\operatorname{osc}\!\left[
\E_{\rho_{n-1}}\{h_{k,i}\mid X_{n-1}=\mathord\cdot\}
\right]                                                       \notag\\
&\qquad\leq
(1-\epsilon)^{r-1}\operatorname{osc}(H_k)                    \notag\\
&\qquad\leq
2B_1G'(\theta)(1-\epsilon)^{r-1}.
...
{\underbar G}                                                 \notag\\
&\leq
\frac{2B_1(\bar G-\underbar G)}{\underbar G}
G'(\theta)(1-\epsilon)^{r-1}.
...
Under Assumptions {\refAssumpBoundedProcess},
{\refAssumpBoundedMeasurement}, and {\refAssumpTangentStability}, with
...
\rho_t(h_{k,i})
=
\int h_{k,i}(x_{0:t})\,\rho_t(dx_{0:t}).
\label{eq:lag-functional-particle-population-pair}
\end{equation}
If the simulator stores random seeds or pathwise derivatives, these are
included in the ancestral path. At \(\theta=\phi\), they do not alter the
transition law of the physical state or the potential \(G_a\); hence the
future normalizer below depends on a path prefix only through its last
physical state. Equation~
...
We next verify, rather than assume, the stability condition used in
Theorem 2 of \citet{chanDelMoral18}. Let \(0\leq a\leq b\leq t\), and let
\(z=(x_0,\ldots,x_a)\) and \(z'=(x'_0,\ldots,x'_a)\) be two path prefixes.
If \(a=b\), then
\begin{equation}
Q_{a,a}(1)(z)=Q_{a,a}(1)(z')=1.
\label{eq:chan-zero-step-normalizer}
\end{equation}
If \(a<b\), the Markov property and the definition of the Feynman--Kac
semigroup give
\begin{align}
Q_{a,b}(1)(z)
&=
...
(\ref{eq:chan-normalizer-ratio}) verify Assumption A1 of
\citet{chanDelMoral18}, uniformly in \(a,b,t\), with a constant no larger
than \(\bar M\bar G/(\underbar M\underbar G)\).

We now record exactly what their Theorem 2 gives after translating their
particle count \(N\) to the present particle count \(J\), and their
path-space target \(\eta_t\) to the present posterior path measure
\(\rho_t\). There is a constant \(C<\infty\), depending only on
\(\bar M,\underbar M,\bar G,\underbar G\), such that, whenever
\(J\geq Ct\), every integrable real path functional \(f\) satisfies
\begin{equation}
\left|
\E m(\xi_t)(f)-\rho_t(f)
\right|
...
\end{equation}
For completeness, the signed version follows from the nonnegative version
in that theorem by writing
\begin{equation}
f=f^+-f^- ,
\qquad
|f|=f^++f^- ,
...
\left|\E m(\xi_t)(f^-)-\rho_t(f^-)\right|                   \notag\\
&\leq
C\frac{t}{J}\{\rho_t(f^+)+\rho_t(f^-)\}                    \notag\\
&=
C\frac{t}{J}\rho_t(|f|).
\label{eq:chan-signed-path-bias-derivation}
\end{align}

Suppose first that \(J\geq Cn\). Since \(t\leq n\) for
\(t\in\{n-1,n\}\), Equation~(\ref{eq:chan-signed-path-bias}) applies at
both times. Substituting \(f=h_{k,i}\), then using
Equation~(\ref{eq:bounded-single-lag-path-functional}), gives
\begin{align}
\left|\E m(\xi_t)(h_{k,i})-\rho_t(h_{k,i})\right|
&\leq
C\frac{t}{J}\rho_t(|h_{k,i}|)                               \notag\\
&\leq
C\frac{t}{J}\rho_t\{B_1G'(\theta)\}                        \notag\\
&=
\frac{CB_1G'(\theta)t}{J}                                   \notag\\
&\leq
\frac{CG'(\theta)n}{J},
...
\frac{CB_1G'(\theta)n}{J}
+
\frac{CB_1G'(\theta)(n-1)}{J}                               \notag\\
&\leq
\frac{2CB_1G'(\theta)n}{J}                                  \notag\\
&\leq
\frac{CG'(\theta)n}{J},
...
(\ref{eq:chan-path-bias-coordinate}), and then calculating the Euclidean
norm coordinate by coordinate, gives
\begin{align}
\left\|\E\Delta_{n,r}^J-\Delta_{n,r}\right\|_2
&=
\left\{
\sum_{i=1}^{p}
\left|\E\Delta_{n,r,i}^J-\Delta_{n,r,i}\right|^2
\right\}^{1/2}                                                \notag\\
&\leq
\left\{
\sum_{i=1}^{p}
...
Under the conditions of Lemma~\ref{lem:lagwise-weak-error}, for
\(\alpha\in[0,1)\),
\begin{equation}
\left\|
\E\nabla_\theta\hat\ell_n^\alpha(\theta)
...
\(\alpha\). Equation~(\ref{eq:unrolled-filtered-weight}) therefore gives
\begin{align}
&\E\nabla_\theta\hat\ell_n^\alpha(\theta)
-\nabla_\theta\ell_n^\alpha(\theta)                            \notag\\
&\qquad=
...
\end{proof}
\begin{assumptionA7}[Finite-Particle Continuation Stability]
For a coordinate $i$, let
$Z_i=\partial_{\theta_i}\hat\ell^\alpha(\theta)$, let
$\mathcal F_m^J$ contain all differentiated particles through time $m$,
and put $M_{m,i}=\E(Z_i\mid\mathcal F_m^J)$. Conditionally on
$\mathcal F_{m-1}^J$, replace differentiated offspring $j$ at time $m$ by
an independent copy from its conditional law, integrate out all later
particle randomness, and denote the resulting conditional expectation by
$M_{m,i}^{(j)}$. There is a constant independent of $N,J,m,i$, and
$\alpha\in[0,1)$ such that
\begin{equation}
\left\{
\E\left[(M_{m,i}-M_{m,i}^{(j)})^2
\mid\mathcal F_{m-1}^J\right]
\right\}^{1/2}
\leq
...
Assumption~(A7) is deliberately separate from Assumption
{\refAssumpTangentStability}. It concerns the finite-$J$, $O(J)$
interacting genealogy, whereas Lemmas~\ref{lem:derivative-measure-forgetting}--
\ref{lem:discounted-dmop-particle-bias} derive deterministic forgetting,
full-genealogy weak bias, and discounted particle bias from the
primitive model conditions. The time-uniform
results of \citet{delMoral15derivative,tadic18} establish the analogous
stability for their $O(J^2)$ backward derivative estimator; they do not by
themselves prove Equation~
(\ref{eq:finite-particle-continuation-stability}) for DMOP. We therefore
use (A7) only for the variance bound below.

\begin{lem}[Variance of the discounted genealogical score]
\label{lem:dmop-alpha-variance}
Under the conditions of
Lemma~\ref{lem:discounted-dmop-particle-bias} and Assumption~(A7), for
$\alpha\in[0,1)$,
\begin{equation}
\Var\left(\nabla_\theta\hat\ell^\alpha(\theta)\right)
\lesssim
...
$Z_i=\partial_{\theta_i}\hat\ell^\alpha(\theta)$. Let
$\mathcal F_m^J$ now include all differentiated particles through time $m$, and
...
Conditional on $\mathcal F_{m-1}^J$, the $J$ differentiated offspring at
time $m$ are independent. With $M_{m,i}$ and $M_{m,i}^{(j)}$ as in
Assumption~(A7), Equation~
(\ref{eq:finite-particle-continuation-stability}) gives the required
one-offspring replacement estimate. No deterministic filter-stability
claim is being transferred here to the finite-$J$ genealogy.

The conditional Efron--Stein inequality bounds the conditional second
moment of $D_{m,i}$ by one half of the sum of these $J$ squared replacement
changes. It therefore gives
\begin{equation}
\E(D_{m,i}^2\mid\mathcal F_{m-1}^J)
\leq
\frac{J}{2}
\left\{\frac{CG'(\theta)}{J(1-\alpha)}\right\}^2
\lesssim
\frac{G'(\theta)^2}{J(1-\alpha)^2}.
\end{equation}
Combining this bound with
Equation~(\ref{eq:dmop-doob-variance}) gives
\begin{equation}
\Var(Z_i)
\leq
...
Suppose $\theta=\phi$, $\alpha\in[0,1]$, and
Assumption~{\refAssumpTangentStability} holds. Then
\begin{equation}
\left\|
\nabla_\theta\ell_n^\alpha(\theta)
...
\begin{proof}[Proof.]
At $\theta=\phi$, every off-parameter weight ratio equals one, so the
deterministic differentiated recursion has the same coefficient identity as
Equation~(\ref{eq:unrolled-filtered-weight}). Thus a contribution inherited
through $r$ filtering updates has coefficient one
in $\nabla_\theta\ell_n(\theta)$ and coefficient $\alpha^r$ in
$\nabla_\theta\ell_n^\alpha(\theta)$. Lemma~
\ref{lem:derivative-measure-forgetting} bounds the Euclidean norm of the
undiscounted lag-$r$ contribution by
$C\sqrt p\,G'(\theta)\varrho^r$. The triangle inequality therefore gives
\begin{align}
\left\|
\nabla_\theta\ell_n(\theta)
-\nabla_\theta\ell_n^\alpha(\theta)
...
Lemmas~\ref{lem:discounted-dmop-particle-bias} and
...
using $2ab\leq a^2+b^2$. If $J\geq N$, the choice
\begin{equation}
\delta=\left(\frac{N}{J}\right)^{1/2}
...
\end{equation}
Thus the proved upper bound vanishes relative to an $N^2$ DMOP-$0$
lower bound when $J/N\to\infty$. It does not, by itself, prove an improvement
over an $N^2/J$ DMOP-$1$ rate.

\begin{cor}[Conditional comparison with the endpoint estimators]
...
$N,J\to\infty$. Suppose, in addition, that the integrated particle
genealogy satisfies
\begin{equation}
\left\|
\E\Delta_{n,r}^J-\Delta_{n,r}
...
\end{equation}
uniformly in $n,r,J$, and that there are constants
...
\begin{proof}[Proof.]
Equation~(\ref{eq:conditional-integrated-genealogy-bound}) and the lag
...
Theorem~\ref{thm:refined-dmop-alpha-mse} with this sharper particle-bias
bound gives
\begin{equation}
\operatorname{MSE}_{\alpha}
\lesssim
...
\end{equation}
With $\delta=(NJ)^{-1/4}$, the right side of
...
Equation~(\ref{eq:conditional-sharp-interior-mse}) have ratios $\sqrt{J/N}$ and
...
consequence of Assumption~{\refAssumpTangentStability}. The deterministic
filter-derivative stability theorem of \citet{tadicHigher20} and the
...
the corollary records precisely the additional particle-level estimate
needed for the sharper regime; it is not claimed under (A6)--(A7) alone.
...
\end{equation}
For any current-state test function $h_N$ with
...
A path-space theorem can contain a separate horizon factor. For example,
if $\widehat H_N$ estimates the exact path expectation of $H_N$, it may
only give
\begin{equation}
