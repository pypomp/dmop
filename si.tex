...
closed form for that finite-sum minimizer.  The simplified display gives
its rate in the regime where the geometric sums have not saturated at the
horizon.  Omitting the fixed factors in $\varrho$, the implicit constants,
and the particle-system mixing-time factor suppressed by $\lesssim$, its
$\alpha$-dependent part is
\begin{equation}
\left(\frac{N}{J}+\frac{N^2}{J^2}\right)(1-\alpha)^{-2}
+N^2(1-\alpha)^2.
...
\begin{equation}
\frac{N^{3/2}}{\sqrt J}
\left(1+\frac NJ\right)^{1/2}.
\label{eq:rate-balanced-mse-envelope}
\end{equation}
Thus, up to the common factor $pG'(\theta)^2$ and the suppressed factors,
the rate-balancing choice and resulting envelope are
\begin{equation}
1-\alpha\asymp
\begin{cases}
J^{-1/2}, & J\lesssim N,\\[3pt]
...
pG'(\theta)^2
\begin{cases}
N^2/J, & J\lesssim N,\\[3pt]
N^{3/2}/\sqrt J, & N\lesssim J\lesssim N^3.
\end{cases}
\label{eq:rate-balanced-alpha-regimes}
\end{equation}
...
$\{N^4/J+N^5/J^2\}(1-\alpha)$, whereas the population-discount term is of
order $N^2(1-\alpha)^2$.  For $J\gg N^3$, balancing these terms gives
\begin{equation}
...
\end{equation}
Combining the regimes gives the rate summary
\begin{equation}
1-\alpha\asymp
\begin{cases}
J^{-1/2}, & J\lesssim N,\\
...
The exact envelope also gives a ridge-type endpoint calculation.  For
$N>1$, the left derivative at $\alpha=1$ is
\begin{equation}
