...
\end{align}
because $1-\alpha\varrho\geq1-\varrho$. Substituting
Equation~(\ref{eq:population-bias-conservative-factor}) into
Equation~(\ref{eq:refined-dmop-alpha-mse}), while retaining its particle
bias term, gives
\begin{align}
&\E\left\|
\nabla_\theta\ell(\theta)-\nabla_\theta\hat\ell^\alpha(\theta)
...
\right],
\end{align}
which is Equation~(\ref{eq:dmop-alpha-conservative-mse}). Lemma~
\ref{lem:dmop-alpha-variance} gives Equation~
(\ref{eq:dmop-alpha-conservative-variance}) directly.
\end{proof}
...
For every fixed collection of simulator random numbers, the simulated state
path and the log measurement densities are continuously differentiable in a
neighborhood of $\phi$. Differentiation may be interchanged with the
expectations defining the filtering recursion. The derivative bound in
Assumption {\refAssumpBoundedMeasurement} is understood as the total
pathwise derivative used by DMOP: uniformly in $n,j$,
\begin{equation}
\left\|
...
We first give sufficient conditions for the two requirements in (A6).
Suppose a simulator step can be written as
\begin{equation}
X_n=\process_n(X_{n-1},U_n;\theta),
\label{eq:reparameterized-simulator-step}
\end{equation}
where the distribution of $U_n$ does not depend on $\theta$. For a
differentiable test function $q_\theta$ whose derivative is dominated by an
integrable function, differentiation under the integral and the chain rule
give
\begin{align}
&\nabla_\theta
\E\{q_\theta(\process_n(x,U_n;\theta))\}                       \notag\\
&\quad=
\nabla_\theta
\int q_\theta(\process_n(x,u;\theta))\,\nu(du)                \notag\\
&\quad=
\int
\nabla_\theta\{q_\theta(\process_n(x,u;\theta))\}\,\nu(du) \notag\\
&\quad=
\int
\left[
\partial_\theta q_\theta(\process_n(x,u;\theta))
+
\nabla_xq_\theta(\process_n(x,u;\theta))
\nabla_\theta\process_n(x,u;\theta)
\right]\nu(du)                                               \notag\\
&\quad=
\E\left[
\partial_\theta q_\theta(X_n)
+\nabla_xq_\theta(X_n)\nabla_\theta X_n
\mid X_{n-1}=x
\right].
\label{eq:pathwise-density-derivative-identity}
\end{align}
Applying Equation~(\ref{eq:pathwise-density-derivative-identity})
successively to the unnormalized filtering numerator and denominator, and
then applying the quotient rule, identifies the differentiated population
recursion with the conditional expectation of the pathwise derivative used
by DMOP. Thus this identification need not be imposed as a separate
stability assumption; domination sufficient for the displayed interchange
is enough.

It remains to give primitive conditions for the uniform bound in
Equation~(\ref{eq:bounded-realized-measurement-mark}). Suppose that, for
every fixed random seed, the simulator Jacobian with respect to its parent
state has operator norm at most $\kappa<1$, its parameter Jacobian and the
derivative of the initial simulated state have norm at most $B_1$, and the
state and parameter derivatives of the log measurement density have norm at
most $B_1G'(\theta)$. Differentiating one simulation step gives
\begin{align}
...
Assumption~{\refAssumpTangentStability} is therefore a first-order
simulation regularity condition, not a particle-error or filter-stability
assumption. The geometric population forgetting used below follows from the
backward-kernel contraction under Assumption
{\refAssumpBoundedProcess}; the particle weak-error bound is proved
separately.
...
We next verify the stability condition used in Theorem 2 of
\citet{chanDelMoral18}. Let \(0\leq a\leq b\leq t\), and let
...
Their Theorem 2, after translating their
...
\end{equation}
The signed version follows from the nonnegative version by writing
\begin{equation}
f=f^+-f^- ,
\qquad
|f|=f^++f^- ,
...
Assumption~(A7) concerns the finite-$J$, $O(J)$
...
\end{equation}
are orthogonal in $L^2$, and their sum is $Z_i-\E Z_i$. To verify these
claims, first note that $\mathcal F_N^J$ contains all randomness used to
construct $Z_i$, so
\begin{align}
\sum_{m=0}^{N}D_{m,i}
&=
\E(Z_i\mid\mathcal F_N^J)
-\E(Z_i\mid\mathcal F_{-1}^J)                                 \notag\\
&=
Z_i-\E Z_i.
\label{eq:dmop-doob-telescope}
\end{align}
If $l<m$, then $D_{l,i}$ is $\mathcal F_{m-1}^J$-measurable and
\begin{align}
\E(D_{l,i}D_{m,i})
&=
\E\left[
D_{l,i}\E(D_{m,i}\mid\mathcal F_{m-1}^J)
\right]                                                       \notag\\
&=0.
...
\begin{equation}
\E(M_{m,i}\mid\mathcal F_{m-1}^J)
=
\E(Z_i\mid\mathcal F_{m-1}^J)
=
M_{m-1,i}.
\label{eq:dmop-martingale-tower}
\end{equation}
Consequently,
\begin{align}
\E(D_{m,i}^2\mid\mathcal F_{m-1}^J)
&=
\E\{(M_{m,i}-M_{m-1,i})^2\mid\mathcal F_{m-1}^J\}           \notag\\
&=
\Var(M_{m,i}\mid\mathcal F_{m-1}^J).
\label{eq:dmop-increment-conditional-variance}
\end{align}
Conditional on $\mathcal F_{m-1}^J$, $M_{m,i}$ is a function of the $J$
conditionally independent offspring at time $m$. The conditional
Efron--Stein inequality, followed by Assumption~(A7), therefore gives
\begin{align}
\E(D_{m,i}^2\mid\mathcal F_{m-1}^J)
&\leq
\frac12\sum_{j=1}^J
\E\{(M_{m,i}-M_{m,i}^{(j)})^2\mid\mathcal F_{m-1}^J\}       \notag\\
&\leq
\frac{J}{2}
\left\{\frac{CG'(\theta)}{J(1-\alpha)}\right\}^2           \notag\\
&\leq
\frac{CG'(\theta)^2}{J(1-\alpha)^2}.
\label{eq:dmop-one-generation-variance}
\end{align}
Taking expectations in Equation~(\ref{eq:dmop-one-generation-variance})
and substituting into Equation~(\ref{eq:dmop-doob-variance}) give, for
$N\geq1$,
\begin{align}
\Var(Z_i)
&\leq
...
&=
\frac{C(N+1)G'(\theta)^2}{J(1-\alpha)^2}                    \notag\\
&\leq
...
\end{align}
because the $r=0$ coefficient is zero. Taking norms, applying the triangle
...
$1-\alpha\varrho=1-\varrho+\delta\varrho\geq1-\varrho$,
\begin{equation}
\frac{\varrho(1-\alpha)}
{(1-\varrho)(1-\alpha\varrho)}
\leq
\frac{\varrho}{(1-\varrho)^2}\delta
\lesssim
\delta.
...
\left[
\frac{N}{J\delta^2}
+N^2\left\{\delta+\frac{N}{J\delta}\right\}^2
\right]                                                       \notag\\
&\leq
CpG'(\theta)^2
\left\{
...
Equation~(\ref{eq:refined-dmop-alpha-mse}). If $J\geq N$, the choice
\begin{equation}
...
$N,J\to\infty$, with
\begin{equation}
\sup_{N,J}pG'(\theta)^2<\infty.
\label{eq:bounded-score-scale-joint-sequence}
\end{equation}
This condition holds, in particular, when $p$ and $\theta$ are fixed.
For $a\in\{0,\alpha,1\}$, define all three endpoint and interior errors
against the same full score by
\begin{equation}
\operatorname{MSE}_a
=
\E\left\|
...
\right\|_2^2.
\label{eq:common-endpoint-mse-definition}
\end{equation}
Suppose, in addition, that the integrated particle
genealogy satisfies
\begin{equation}
\left\|
\E\Delta_{n,r}^J-\Delta_{n,r}
