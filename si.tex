...
the sharper current-particle upper bound $O(N^2/J)$ at $\alpha=1$.
...
first- and second-order tangents propagated by automatic differentiation.  Set
\begin{align}
G_n^\theta(Z_n)
...
its tangents for $0\leq q\leq N-2$, and put
...
$O(N/J)$ for fixed $\alpha<1$ and $O(N^2/J)$ at $\alpha=1$, up to the
common parameter and mixing-time factors.
...
The precise prediction-to-filter $L^4$ bridge and the passage from
Theorem~3.1 of \citet{karjalainen23} to the score mixing coefficients are
written in Equations~(\ref{eq:karjalainen-predictor-l4})--
(\ref{eq:conditional-score-alpha-mixing}) below. At $\alpha=0$, they give
\begin{align}
\left\|
...
For example, the proof may take
$\varrho=1-\underbar M/(2\bar M)$, which lies strictly between
$1-\underbar M/\bar M$ and one in Equation~
(\ref{eq:backward-contraction-to-varrho}). Thus $\varrho$ depends only on
the model minorization bounds and is independent of $N,J,\alpha$, and any
lag truncation $k$.
...
their locations, simulator tangents, and weight tangents are coordinates of
the particle state, rather than an external process appended to the cloud.
Thus $f$ is a current-state functional in the Feynman--Kac model already
defined in Section~\ref{appendix:feynman}.  The current-state marginal of
...
\frac{NpG'(\theta)^2}{J}
\begin{cases}
1+\dfrac1N\left(\dfrac{1-\alpha^N}{1-\alpha}\right)^2,
&0\leq\alpha<1,\\[6pt]
1+N,
&\alpha=1.
\end{cases}
...
\end{align}
Apart from the
explicitly suppressed particle-system mixing-time factor, the
multiplicative constants are independent of $N,J$, and $\alpha$, and
$\Var(Z)=\E\|Z-\E Z\|_2^2$.
\end{lem}
...
particle state. We now apply the precise form of the particle result and
write out the filtering step that it does not cover directly.
...
$\pi_n^J=\Psi_n(\eta_n^J)$. If a filtered tangent coordinate is obtained
...
(\ref{eq:filtered-particle-l4}) to the two tangent functions, and using
...
weight-tangent average in Equation~
...
\begin{equation}
\frac{pG'(\theta)^2}{J}
\left\{N+\left(\frac{1-\alpha^N}{1-\alpha}\right)^2\right\},
\end{equation}
so its horizon order is $N/J$.  The dependence on $\alpha$ is confined
to the single terminal current-particle average.
Because the differentiable particle includes its location and tangents,
no separate augmented-state or tangent-stability assumption is used.
\end{remark}
...
\begin{remark}[Choice of $\alpha$ within the proved MSE envelope]
\label{rem:alpha-envelope-choice}
...
\label{eq:simplified-alpha-envelope}
\end{equation}
...
Equation~(\ref{eq:simplified-alpha-envelope}) gives
\begin{align}
0
&=
\frac{d}{d(1-\alpha)}
...
(\ref{eq:simplified-alpha-envelope}) are each of order $N/\sqrt J$.
...
\right\}.
\label{eq:rate-balanced-mse-envelope}
\end{equation}
...
(\ref{eq:rate-balanced-mse-envelope}) is a property of the proved uniform
upper envelope, not an actual-MSE lower bound.  Indeed,
\begin{equation}
\nabla_\theta\ell(\theta)-\nabla_\theta\ell^\alpha(\theta)
=
...
smaller than this envelope.  No such cancellation is assumed in
...
simulator tangent, and weight tangent are all functionals of the current
...
\end{equation}
This upper envelope can therefore be sharper than the tuned
DMOP-$\alpha$ envelope.  The reason is not an extra factor of $N$ in the
...
assumptions.  This comparison concerns the proved upper envelopes and
...
location and tangents, this statement uses no separate tangent-stability
assumption.
\end{remark}
...
Remark~\ref{rem:alpha-envelope-choice} gives the corresponding calculation
for the proved upper envelope.  It identifies rate-balancing interior
...
$X_n^{\theta_0}$ and its tangent, the next observation has density
...
not a loss of the $N^{-1/2}$ parameter rate.  Under uniform-in-parameter
extensions of Lemmas~\ref{lem:dmop-full-particle-bias} and
\ref{lem:dmop-alpha-variance}, the finite-$J$ score bias $O(N/J)$ gives a
parameter displacement $O(J^{-1})$, while the fixed-$\alpha$ particle
score variance $O(N/J)$ gives parameter variance $O\{(NJ)^{-1}\}$. Thus
first-order equivalence with the population root requires, in particular,
$\sqrt N/J\to0$; these uniform stochastic extensions are not asserted by
Proposition~\ref{prop:dmop-alpha-parametric-inference}.
