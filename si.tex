...
Assumption~{\refAssumpTangentStability} supplies the first-order simulation
regularity used below. Geometric population forgetting follows from the
backward-kernel contraction under Assumption
{\refAssumpBoundedProcess}, and the particle weak-error bound follows from
the full-genealogy argument in Lemma~\ref{lem:lagwise-weak-error}.
...
with potential \(G_t\), followed by the identity mutation. The resulting
target at horizon \(t+1\) is \(\rho_t\), and, after relabeling the unchanged
paths, its path-particle empirical measure is \(m(\xi_t)\).

We verify Assumption A1 of \citet{chanDelMoral18} for this augmented model.
Let \(0\leq a\leq b\leq t+1\), and let
...
&\leq
C\frac{t+1}{J}\rho_t\{B_1G'(\theta)\}                    \notag\\
&=
\frac{CB_1G'(\theta)(t+1)}{J}                               \notag\\
&\leq
\frac{CG'(\theta)n}{J},
...
Assumption~(A7) concerns the finite-$J$, $O(J)$ interacting genealogy.
Lemmas~\ref{lem:derivative-measure-forgetting}--
\ref{lem:discounted-dmop-particle-bias} derive deterministic forgetting,
full-genealogy weak bias, and discounted particle bias from the primitive
model conditions. The time-uniform results of
\citet{delMoral15derivative,tadic18} establish error and stability bounds for
their $O(J^2)$ backward derivative estimator; they do not imply (A7) for
DMOP. Assumption~(A7) is used only for the variance bound below.
