...
historical direct bound that remains useful at $\alpha=1$.
...
\end{align}
In addition, the historical direct argument gives
\begin{equation}
\E\left\|
\nabla_\theta\ell(\theta)-\nabla_\theta\hat\ell^\alpha(\theta)
\right\|_2^2
...
\right\}^2
\right].
\label{eq:dmop-alpha-historical-direct-mse}
\end{equation}
...
(\ref{eq:dmop-alpha-conservative-variance}), and Proposition~
\ref{prop:direct-dmop-alpha-mse} gives Equation~
(\ref{eq:dmop-alpha-historical-direct-mse}). Finally, Equation~
...
filter probabilities, so the differentiable particles together with their
parent indices form the bootstrap particle genealogy. Fix \(0\leq r<n\), put
...
\end{equation}
Its population counterpart is \(\rho_t(h_{k,i})\) in Equation~
(\ref{eq:canonical-extended-posterior-path-law}). Since a tangent is a
functional of the differentiable particle, the future normalizer below
depends on a path prefix only through its last particle state. Equation~
...
We verify Assumption A1 of \citet{chanDelMoral18} for this particle model.
Let \(0\leq a\leq b\leq t+1\), and let \(z\) and \(z'\) be two
path prefixes whose last particle states are \(x_a\) and \(x'_a\),
respectively. As noted above, the future normalizer depends on each prefix
only through that last particle state.
If \(a=b\), then
\begin{equation}
...
It remains to prove the sharper $\alpha$-dependent form. The time-uniform
first-order weak-bias expansion for a bootstrap particle filter under
strong mixing \citep[Section~7.4.3]{delMoral04} gives, for every bounded
real function $f$ of a current differentiable particle and every $t$,
\begin{align}
\left|
\E m(\xi_t)(f)-\rho_t(f)
\right|
...
\end{align}
The first inequality is the cited first-order telescoping expansion:
Assumption~{\refAssumpBoundedProcess} bounds the contribution of a
one-step sampling error inserted $l$ steps in the past by
$C\varrho^l\operatorname{osc}(f)/J$. The second inequality follows from
the explicit geometric sum
\begin{align}
\sum_{l=0}^{t}
\frac{C\varrho^l}{J}\operatorname{osc}(f)
&\leq
\frac{C\operatorname{osc}(f)}{J}
...
&\leq
\frac{C}{J}\operatorname{osc}(f),
\label{eq:uniform-current-particle-bias-geometric-sum}
\end{align}
where the last line enlarges $C$ by the fixed factor
$(1-\varrho)^{-1}$. This proves the uniform $O(J^{-1})$ weak-bias
estimate used here; no horizon-dependent path-space constant remains.

Equation~(\ref{eq:unrolled-filtered-weight}) and the bounded-mark
condition give
\begin{align}
\left\|
...
\end{align}
Apply Equation~(\ref{eq:uniform-current-particle-weak-bias}) to these two
weight-tangent coordinate functions in Equation~
(\ref{eq:conditional-score-weight-difference}). Equation~
(\ref{eq:weight-tangent-bounds-for-bias}), together with
...
\end{align}
Summing Equation~
(\ref{eq:uniform-conditional-score-particle-bias}) over
$n=1,\ldots,N$ gives
\begin{align}
&\left|
\E\partial_{\theta_i}\hat\ell^\alpha(\theta)
...
\end{align}
Squaring Equation~
(\ref{eq:uniform-full-score-particle-bias-coordinate}), summing over the
$p$ coordinates, and taking square roots proves Equation~
(\ref{eq:dmop-full-particle-bias-alpha}). Finally,
\begin{align}
\sum_{n=1}^{N}\sum_{r=0}^{n-1}\alpha^r
&\leq
N\sum_{r=0}^{N-1}\alpha^r                                  \notag\\
&=
N\frac{1-\alpha^N}{1-\alpha},
\label{eq:full-score-particle-bias-geometric-sum}
\end{align}
with the ratio interpreted as $N$ at $\alpha=1$. Thus Equation~
(\ref{eq:dmop-full-particle-bias-alpha}) also implies Equation~
(\ref{eq:dmop-full-particle-bias}) after the hidden constant is enlarged.
\end{proof}
...
\end{align}
Here, as elsewhere in the supplement, $\lesssim$ suppresses the
particle-system mixing-time factor. In particular,
\begin{equation}
\Var\left(\nabla_\theta\hat\ell^\alpha(\theta)\right)
\lesssim
\frac{NpG'(\theta)^2}{J}
...
\end{equation}
where the ratio is interpreted as $N$ at $\alpha=1$. The constant $C$
is independent of $N,J$, and $\alpha$, and
$\Var(Z)=\E\|Z-\E Z\|_2^2$.
\end{lem}

\begin{proof}[Proof.]
We first prove the historical-particle estimate that will be applied.
For $0\leq a<b\leq t$, the one-step normalized kernel in the
factorization used by \citet{chanDelMoral18} assigns a measurable set $A$
the probability
\begin{equation}
\frac{
\displaystyle\int_A
f_{X_{a+1}\mid X_a}(u\mid x_a;\theta)
Q_{a+1,b}(1)(u)\,du
}{
\displaystyle\int
f_{X_{a+1}\mid X_a}(u\mid x_a;\theta)
...
\label{eq:chan-normalized-one-step-kernel}
\end{equation}
The potential at time $a$ cancels between the numerator and denominator.
Assumption {\refAssumpBoundedProcess} gives, for every $x_a$ and $A$,
\begin{align}
&\frac{
\displaystyle\int_A
f_{X_{a+1}\mid X_a}(u\mid x_a;\theta)
...
&\quad\geq
\frac{
\underbar M\displaystyle\int_A Q_{a+1,b}(1)(u)\,du
}{
\bar M\displaystyle\int Q_{a+1,b}(1)(u)\,du
}                                                               \notag\\
&\quad=
\frac{\underbar M}{\bar M}
\frac{
\displaystyle\int_A Q_{a+1,b}(1)(u)\,du
}{
\displaystyle\int Q_{a+1,b}(1)(u)\,du
}.
\label{eq:chan-normalized-kernel-minorization}
\end{align}
The probability measure in the last line does not depend on $x_a$.
Consequently, the Dobrushin coefficient of each kernel in
Equation~(\ref{eq:chan-normalized-one-step-kernel}) is at most
\begin{equation}
1-\frac{\underbar M}{\bar M}.
\label{eq:chan-normalized-kernel-contraction}
\end{equation}
Writing $\eta_{a,b}^{x_a}$ for the corresponding conditional terminal
law, as in \citet{chanDelMoral18}, composition of the $b-a$ kernels gives
\begin{equation}
\sup_{x_a,x'_a}
\left\|
\eta_{a,b}^{x_a}-\eta_{a,b}^{x'_a}
\right\|_{\rm TV}
\leq
2\left(1-\frac{\underbar M}{\bar M}\right)^{b-a}.
\label{eq:chan-conditional-terminal-contraction}
\end{equation}
The appended identity mutation at horizon $t+1$ can remove at most one
contraction factor.  Hence the summability condition in Assumption~(A2)
of \citet{chanDelMoral18} follows from the explicit calculation
\begin{align}
&\sum_{d=0}^{\infty}
2\left(1-\frac{\underbar M}{\bar M}\right)^{\max\{d-1,0\}}
                                                               \notag\\
&\quad=
4+
2\sum_{d=2}^{\infty}
\left(1-\frac{\underbar M}{\bar M}\right)^{d-1}              \notag\\
&\quad=
4+
2\frac{1-\underbar M/\bar M}{\underbar M/\bar M}            \notag\\
&\quad=
2+2\frac{\bar M}{\underbar M}
<\infty.
\label{eq:chan-mixing-sum}
\end{align}

It remains to verify the other hypotheses of that result. Let $z$ and
$z'$ be two particle-path prefixes through time $a$, with last particle
states $x_a$ and $x'_a$. Future potentials and transitions depend on these
prefixes only through their last particle states, so, for
$a\leq b\leq t+1$,
\begin{align}
Q_{a,b}(1)(z)
&=Q_{a,b}(1)(x_a),
...
Q_{a,b}(1)(z')
&=Q_{a,b}(1)(x'_a).
\label{eq:historical-extended-normalizer-identity}
\end{align}
Equations~(\ref{eq:chan-zero-step-normalizer}),
(\ref{eq:chan-normalizer-ratio}), and
(\ref{eq:chan-terminal-normalizer-ratio}) therefore imply
\begin{equation}
Q_{a,b}(1)(z)
\leq
\max\left\{1,
...
\right\}
Q_{a,b}(1)(z').
\label{eq:historical-uniform-normalizer-ratio}
\end{equation}
Assumption {\refAssumpBoundedMeasurement} gives, at every selection time,
\begin{equation}
0<\underbar G\leq G_a\leq\bar G<\infty.
\label{eq:historical-uniform-potential-bounds}
\end{equation}
Thus Equations~(\ref{eq:chan-conditional-terminal-contraction})--
(\ref{eq:historical-uniform-potential-bounds}) verify Assumptions~(A1)--
(A2) of \citet{chanDelMoral18} for the particle model. For
$q\in\{1,2\}$ and every $0\leq F\leq1$ on $q$ particle paths, positivity
and Equation~(\ref{eq:historical-extended-normalizer-identity}) give
\begin{align}
0
&\leq
Q_{a,b}^{\otimes q}(F)(z_1,\ldots,z_q)                       \notag\\
&\leq
Q_{a,b}^{\otimes q}(1)(z_1,\ldots,z_q)                       \notag\\
&=
\prod_{l=1}^{q}Q_{a,b}(1)(x_a^l).
\label{eq:marked-historical-termwise-bound}
\end{align}
The coalescence operators in Appendix~C of \citet{chanDelMoral18} are
Markov kernels, and hence are positive and send $1$ to $1$.  Equation~
(\ref{eq:marked-historical-termwise-bound}) therefore remains valid after
every coalescence pattern, term by term.  Together with Equation~
(\ref{eq:historical-uniform-normalizer-ratio}), this shows that every bound
in their Lemma~C.1 and Proposition~C.2 is unchanged when the bounded
tangent functional is evaluated on the particle paths.

Apply the coalescent proof of Theorem~3.5 of \citet{chanDelMoral18} to
this marked historical model at the appended horizon $t+1$.  Replacing
their particle count by $J$ and enlarging one constant $C\geq2$ so that it
applies to $q=1$ and $q=2$, for
$0\leq g\leq1$ and $J>C(t+1)$ the theorem gives
\begin{align}
\left(1-C\frac{t+1}{J}\right)\rho_t(g)
&\leq
\E g(\xi_t^1)
\leq
\left(1+C\frac{t+1}{J}\right)\rho_t(g),
\label{eq:historical-one-lineage-two-sided}
\\
\left(1-C\frac{t+1}{J}\right)\rho_t(g)^2
&\leq
\E\{g(\xi_t^1)g(\xi_t^2)\}                                  \notag\\
&\leq
\rho_t(g)^2+C\frac{t+1}{J}\rho_t(g^2),
\label{eq:historical-two-lineage-two-sided}
\end{align}
where the terminal labels $1$ and $2$ are distinct.  Since
$0\leq\rho_t(g),\rho_t(g^2)\leq1$, the first line gives
\begin{equation}
...
\end{equation}
and subtracting $\rho_t(g)^2$ in the second line gives
\begin{align}
-C\frac{t+1}{J}\rho_t(g)^2
&\leq
\E\{g(\xi_t^1)g(\xi_t^2)\}-\rho_t(g)^2                     \notag\\
&\leq
C\frac{t+1}{J}\rho_t(g^2),                                  \notag\\
\left|
\E\{g(\xi_t^1)g(\xi_t^2)\}-\rho_t(g)^2
...
Let $f$ be a bounded real path functional.  If $\|f\|_\infty=0$, the
claim is immediate.  Otherwise set
\begin{equation}
...
f(\xi_t^j)-\rho_t(f)
=
2\|f\|_\infty\{g(\xi_t^j)-\rho_t(g)\}.
\label{eq:historical-centering-rescaling}
\end{equation}
Exchangeability and Equations~(\ref{eq:historical-one-lineage-bound})--
(\ref{eq:historical-two-lineage-bound}) therefore give
\begin{align}
&\left|\E\left[
\{f(\xi_t^1)-\rho_t(f)\}
\{f(\xi_t^2)-\rho_t(f)\}
...
+\rho_t(g)\left|\E g(\xi_t^1)-\rho_t(g)\right|
+\rho_t(g)\left|\E g(\xi_t^2)-\rho_t(g)\right|
\right]                                                       \notag\\
&\quad\leq
...
For $J>C(t+1)$, exchangeability of the $J$ terminal paths gives
\begin{align}
&\E\left|m(\xi_t)(f)-\rho_t(f)\right|^2                     \notag\\
&\quad=
\frac1{J^2}\sum_{j=1}^{J}
\E\left|f(\xi_t^j)-\rho_t(f)\right|^2                       \notag\\
&\qquad+
\frac1{J^2}\sum_{\substack{1\leq j,l\leq J\\j\ne l}}
\E\left[
\{f(\xi_t^j)-\rho_t(f)\}
\{f(\xi_t^l)-\rho_t(f)\}
\right]                                                       \notag\\
...
&\quad\leq
C\frac{t+1}{J}\|f\|_\infty^2.
\label{eq:historical-particle-second-moment-large-j}
\end{align}
For $J\leq C(t+1)$,
\begin{align}
\E\left|m(\xi_t)(f)-\rho_t(f)\right|^2
...
4\|f\|_\infty^2                                            \notag\\
&\leq
4C\frac{t+1}{J}\|f\|_\infty^2.
\label{eq:historical-particle-second-moment-small-j}
\end{align}
After enlarging $C$, the two cases prove, for every $J\geq1$,
\begin{equation}
\E\left|m(\xi_t)(f)-\rho_t(f)\right|^2
\leq
C\frac{t+1}{J}\|f\|_\infty^2.
...
particle state.  The time-uniform $L^4$ particle bound in Lemma~2 of
\citet{karjalainen23}, applied to these two functions, gives
\begin{align}
&\left\|
\frac1J\sum_{j=1}^{J}
\left.\partial_{\theta_i}\log w_{n,j}^{F,\theta}
...
differentiable particle clouds at times $n-1$ and $n$. Theorem~3 of
...
&\leq
\sum_{l=1}^{N-1}
q^{\left\lfloor(l-1)/\{c\log J\}\right\rfloor}             \notag\\
&\leq
\min\left\{
N-1,
...
Equation~(\ref{eq:refined-dmop-alpha-variance}). Combining the same
deterministic bound with Equation~
...
\begin{remark}[DMOP-$0$ and the fixed-lag variance calculation]
The earlier DMOP-$0$ and DMOP-$(\alpha,k)$ calculations can be retained,
but the covariance premise must be stated explicitly.  For
$0\leq k\leq N-1$, define the fixed-lag score by truncating the existing
lag decomposition:
\begin{equation}
\partial_{\theta_i}\hat s_k^\alpha(\theta)
=
\sum_{n=1}^{N}
\sum_{r=0}^{\min\{k,n-1\}}
\alpha^r\Delta_{n,r,i}^J.
\label{eq:fixed-lag-score-existing-coefficients}
\end{equation}
Thus $\hat s_0^\alpha=\hat\ell^0$, independently of $\alpha$, and
$\hat s_{N-1}^\alpha=\hat\ell^\alpha$.

We first record the original DMOP-$0$ argument in its intended
$\lesssim$ convention.  If the strong-mixing estimate is understood to
apply to the differentiated particle state, consisting of the physical
particle together with its simulator tangent, Davydov's inequality and the
centered fourth-moment particle bound give
\begin{equation}
\left|
\Cov\left(\Delta_{n,0,i}^J,\Delta_{m,0,i}^J\right)
\right|
\leq
\frac{CG'(\theta)^2}{J}
(1-\epsilon)^{\frac12\left\lfloor
|n-m|/\{c\log J\}\right\rfloor}.
\label{eq:dmop-zero-original-covariance}
\end{equation}
Therefore
\begin{align}
\Var\left(\partial_{\theta_i}\hat\ell^0(\theta)\right)
&=
\sum_{n,m=1}^{N}
\Cov\left(\Delta_{n,0,i}^J,\Delta_{m,0,i}^J\right)            \notag\\
&\leq
\frac{CNG'(\theta)^2}{J}
\{1+\min(N,\log J)\},
\label{eq:dmop-zero-variance-with-mixing-time}
\end{align}
after summing the blocked geometric series.  Thus, with the manuscript's
convention that $\lesssim$ suppresses this particle-system mixing-time
factor,
\begin{equation}
\Var\left(\nabla_\theta\hat\ell^0(\theta)\right)
\lesssim
\frac{NpG'(\theta)^2}{J}.
\label{eq:dmop-zero-variance-restored}
\end{equation}

For the fixed-lag and full-$\alpha$ extensions, first suppose that there
are $C<\infty$ and $\kappa\in(0,1)$, independent
of $N,J,n,m,r$, such that, for $0\leq r\leq k$ and $r<n,m\leq N$,
\begin{equation}
\left|
\Cov\left(\Delta_{n,r,i}^J,\Delta_{m,r,i}^J\right)
\right|
\leq
\frac{CG'(\theta)^2}{J}\kappa^{|n-m|}.
\label{eq:fixed-lag-same-lag-covariance}
\end{equation}
For each fixed $r$, summing the covariance matrix entries gives
\begin{align}
&\Var\left(
\sum_{n=r+1}^{N}\Delta_{n,r,i}^J
\right)                                                       \notag\\
&\quad=
\sum_{n,m=r+1}^{N}
\Cov\left(\Delta_{n,r,i}^J,\Delta_{m,r,i}^J\right)           \notag\\
&\quad\leq
\frac{CG'(\theta)^2}{J}
\sum_{n,m=r+1}^{N}\kappa^{|n-m|}                             \notag\\
&\quad\leq
\frac{CG'(\theta)^2}{J}(N-r)
\left(1+2\sum_{l=1}^{\infty}\kappa^l\right)                 \notag\\
&\quad=
\frac{CG'(\theta)^2}{J}(N-r)
\frac{1+\kappa}{1-\kappa}.
\label{eq:fixed-lag-one-block-variance}
\end{align}
Centering Equation~(\ref{eq:fixed-lag-score-existing-coefficients}), using
Minkowski's inequality in $L^2$, and then applying Equation~
(\ref{eq:fixed-lag-one-block-variance}) yield
\begin{align}
&\Var\left(
\partial_{\theta_i}\hat s_k^\alpha(\theta)
\right)^{1/2}                                                \notag\\
...
&\quad\leq
\frac{CG'(\theta)}{\sqrt J}
\sum_{r=0}^{k}\alpha^r\sqrt{N-r}                            \notag\\
&\quad\leq
C G'(\theta)\sqrt{\frac{N}{J}}
\sum_{r=0}^{k}\alpha^r.
\label{eq:fixed-lag-minkowski-variance}
\end{align}
Squaring Equation~(\ref{eq:fixed-lag-minkowski-variance}) and summing over
the $p$ coordinates give the finite, endpoint-continuous bound
\begin{align}
\Var\left(\nabla_\theta\hat s_k^\alpha(\theta)\right)
&\leq
\frac{CNpG'(\theta)^2}{J}
\left(\sum_{r=0}^{k}\alpha^r\right)^2                       \notag\\
&=
\frac{CNpG'(\theta)^2}{J}
\begin{cases}
\left\{(1-\alpha^{k+1})/(1-\alpha)\right\}^2,
&0\leq\alpha<1,\\
(k+1)^2,&\alpha=1.
\end{cases}
\label{eq:fixed-lag-variance-same-lag-control}
\end{align}
In particular, $k=0$ gives the non-polylogarithmic form of the earlier
DMOP-$0$ conclusion whenever the uniform covariance bound in Equation~
(\ref{eq:fixed-lag-same-lag-covariance}) is available:
\begin{equation}
\Var\left(\nabla_\theta\hat\ell^0(\theta)\right)
\leq
\frac{CNpG'(\theta)^2}{J}.
\label{eq:dmop-zero-variance-from-covariance}
\end{equation}
For $k=N-1$, Equation~
(\ref{eq:fixed-lag-variance-same-lag-control}) gives
\begin{equation}
\Var\left(\nabla_\theta\hat\ell^\alpha(\theta)\right)
\leq
\frac{CNpG'(\theta)^2}{J}
\left(\frac{1-\alpha^N}{1-\alpha}\right)^2,
\label{eq:dmop-alpha-variance-same-lag-control}
\end{equation}
where the ratio is interpreted as $N$ at $\alpha=1$.  Thus separate
fixed-lag covariance control gives the requested $N/J$ factor multiplied
by a finite function of $\alpha$, but for fixed $\alpha<1$ that function
has order $(1-\alpha)^{-2}$.

The sharper power $(1-\alpha)^{-1}$ requires covariance control jointly
across the observation time and the injection time.  Under Equation~
(\ref{eq:needed-two-lineage-covariance}) below, the two-dimensional
convolution calculation in Equation~
(\ref{eq:two-lineage-geometric-convolution}) instead gives
\begin{equation}
\Var\left(\nabla_\theta\hat\ell^\alpha(\theta)\right)
\leq
\frac{CpG'(\theta)^2}{J}
\sum_{r=0}^{N-1}(N-r)\alpha^{2r}.
\label{eq:dmop-alpha-variance-joint-lag-control}
\end{equation}
For $0\leq\alpha<1$, the finite sum is
\begin{align}
\sum_{r=0}^{N-1}(N-r)\alpha^{2r}
&=
\frac{N-(N+1)\alpha^2+\alpha^{2N+2}}
{(1-\alpha^2)^2}                                             \notag\\
&\leq
N\min\left\{N,\frac1{1-\alpha^2}\right\}.
\label{eq:finite-alpha-variance-sum}
\end{align}
Its continuous value at $\alpha=1$ is $N(N+1)/2$.  Hence Equation~
(\ref{eq:dmop-alpha-variance-joint-lag-control}) recovers $O(N/J)$ at
$\alpha=0$, gives $O\{N/[J(1-\alpha)]\}$ for fixed $\alpha<1$, and gives
$O(N^2/J)$ at $\alpha=1$.

The simulator tangent is a function of the differentiated particle state,
so Equations~(\ref{eq:dmop-zero-original-covariance})--
(\ref{eq:dmop-zero-variance-restored}) are exactly the original argument
when the quoted strong-mixing estimate is understood for that augmented
state.  The bounded-density assumptions directly establish the quoted
mixing theorem for the physical cloud; applying it to the augmented
state--tangent cloud is the remaining scope condition, because the tangent
is not generally recoverable from the physical locations alone.  The
$O(\log J)$ factor is not a separate objection under the manuscript's
$\lesssim$ convention.  Truncating the discounted weights does not itself
truncate the tangent, so the uniform fixed-lag and joint-lag covariance
bounds are recorded separately.  Equations~
(\ref{eq:fixed-lag-variance-same-lag-control}) and
(\ref{eq:dmop-alpha-variance-joint-lag-control}) therefore restore the
valid variance implications of the earlier argument, while Lemma~
\ref{lem:dmop-alpha-variance} remains the unconditional result under the
present assumptions.
\end{remark}
...
\begin{prop}[Direct MSE companion bound]
\label{prop:direct-dmop-alpha-mse}
...
\right\}^2
\right],
\label{eq:direct-dmop-alpha-mse-sharp}
\end{align}
...
\right\}^2
\right].
\label{eq:direct-dmop-alpha-mse-simplified}
\end{align}
In particular, the variance contribution is
$N/[J(1-\alpha)^2]$ for fixed $\alpha<1$.
\end{prop}

\begin{proof}[Proof.]
We first prove the sharper direct bound. Minkowski's inequality in $L^2$
gives
\begin{align}
&\left\{
\E\left\|
...
\end{align}
The three terms on the right of Equation~
(\ref{eq:direct-sharp-minkowski-three-terms}) are bounded by Lemma~
\ref{lem:dmop-alpha-variance}, Lemma~
\ref{lem:dmop-full-particle-bias}, and Lemma~
\ref{lem:dmop-population-discount-bias}, respectively. Hence
\begin{align}
&\left\{
\E\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
...
{(1-\varrho)(1-\alpha\varrho)}
\right].
\label{eq:direct-sharp-l2-three-bounds}
\end{align}
For nonnegative $a,b,c$,
$(a+b+c)^2\leq3(a^2+b^2+c^2)$. Squaring Equation~
(\ref{eq:direct-sharp-l2-three-bounds}) and applying this inequality
proves Equation~(\ref{eq:direct-dmop-alpha-mse-sharp}).

Applying the geometric-sum bounds in Equation~
(\ref{eq:refined-mse-geometric-simplifications}) proves Equation~
(\ref{eq:direct-dmop-alpha-mse-simplified}).
\end{proof}

\begin{remark}[Coarse historical direct cap]
For comparison, the historical occupation-measure argument gives a
weaker endpoint-safe cap. Summing
Equation~(\ref{eq:historical-coordinate-mse}) over the $p$ coordinates
gives
\begin{equation}
\E\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
...
\leq
\frac{CpG'(\theta)^2N^3}{J}.
\label{eq:direct-particle-l2-n-three}
\end{equation}
There is also a deterministic bound.  Equation~
(\ref{eq:full-score-deterministic-bound}) gives, for every coordinate $i$,
\begin{equation}
\left|\partial_{\theta_i}\hat\ell^\alpha(\theta)\right|
\leq NG'(\theta).
\label{eq:direct-particle-score-deterministic}
\end{equation}
The identical calculation applied to Equation~
(\ref{eq:population-full-score-summation-by-parts}), with $m(\xi_t)$
replaced by $\rho_t$, gives
\begin{equation}
\left|\partial_{\theta_i}\ell^\alpha(\theta)\right|
\leq NG'(\theta).
\label{eq:direct-population-score-deterministic}
\end{equation}
The scalar triangle inequality, Equations~
(\ref{eq:direct-particle-score-deterministic})--
(\ref{eq:direct-population-score-deterministic}), and then summation over
coordinates give
\begin{align}
&\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
...
&\quad\leq
4pN^2G'(\theta)^2.
\label{eq:direct-particle-l2-deterministic}
\end{align}
Combining Equations~(\ref{eq:direct-particle-l2-n-three}) and
(\ref{eq:direct-particle-l2-deterministic}), and taking square roots,
yields
\begin{equation}
\left\{
\E\left\|
...
\leq
C\sqrt p\,G'(\theta)
\min\left\{N,\frac{N^{3/2}}{\sqrt J}\right\}.
\label{eq:direct-particle-l2-minimum}
\end{equation}

We next add the deterministic population discount error.  The triangle
...
\frac{\varrho(1-\alpha)}
{(1-\varrho)(1-\alpha\varrho)}.
\label{eq:direct-full-population-discount-error}
\end{align}
Minkowski's inequality in $L^2$, followed by Equations~
(\ref{eq:direct-particle-l2-minimum}) and
(\ref{eq:direct-full-population-discount-error}), now gives
\begin{align}
&\left\{
\E\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
...
{(1-\varrho)(1-\alpha\varrho)}
\right].
\label{eq:direct-total-l2-before-square}
\end{align}
Squaring Equation~(\ref{eq:direct-total-l2-before-square}) and using
$(a+b)^2\leq2a^2+2b^2$ proves
\begin{align}
&\E\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
-\nabla_\theta\ell(\theta)
...
\right\}^2
\right].
\label{eq:direct-dmop-alpha-mse-before-cap}
\end{align}
Finally, Equation~(\ref{eq:direct-particle-score-deterministic}) at the
given $\alpha$ and
Equation~(\ref{eq:direct-population-score-deterministic}) at $\alpha=1$
give, exactly as in Equation~
(\ref{eq:direct-particle-l2-deterministic}),
\begin{equation}
\E\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
-\nabla_\theta\ell(\theta)
...
\leq
4pN^2G'(\theta)^2.
\label{eq:direct-total-deterministic-cap}
\end{equation}
Combining Equations~(\ref{eq:direct-dmop-alpha-mse-before-cap}) and
(\ref{eq:direct-total-deterministic-cap}), and enlarging $C$, proves
\begin{equation}
\E\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
-\nabla_\theta\ell(\theta)
...
\right\}^2
\right].
\label{eq:direct-dmop-alpha-mse}
\end{equation}
This is not the rate asserted by Proposition~
\ref{prop:direct-dmop-alpha-mse}; it is retained only as a coarse
historical comparison, particularly at the singular endpoint
$\alpha=1$.
\end{remark}

\begin{remark}[Relation to the direct historical bound of Del Moral and
Doucet]
The proof of Proposition~\ref{prop:direct-dmop-alpha-mse} follows the
direct-$L^2$ strategy of \citet[Lemma~4]{delMoral03}, but uses the sharper
two-lineage historical estimate already proved in Lemma~
\ref{lem:dmop-alpha-variance}.  Applied literally to a norm-one bounded
path functional, the generic inequality of \citet{delMoral03} has a sum of
$N$ uniformly bounded future-normalizer ratios and therefore gives RMSE
$O(N/\sqrt J)$.  The unnormalized score has norm
$O\{NG'(\theta)\}$, so homogeneity would give
\begin{equation}
\operatorname{RMSE}=O\left\{\frac{N^2G'(\theta)}{\sqrt J}\right\},
\qquad
\operatorname{MSE}=O\left\{\frac{N^4G'(\theta)^2}{J}\right\}.
\label{eq:del-moral-doucet-direct-generic-rate}
\end{equation}
This is the generic historical rate recalled by \citet{poyiadjis11}.
The time-homogeneous specialization in Theorem~4 of \citet{delMoral03}
uses their additional condition~(23), which is not one of the present
assumptions.  By contrast, Equations~
(\ref{eq:historical-one-lineage-bound})--
(\ref{eq:historical-particle-second-moment}) give RMSE
$O(N^{3/2}/\sqrt J)$ for the present bounded score functional and hence
the sharper $O(N^3/J)$ direct MSE in Proposition~
\ref{prop:direct-dmop-alpha-mse}. The current-particle argument in the
first part of that proposition is sharper for $\alpha<1$ and agrees with
the $N^3/J$ historical order at $\alpha=1$. Improving its
$(1-\alpha)^{-2}$ factor to $(1-\alpha)^{-1}$ requires the signed
two-time covariance control described next; it is not a separate
tangent-stability question.
\end{remark}

\begin{remark}[Why the endpoint historical bound is cubic]
The factor $N^3/J$ is the $\alpha=1$ endpoint, not the variance order for
every $\alpha$. The
...
functional with norm $O(NG'(\theta))$. Homogeneity therefore gives
\begin{equation}
\frac{N}{J}\{NG'(\theta)\}^2
=
\frac{N^3G'(\theta)^2}{J}.
...
For the differentiable particle, the tangent and score accumulator are
measurable particle functionals. The centered particle $L^4$ bound,
Davydov's inequality, and particle-system mixing therefore give the
stronger finite-$\alpha$ result already proved in Equation~
(\ref{eq:conditional-score-variance-with-mixing-time}):
\begin{align}
\Var\left(\nabla_\theta\hat\ell^\alpha(\theta)\right)
&\lesssim
\frac{pG'(\theta)^2}{J}
...
\end{align}
At $\alpha=0$, the sum on the right is $N$. For fixed $\alpha<1$, it is
at most $N/(1-\alpha)^2$. At $\alpha=1$, it is exactly
\begin{equation}
\sum_{n=1}^{N}n^2
=
\frac{N(N+1)(2N+1)}6,
...
\end{equation}
which recovers the cubic endpoint. The particle-system mixing-time factor
is the factor intentionally suppressed by $\lesssim$.
