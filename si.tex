...
The truncation proof above treats two cancelling pieces separately. We now
analyze the conditional-score bias directly. Write
$\nabla_\theta\ell_n^\alpha(\theta)$ for the infinite-particle limit of the
$n$th DMOP-$\alpha$ conditional-score estimate. This is the only additional
quantity required below; in particular, we do not introduce a separate
lag-indexed score process.

\begin{lem}[Uniform particle bias of a DMOP conditional score]
\label{lem:uniform-dmop-particle-bias}
Suppose Assumptions~{\refAssumpBoundedProcess},
{\refAssumpBoundedMeasurement}, and
{\refAssumpTangentStability} hold and multinomial resampling is used. Then
there is a constant $C<\infty$, independent of $n$, $J$, and
...
\begin{proof}
At $\theta=\phi$, the particles follow the ordinary bootstrap filter. At
step $m$, for any bounded measurable $h$, its local empirical sampling
error is
\begin{equation}
V_m^J(h)
:=
\eta_m^J(h)-\Phi_m(\eta_{m-1}^J)(h).
\label{eq:local-empirical-error}
\end{equation}
Conditional on the particle system through time $m-1$, multinomial
resampling and propagation make the $J$ particles conditionally
independent. Consequently,
\begin{equation}
\E\left[V_m^J(h)\mid\mathcal F_{m-1}^J\right]=0,
\qquad
\E\left[|V_m^J(h)|^2\mid\mathcal F_{m-1}^J\right]
\leq\frac{\operatorname{osc}(h)^2}{4J}.
\label{eq:local-empirical-moments}
\end{equation}
The initial empirical measure satisfies the same two bounds without
conditioning.

Telescope the differentiated recursion by replacing, one time at a time,
the empirical update $\eta_m^J$ by its deterministic update
$\Phi_m(\eta_{m-1}^J)$. Apply the second-order perturbation expansion in
Assumption~{\refAssumpTangentStability} to each replacement. The resulting
first-order term is linear in $V_m^J$ and has conditional expectation zero
by Equation~(\ref{eq:local-empirical-moments}). The norm of the expected
second-order remainder generated at time $m$ and observed in conditional
score $n$ is bounded by
\begin{equation}
\frac{C\sqrt p\,G'(\theta)}{J}\rho^{n-m}.
\label{eq:local-second-order-remainder}
\end{equation}
This is the standard weak-error mechanism for normalized Feynman--Kac
particle measures: the $J^{-1/2}$ first-order errors are centered, so bias
starts at second order. The only additional point here is that the
differentiated flow is used; precisely this flow and its second-order
remainder are covered by Assumption~{\refAssumpTangentStability}. The
semigroup estimates used by \citet{delMoral15derivative} and
\citet{tadic18} establish the analogous uniform bounds for the stable
filter derivative.

Summing Equation~(\ref{eq:local-second-order-remainder}) over the possible
insertion times gives
\begin{equation}
\left\|
\E\nabla_\theta\hat\ell_n^\alpha(\theta)
-\nabla_\theta\ell_n^\alpha(\theta)
...
\sum_{m=0}^{n}\rho^{n-m}
\leq
\frac{C\sqrt p\,G'(\theta)}{J(1-\rho)}.
\end{equation}
Absorbing $(1-\rho)^{-1}$ into $C$ proves the claim. Uniformity in
$\alpha$ follows because the inherited tangent is multiplied by
$\alpha^r\leq1$ and the constants in
Assumption~{\refAssumpTangentStability} are uniform on $[0,1]$.
\end{proof}

Two aspects of this proof are important. First, it is a weak-bias result;
it does not imply a time-uniform variance for the genealogical estimator.
Second, the $O(J^2)$ backward recursion studied by
\citet{delMoral15derivative,tadic18} is not being identified with DMOP.
Their stability estimates motivate Assumption~{\refAssumpTangentStability},
while Equations~(\ref{eq:local-empirical-error})--
(\ref{eq:local-second-order-remainder}) apply the weak-error expansion to
the differentiated DMOP recursion itself.
...
+N^2
\left\{
\frac{\rho(1-\alpha)}{(1-\rho)(1-\alpha\rho)}
+\frac{1}{J}
\right\}^2
\right].
...
\begin{proof}
The same local expansion as in the preceding lemma gives the centered
fluctuation bound. A sampling error introduced at one time has conditional
second moment $O(pG'(\theta)^2/J)$. Its total influence on all subsequent
discounted conditional scores is at most
\begin{equation}
\sum_{r=0}^{\infty}\alpha^r=\frac{1}{1-\alpha}.
\end{equation}
The first-order local errors are martingale differences, so their
cross-products vanish before the influences are summed. Adding their
squared influences over the $N$ insertion times gives
\begin{equation}
\E\left\|
\nabla_\theta\hat\ell^\alpha(\theta)
-\E\nabla_\theta\hat\ell^\alpha(\theta)
...
\lesssim
\frac{NpG'(\theta)^2}{J(1-\alpha)^2}.
\label{eq:dmop-centered-score-bound}
\end{equation}
The centered second-order remainders are smaller by another factor
$J^{-1}$ and are absorbed in this bound. This proves
Equation~(\ref{eq:refined-dmop-alpha-variance}).

It remains to bound the systematic bias. Unrolling the differentiated MOP
recursion shows that a score contribution inherited over $r$ steps has
coefficient one for DMOP-$1$ and $\alpha^r$ for DMOP-$\alpha$.
Assumption~{\refAssumpTangentStability} bounds the effect of that
contribution on the current conditional score by
$C\sqrt p\,G'(\theta)\rho^r$. Therefore the per-time population
discounting bias is at most
\begin{align}
C\sqrt p\,G'(\theta)
\sum_{r=1}^{\infty}(1-\alpha^r)\rho^r
&=
C\sqrt p\,G'(\theta)
\left\{
\frac{\rho}{1-\rho}-\frac{\alpha\rho}{1-\alpha\rho}
\right\}                                                       \notag\\
&=
C\sqrt p\,G'(\theta)
\frac{\rho(1-\alpha)}{(1-\rho)(1-\alpha\rho)}.
...
\end{align}
Lemma~\ref{lem:uniform-dmop-particle-bias} adds at most
$C\sqrt p\,G'(\theta)/J$ of finite-particle bias per conditional score.
Summing these two biases over the $N$ observation times and squaring gives
the second term in Equation~(\ref{eq:refined-dmop-alpha-mse}). Combining
it with Equation~(\ref{eq:dmop-centered-score-bound}) proves the result.
\end{proof}

For fixed $\rho<1$ and $\alpha$ near one, the leading terms in
Equation~(\ref{eq:refined-dmop-alpha-mse}) are
\begin{equation}
\frac{N}{J(1-\alpha)^2}
+N^2(1-\alpha)^2
+\frac{N^2}{J^2}.
...
\end{equation}
Balancing the first two gives
\begin{equation}
1-\alpha\asymp(NJ)^{-1/4}
...
\lesssim
pG'(\theta)^2
\left(\frac{N^{3/2}}{\sqrt J}+\frac{N^2}{J^2}\right).
\label{eq:dmop-interior-mse}
\end{equation}

\begin{cor}[A regime with a strict interior advantage]
\label{cor:dmop-interior-advantage}
In addition to the conditions of
Theorem~\ref{thm:refined-dmop-alpha-mse}, suppose the nondegeneracy
condition of \citet{poyiadjis11} gives
$\operatorname{MSE}_1\gtrsim N^2/J$ uniformly in the joint limit, and
suppose the omitted historical score for DMOP-$0$ does not cancel, so that
$\operatorname{MSE}_0\gtrsim N^2$. If $1\ll J\ll N$ and
$1-\alpha\asymp(NJ)^{-1/4}$, then
\begin{equation}
...
\begin{proof}
Relative to the DMOP-$1$ MSE lower bound, the two terms in
Equation~(\ref{eq:dmop-interior-mse}) have ratios $\sqrt{J/N}$ and
$1/J$, respectively. Relative to the DMOP-$0$ MSE lower bound, their
ratios are $1/\sqrt{NJ}$ and $1/J^2$. All four ratios vanish under
$1\ll J\ll N$.
\end{proof}

Finally, the normalization issue does not by itself create a quartic
rate. A current-state result stated for $\operatorname{osc}(h)\leq1$
scales by homogeneity to RMSE $O(N/\sqrt J)$, and hence MSE
$O(N^2/J)$, for an $O(N)$ functional. The generic $O(N^4/J)$ upper
bound arises only when a path-space expansion contributes another factor
$N$ in RMSE. By contrast, \citet{poyiadjis11} prove a quadratic
\emph{lower} bound on the path estimator's asymptotic variance under a
nondegeneracy condition; they do not prove that the quartic generic upper
bound is a sharp variance law.
