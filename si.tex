...
comparison. A differentiable particle includes its location and tangent,
...
\end{equation}
The uniformity in $t$ can be seen directly in the cited first-order
expansion. Assumption~{\refAssumpBoundedProcess} bounds the contribution
of a one-step sampling error inserted $l$ steps in the past by
$C\varrho^l\operatorname{osc}(f)/J$. Hence
\begin{align}
\sum_{l=0}^{t}
...
$z'$ be two seed--state--tangent path prefixes through time $a$, with last
physical states $x_a$ and $x'_a$.  Future potentials and physical-state
transitions depend on these prefixes only through their last physical
states, so, for $a\leq b\leq t+1$,
\begin{align}
Q_{a,b}(1)(z)
&=Q_{a,b}(1)(x_a),
&
...
(A2) of \citet{chanDelMoral18} for the physical model.  For
$q\in\{1,2\}$ and every $0\leq F\leq1$ on $q$ extended paths, positivity
...
in their Lemma~C.1 and Proposition~C.2 is unchanged by the seed and tangent
marks; no total-variation contraction of those marks is invoked.
...
\end{align}
Thus Equation~(\ref{eq:direct-dmop-alpha-mse-sharp}) gives the sharper
$N/J$-times-an-$\alpha$-factor argument, while Equation~
(\ref{eq:direct-dmop-alpha-mse}) retains the historical direct cap near
$\alpha=1$.
\end{prop}
...
For completeness, we next prove the historical direct cap. Summing
...
Equation~(\ref{eq:direct-dmop-alpha-mse}).
\end{proof}
...
\ref{prop:direct-dmop-alpha-mse}.  The direct proof removes the redundant
$N^4/J^2$ term, but it does not remove the remaining historical factor
$N$: that improvement still requires signed cross-time covariance control
or stability of the inherited tangent.
\end{remark}
