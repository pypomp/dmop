...
Write $f=-\ell$.  The analysis is local to a convex region on which
$f$ is $\gamma$-strongly convex and $\Gamma$-smooth and which contains all
second-stage iterates.  The first lemma supplies a pointwise particle bound.
The theorem applies it conditionally at the adaptively chosen iterates and uses
a union bound over a prespecified finite iteration budget.

At $\theta=\phi$, let $Z_{N,j}$ denote the complete augmented ancestral state
of the $j$th after-resampling particle, including the simulator tangents carried
by automatic differentiation, and define the total path-score functional
\begin{equation}
\psi_\theta(Z_N)
 =\nabla_\theta\log f_{Y_{1:N}\mid X_{1:N}}
   (y_{1:N}^*\mid X_{1:N}^{\theta};\theta).
\label{eq:path-score-functional}
\end{equation}
The derivative in Equation~\eqref{eq:path-score-functional} is the total
fixed-random-number derivative through the simulated path.  To identify its
population target, write $U$ for the parameter-independent base simulator
variables and
\begin{equation}
W_\theta(U)=\prod_{n=1}^N
f_{Y_n\mid X_n}(y_n^*\mid X_n^\theta(U);\theta).
\end{equation}
Then $\lik(\theta)=\E\{W_\theta(U)\}$ and
$\nabla_\theta W_\theta=W_\theta\psi_\theta$.  Assumptions~
{\refAssumpBoundedMeasurement} and {\refAssumpDifferentiableSimulator}
provide a finite-horizon dominating function: coordinatewise,
$|\partial_{\theta_i}W_\theta(U)|\leq
N\bar G^N G'(\theta)$.  Therefore
\begin{equation}
\nabla_\theta\ell(\theta)
=\frac{\E\{W_\theta(U)\psi_\theta(U)\}}
       {\E\{W_\theta(U)\}}.
\label{eq:path-score-target}
\end{equation}
If $\rho_N^J$ is the empirical measure of the augmented genealogical particles
and $\rho_N$ is its Feynman--Kac limit, the right side of
Equation~\eqref{eq:path-score-target} is $\rho_N(\psi_\theta)$.  Lemma~
\ref{lem:mop-1-formula} therefore gives
\begin{equation}
\nabla_\theta\hat\ell^1(\theta)=\rho_N^J(\psi_\theta),
\qquad
\nabla_\theta\ell(\theta)=\rho_N(\psi_\theta).
\label{eq:path-score-empirical}
\end{equation}

\begin{lem}[Concentration for the after-resampling DMOP-$1$ gradient]
\label{lemma:grad_bound}
Assume multinomial resampling.  For $i=1,\ldots,p$, let
\begin{equation}
A_N(\theta)=\max_{1\leq i\leq p}
\operatorname{osc}(\psi_{\theta,i}),
\qquad
\operatorname{osc}(u)=\sup_{z,z'}|u(z)-u(z')|.
\end{equation}
Under Assumptions~{\refAssumpBoundedMeasurement} and
{\refAssumpDifferentiableSimulator}, $A_N(\theta)<\infty$; in particular,
$A_N(\theta)\leq2NG'(\theta)$ under the displayed bound in
Assumption~{\refAssumpBoundedMeasurement}.  Let $r_N(\theta)$ and
$\beta_N(\theta)$ be the finite, $J$-independent constants in Theorem~1.2 of
\citet{delMoral11} for the full augmented historical mutation--selection
mean-field system, and abbreviate them as $r_N$ and $\beta_N$ below.  Set
$h(u)=\{u-\log(1+u)\}/2$.  Then, for every $\delta\in(0,1)$, with probability
at least $1-\delta$,
\begin{align}
\|\nabla_\theta\hat\ell^1(\theta)-\nabla_\theta\ell(\theta)\|_2
\leq A_N(\theta)\sqrt p\left[
\frac{r_N}{J}\left\{1+h^{-1}\!\left(\log\frac{2p}{\delta}\right)\right\}
+\beta_N\sqrt{\frac{2\log(2p/\delta)}{J}}
\right].
\label{eq:correct-gradient-concentration}
\end{align}
Consequently, the right side is at most $\epsilon$ if
\begin{align}
J\geq\max\Bigg\{
&\frac{2A_N(\theta)r_N\sqrt p}{\epsilon}
 \left\{1+h^{-1}\!\left(\log\frac{2p}{\delta}\right)\right\},
\frac{8A_N(\theta)^2\beta_N^2p\log(2p/\delta)}{\epsilon^2}
\Bigg\}.
\label{eq:correct-gradient-particle-count}
\end{align}
\end{lem}

\begin{proof}[Proof.]
If $A_N(\theta)=0$, every coordinate of $\psi_\theta$ is constant on the
augmented state space, so the left side of
Equation~\eqref{eq:correct-gradient-concentration} is zero.  Suppose henceforth
that $A_N(\theta)>0$.
Fix a coordinate $i$.  Apply Theorem~1.2 of \citet{delMoral11} to the
single terminal path-space test function
$\psi_{\theta,i}/A_N(\theta)$ and to its negative.  Equation~
\eqref{eq:path-score-empirical} then gives, with probability at least
$1-2e^{-t}$,
\begin{equation}
|\rho_N^J(\psi_{\theta,i})-\rho_N(\psi_{\theta,i})|
\leq A_N(\theta)\left[
\frac{r_N}{J}\{1+h^{-1}(t)\}
+\beta_N\sqrt{\frac{2t}{J}}
\right].
\end{equation}
A union bound over the $p$ coordinates and the choice
$t=\log(2p/\delta)$ give a simultaneous coordinatewise bound with failure
probability at most $\delta$.  The inequality
$\|v\|_2\leq\sqrt p\|v\|_\infty$ proves
Equation~\eqref{eq:correct-gradient-concentration}.  Requiring each of its two
terms to be at most $\epsilon/2$ gives
Equation~\eqref{eq:correct-gradient-particle-count}.
\end{proof}

The concentration theorem is applied once to the complete terminal path-score
functional.  Thus the only union bound in Lemma~\ref{lemma:grad_bound} is over
the $p$ coordinates; a second union over the $N$ summands would both target the
wrong filtering expectations and introduce an unnecessary factor of $N$.

The next lemma supplies the deterministic spectral property needed by the
optimization argument.  It avoids treating a particle log-likelihood Hessian
as a sum of independent or particlewise matrices: differentiating a logarithm
of an empirical average also produces outer-product and empirical-mean terms,
so that decomposition is generally unavailable.

\begin{lem}[Spectral regularization of a curvature estimate]
\label{lemma:hess_bound}
Let $\widetilde B=Q\operatorname{diag}(\lambda_1,\ldots,\lambda_p)Q^T$ be
an orthogonal eigendecomposition of any symmetric raw curvature estimate.  For
fixed $0<c\leq C$, define
\begin{equation}
B=Q\operatorname{diag}
\big(\min\{C,\max\{c,\lambda_i\}\}:i=1,\ldots,p\big)Q^T.
\label{eq:spectral-clipping}
\end{equation}
Then
\begin{equation}
cI_p\preceq B\preceq CI_p,
\qquad
C^{-1}I_p\preceq B^{-1}\preceq c^{-1}I_p.
\label{eq:preconditioner-bounds}
\end{equation}
In particular, $B$ is invertible.  The same conclusions hold for the choice
$B=I_p$ with $c=C=1$.
\end{lem}

\begin{proof}[Proof.]
Every eigenvalue in Equation~\eqref{eq:spectral-clipping} belongs to $[c,C]$.
The first Loewner-order statement follows immediately, and inversion reverses
the Loewner order to give the second.
\end{proof}

\begin{proof}[Proof of Theorem~{\thmMopConvergence}.]
Write $g_m=-\nabla_\theta\hat\ell^1(\theta_m)$ and
$p_m=-B_m^{-1}g_m$.  At each iteration, conditional on the past, $\theta_m$
is fixed and the fresh particle system satisfies Lemma~\ref{lemma:grad_bound}.
Use the uniform upper bounds on $A_N(\theta)$, $r_N$, and $\beta_N$ over the
region of interest in Equation~\eqref{eq:correct-gradient-particle-count}, with
$\delta$ replaced by $\delta/M$.  Let $\mathcal I_M$ be the random set of
iterations at which a particle filter is actually executed before stopping or
reaching the cap $M$.  If $\mathcal F_m$ is the history before the $m$th call,
then $\theta_m$ and the event $\{m\in\mathcal I_M\}$ are
$\mathcal F_m$-measurable, and the conditional failure probability of that
call is at most $\delta/M$.  Consequently,
\begin{align}
\Pr\left(\bigcup_{m\in\mathcal I_M}
\{\|g_m-\nabla f(\theta_m)\|_2>\epsilon\}\right)
&\leq\sum_{m=0}^{M-1}
\E\left[\mathbf 1_{\{m\in\mathcal I_M\}}
\Pr\{\|g_m-\nabla f(\theta_m)\|_2>\epsilon
\mid\mathcal F_m,\,m\in\mathcal I_M\}\right]\\
&\leq\delta.
\label{eq:all-gradient-events}
\end{align}
Thus, with probability at least $1-\delta$, the required oracle bound holds on
every executed call.  Independence across iterations is not needed for this
conditional union bound, but fresh particle randomness ensures that the
pointwise concentration statement applies at the adaptive iterate.

Work on this success event.  If the stopping condition
$\|g_m\|_2\leq\sigma\epsilon$ holds, then
\begin{equation}
\|\nabla f(\theta_m)\|_2\leq(\sigma+1)\epsilon.
\end{equation}
Strong convexity implies
$f(\theta)-f(\theta^*)\leq\|\nabla f(\theta)\|_2^2/(2\gamma)$, and hence
\begin{equation}
f(\theta_m)-f(\theta^*)
\leq\frac{(\sigma+1)^2\epsilon^2}{2\gamma}.
\label{eq:stopping-neighborhood}
\end{equation}

Suppose instead that $\|g_m\|_2>\sigma\epsilon$.  Smoothness of $f$ gives
\begin{align}
...
&\leq-\eta p_m^TB_mp_m
      +\eta\epsilon\|p_m\|_2
      +\frac{\Gamma\eta^2}{2}\|p_m\|_2^2.
\label{eq:correct-descent-start}
\end{align}
Because $g_m=-B_mp_m$ and $B_m\preceq CI_p$,
$\|g_m\|_2\leq C\|p_m\|_2$.  The nonstopping condition therefore yields
\begin{equation}
\epsilon\|p_m\|_2<\frac{C}{\sigma}\|p_m\|_2^2.
\end{equation}
The choices
$\sigma\geq2C/\{c(1-\beta)\}$ and
$\eta\leq c(1-\beta)/\Gamma$, together with
$p_m^TB_mp_m\geq c\|p_m\|_2^2$, now turn
Equation~\eqref{eq:correct-descent-start} into
\begin{equation}
f(\theta_{m+1})-f(\theta_m)
\leq-\eta\beta p_m^TB_mp_m.
\label{eq:correct-descent}
\end{equation}

It remains to relate the decrease to the objective gap.  On a nonstopping
iteration,
\begin{equation}
\|\nabla f(\theta_m)\|_2
\leq\|g_m\|_2+\epsilon
<\left(1+\frac1\sigma\right)\|g_m\|_2,
\end{equation}
and Equation~\eqref{eq:preconditioner-bounds} gives
\begin{align}
p_m^TB_mp_m
  &=g_m^TB_m^{-1}g_m\\
  ...
  &\geq\frac{2\gamma}{C}
     ...
     \{f(\theta_m)-f(\theta^*)\}.
\end{align}
Combining this inequality with Equation~\eqref{eq:correct-descent} proves
\begin{equation}
f(\theta_{m+1})-f(\theta^*)
\leq\left\{1-\frac{2\eta\beta\gamma}{C}
\left(\frac{\sigma}{\sigma+1}\right)^2\right\}
\{f(\theta_m)-f(\theta^*)\}.
\end{equation}
Finally substitute $f=-\ell$ and combine this result with
Equation~\eqref{eq:stopping-neighborhood}.
\end{proof}

The proof is deliberately stated for DMOP-$1$.  For $\alpha<1$, consistency as
$J\to\infty$ is toward a discounted population gradient rather than directly
toward the exact score.  Extending Theorem~{\thmMopConvergence} therefore
requires adding a high-probability bound for that discount bias; the claim does
not follow solely from choosing $\alpha$ sufficiently close to one.
