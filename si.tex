...
\begin{thm}[MSE of DMOP-$\alpha$]
    ...
    \begin{equation}
        \E\big\|\nabla_\theta\ell(\theta) - \nabla_\theta \hat\ell^\alpha(\theta)\big\|_2^2 \; \lesssim \; \min_{k \leq N} Np \, G'(\theta)^2\left(\frac{k^2}{J}+(1-\epsilon)^{\floor{k/(c\log(J))}}+k -\frac{\alpha(1-\alpha^k)}{1-\alpha}
+
\frac{\alpha^{2k}}{(1-\alpha)^2}\right).
        \nonumber
    \end{equation}
\end{thm}
...
The first term is the problem term. We will first bound the ground truth of conditional scores from particle approximation conditional on correct filtering distribution at time $n-k$, and then bound that from the particle approximation resulting from an arbitrary filtering distribution at time $n-k$. That is, write $\hat\pi_n$ for the particle approximation at time $n$ of the posterior $\pi_n$, and write $\nabla_\theta\hat s_{n,k|\hat\pi_{n-k}=\pi_{n-k}}^1(\theta)$ for the truncated MOP-$1$ conditional score estimate given the correct filtering distribution at time $n-k$.


We can first observe, by applying the triangle inequality in $L^2$ before
squaring, that
\begin{align}
    &\left(\E\big\|\nabla_\theta\ell(\theta) - \nabla_\theta \hat\ell^\alpha(\theta)\big\|_2^2\right)^{1/2}
    \nonumber \\
    &\leq
    \left(\E\big\|\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)\big\|_2^2\right)^{1/2}
    +
    \left(\E\big\|\nabla_\theta\hat s_k^1(\theta) - \nabla_\theta\hat s_k^\alpha(\theta)\big\|_2^2\right)^{1/2}
    \nonumber \\
    &\hspace{20mm}
    +
    \left(\E\big\|\nabla_\theta\hat s_k^\alpha(\theta) - \nabla_\theta\hat\ell^\alpha(\theta)\big\|_2^2\right)^{1/2}.
...
\paragraph{First term.} We bound the first term of Equation~(\ref{eq:mse-decomp}),
$\E\|\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)\|_2^2$, by decomposing it into two terms. By the same $L^2$ triangle inequality,
\begin{align}
    &\E\big\|\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)\big\|_2^2
    \nonumber \\
    &\lesssim
...
%way too loose. tighten

\paragraph{1a.} The former term is a sum of particle approximation errors, each of which
depends on at most $k$ timesteps. 
For each $n$, decompose the $k$-truncated conditional score approximation
error according to the lag of the score contribution:
\begin{equation}
    \nabla_\theta\ell_n(\theta)
    -
    \nabla_\theta\hat s^1_{n,k\mid\hat\pi_{n-k}=\pi_{n-k}}(\theta)
    =
    \sum_{r=0}^{k} Z_{n,r},
    \label{eq:conditional-score-lag-decomp}
\end{equation}
where $Z_{n,r}$ denotes the particle approximation error associated
with the score contribution at time $n-r$. By Lemma~2 of
\cite{karjalainen23}, applied to a single bounded score functional,
\begin{equation}
    \left(\E\|Z_{n,r}\|_2^2\right)^{1/2}
    \lesssim
    \frac{\sqrt{p}\,G'(\theta)}{\sqrt{J}},
    \qquad
    \left(\E\|Z_{n,r}\|_2^4\right)^{1/4}
    \lesssim
    \frac{\sqrt{p}\,G'(\theta)}{\sqrt{J}},
    \label{eq:conditional-score-lag-moments}
\end{equation}
uniformly in $n$ and $0\leq r\leq k$.

Fix a lag $r$. Since $Z_{n,r}$ is a measurable functional of the
particle system over a fixed number of consecutive filtering steps,
Lemma~S9 and Theorem~3 of \cite{karjalainen23} imply that
$\{Z_{n,r}\}_{n\geq1}$ is strongly mixing, with coefficients satisfying
\begin{equation}
    a_r(l)
    \lesssim
    (1-\epsilon)^{\floor{l/(c\log(J))}}.
    \label{eq:conditional-score-lag-mixing}
\end{equation}
For each coordinate $q=1,\ldots,p$, Davydov's inequality and
Equation~(\ref{eq:conditional-score-lag-moments}) therefore give
\begin{align}
    &\left|
    \Cov\left(
    Z_{n,r}^{(q)},
    ...
    &\lesssim
    a_r(l)^{1/2}
    \left(\E|Z_{n,r}^{(q)}|^4\right)^{1/4}
    \left(\E|Z_{n+l,r}^{(q)}|^4\right)^{1/4}
    \nonumber \\
    &\lesssim
    \frac{G'(\theta)^2}{J}
    (1-\epsilon)^{
    \frac{1}{2}\floor{l/(c\log(J))}}.
    \label{eq:conditional-score-lag-covariance}
\end{align}
Summing over the $p$ coordinates yields
\begin{equation}
    \left|
    \E\left[
    \left\langle Z_{n,r},Z_{n+l,r}\right\rangle
    \right]
    \right|
    \lesssim
    \frac{p\,G'(\theta)^2}{J}
    (1-\epsilon)^{
    \frac{1}{2}\floor{l/(c\log(J))}}.
    \label{eq:conditional-score-lag-inner-product}
\end{equation}
Moreover,
\begin{align}
    \sum_{l=0}^{\infty}
    ...
    &\lesssim
    \log(J).
    \label{eq:conditional-score-lag-geometric-sum}
\end{align}
...
\begin{align}
    ...
    \right\|_2^2
    &=
    \sum_{n=1}^N\E\|Z_{n,r}\|_2^2
    +
    2\sum_{1\leq m<n\leq N}
    \E\left[
    \left\langle Z_{m,r},Z_{n,r}\right\rangle
    \right]
    \nonumber \\
    &\lesssim
    \frac{Np\,G'(\theta)^2}{J}
    ...
    \nonumber \\
    &\lesssim
    \frac{Np\,G'(\theta)^2\log(J)}{J}.
    \label{eq:conditional-score-fixed-lag-mse}
\end{align}
Equivalently,
\begin{equation}
    \left(
    ...
    \right)^{1/2}
    \lesssim
    \frac{\sqrt{Np}\,G'(\theta)\sqrt{\log(J)}}{\sqrt{J}}.
    \label{eq:conditional-score-fixed-lag-rmse}
\end{equation}

We now apply the triangle inequality in $L^2$ over the $k+1$ lag
components in Equation~(\ref{eq:conditional-score-lag-decomp}):
\begin{align}
    &\left(
    \E\left\|
    \sum_{n=1}^N
    ...
    -
    \nabla_\theta
    \hat s^1_{n,k\mid\hat\pi_{n-k}=\pi_{n-k}}(\theta)
    \right)
    \right\|_2^2
    \right)^{1/2}
    \nonumber \\
    ...
    \nonumber \\
    &\lesssim
    \frac{k\sqrt{Np}\,G'(\theta)\sqrt{\log(J)}}{\sqrt{J}}.
    \label{eq:conditional-score-total-rmse}
\end{align}
...
\begin{equation}
    ...
    \right\|_2^2
    \lesssim
    \frac{Np\,G'(\theta)^2k^2}{J}.
    \label{eq:conditional-score-total-mse}
\end{equation}
...
% Therefore, combining the Monte Carlo approximation term with the initialization
% bias term gives
% \begin{align}
%     &\E\big\|\nabla_\theta\ell(\theta) - \nabla_\theta\hat s_k^1(\theta)\big\|_2^2
%     \nonumber \\
%     &\lesssim
%     N\frac{Cp \, G'(\theta)^2k^2}{J}
%     +
%     \left\|
%     \sum_{n=1}^N
%     \E\left[
%     \nabla_\theta\hat s^1_{n,k|\hat\pi_{n-k}=\pi_{n-k}}(\theta)
%     -
%     \nabla_\theta\hat s^1_{n,k}(\theta)
%     \right]
%     \right\|_2^2 .
% \end{align}

\paragraph{1b.} Bounding the latter term in the decomposition of
$\E\|\nabla_\theta\ell(\theta)-\nabla_\theta\hat s_k^1(\theta)\|_2^2$
amounts to bounding the effect of initializing the $k$-step particle
system from the empirical filtering distribution $\hat\pi_{n-k}$ rather
than the true filtering distribution $\pi_{n-k}$. The total-variation
contraction controls the difference between the corresponding exact
conditional score functionals, while the particle approximation errors
must be controlled separately.

For any probability measure $\mu$ on the particle system at time $n-k$,
write
\begin{equation}
    \nabla_\theta s^1_{n,k\mid\mu}(\theta)
    :=
    \E_{\mu\mathbf{M}_{n-k,n}}
    \left[
        \nabla_\theta\hat s^1_{n,k}(\theta)
    \right],
\end{equation}
where the expectation is over the $k$-step particle system initialized
from $\mu$. By the triangle inequality in $L^2$,
\begin{align}
    &\left(
    \E\left\|
    \sum_{n=1}^N
    \left(
    \nabla_\theta\hat s_{n,k\mid\hat\pi_{n-k}=\pi_{n-k}}^1(\theta)
    -
    \nabla_\theta\hat s_{n,k}^1(\theta)
    \right)
    \right\|_2^2
    ...
    \nabla_\theta\hat s_{n,k\mid\hat\pi_{n-k}=\pi_{n-k}}^1(\theta)
    -
    \nabla_\theta s^1_{n,k\mid\pi_{n-k}}(\theta)
    \right)
    \right\|_2^2
    \right)^{1/2}
    \nonumber \\
    ...
    \nabla_\theta s^1_{n,k\mid\pi_{n-k}}(\theta)
    -
    \nabla_\theta s^1_{n,k\mid\hat\pi_{n-k}}(\theta)
    \right)
    \right\|_2^2
    \right)^{1/2}
    \nonumber \\
    ...
    \right\|_2^2
    \right)^{1/2}.
    \label{eq:initialization-rmse-decomp}
\end{align}

The first and third terms in
Equation~(\ref{eq:initialization-rmse-decomp}) are particle
approximation errors for $k$-truncated score functionals. Applying the
fixed-lag covariance bound above to each of these terms gives
\begin{align}
    &\E\left\|
    \sum_{n=1}^N
    \left(
    ...
    \nonumber \\
    &\lesssim
    \frac{Np\,G'(\theta)^2k^2\log(J)}{J}.
    \label{eq:initialization-monte-carlo}
\end{align}

It remains to control the initialization discrepancy between the two
exact conditional score functionals. The $k$-truncated conditional
score has oscillation bounded by $k\sqrt p\,G'(\theta)$, since it is a
sum of at most $k$ score increments, each bounded by
$\sqrt p\,G'(\theta)$ under
Assumption~{\refAssumpBoundedMeasurement}. Consequently,
\begin{align}
    &\left\|
    \nabla_\theta s^1_{n,k\mid\pi_{n-k}}(\theta)
    -
    ...
    k\sqrt p\,G'(\theta)
    \left\|
    \pi_{n-k}\mathbf{M}_{n-k,n}
    -
    \hat\pi_{n-k}\mathbf{M}_{n-k,n}
    \right\|_{\mathrm{TV}}
    \nonumber \\
    &\leq
    k\sqrt p\,G'(\theta)
    \beta_{\mathrm{TV}}(\mathbf{M}_{n-k,n})
    \left\|
    \pi_{n-k}-\hat\pi_{n-k}
    \right\|_{\mathrm{TV}}.
    \label{eq:initialization-tv-bound}
\end{align}
By Theorem~3 of \cite{karjalainen23},
\begin{equation}
    \beta_{\mathrm{TV}}(\mathbf{M}_{n-k,n})
    \leq
    (1-\epsilon)^{\floor{k/(c\log(J))}}.
\end{equation}
Using
$\|\pi_{n-k}-\hat\pi_{n-k}\|_{\mathrm{TV}}\leq1$ in
Equation~(\ref{eq:initialization-tv-bound}) gives
\begin{equation}
    \left\|
    \nabla_\theta s^1_{n,k\mid\pi_{n-k}}(\theta)
    -
    ...
    k\sqrt p\,G'(\theta)
    (1-\epsilon)^{\floor{k/(c\log(J))}}.
    \label{eq:initialization-per-time-rmse}
\end{equation}
Therefore, applying the triangle inequality across timesteps,
\begin{align}
    &\left(
    \E\left\|
    \sum_{n=1}^N
...
\begin{align}
    ...
    &\lesssim
    N^2p\,G'(\theta)^2k^2
    (1-\epsilon)^{2\floor{k/(c\log(J))}}.
    \label{eq:initialization-total-mse}
\end{align}

Combining Equations~(\ref{eq:initialization-rmse-decomp}),
(\ref{eq:initialization-monte-carlo}), and
(\ref{eq:initialization-total-mse}), we obtain
\begin{align}
    &\E\left\|
    \sum_{n=1}^N
    \left(
    ...
    N^2p\,G'(\theta)^2k^2
    (1-\epsilon)^{2\floor{k/(c\log(J))}}.
    \label{eq:initialization-difference-total}
\end{align}

Combining this with the preceding bound for the particle approximation
conditional on the correct filtering distribution, the first term in
Equation~(\ref{eq:mse-decomp}) satisfies
\begin{align}
    &\E\big\|
    \nabla_\theta\ell(\theta)
    -
    ...
    N^2p\,G'(\theta)^2k^2
    (1-\epsilon)^{2\floor{k/(c\log(J))}}.
    \label{eq:first-mse-term}
\end{align}

Now that the first term,
$\E\|\nabla_\theta\ell(\theta)
-\nabla_\theta\hat s_k^1(\theta)\|_2^2$, is controlled, it remains to
bound
$\E\|\nabla_\theta\hat s_k^1(\theta)
...
\paragraph{Second term.} We see, by a similar argument to the variance bound, that
\begin{align}
    &\left(
    \E\big\|
    \nabla_\theta\hat s_k^1(\theta)
    ...
    \left[
    \log\left(\sum_{j=1}^J
    w_{n,j}^{F,\theta,1,k}\right)
    -
    \log\left(\sum_{j=1}^J
    w_{n,j}^{F,\theta,\alpha,k}\right)
    \right]
    \right\|_2^2
    \right)^{1/2}
    \nonumber \\
    ...
    \left[
    \log\left(\sum_{j=1}^J
    w_{n-1,j}^{A,F,\theta,1,k}\right)
    -
    \log\left(\sum_{j=1}^J
    w_{n-1,j}^{A,F,\theta,\alpha,k}\right)
    \right]
    \right\|_2^2
    \right)^{1/2}.
    \label{eq:second-term-rmse-decomposition}
\end{align}

At $\theta=\phi$,
$w_{n,j}^{F,\theta,1,k}
=
w_{n,j}^{F,\theta,\alpha,k}
=1$, and therefore
\begin{align}
    &\nabla_\theta\log\left(
    \sum_{j=1}^Jw_{n,j}^{F,\theta,1,k}
    \right)
    -
    \nabla_\theta\log\left(
    \sum_{j=1}^Jw_{n,j}^{F,\theta,\alpha,k}
    \right)
    \nonumber \\
    &=
    \frac{1}{J}
    \sum_{j=1}^J
    \sum_{i=n-k}^{n}
    \left(1-\alpha^{n-i}\right)
    \nabla_\theta\log
    \left(g_{i,j}^{A,F,\theta}\right).
    \label{eq:second-term-filtering-difference}
\end{align}
Hence, by Jensen's inequality followed by the weighted
...
\begin{align}
    ...
    \left(
    \sum_{r=1}^{k}
    \left(1-\alpha^r\right)
    \right)^2
    \nonumber \\
    &=
    p\,G'(\theta)^2
    \left(
    k-\frac{\alpha(1-\alpha^k)}{1-\alpha}
    \right)^2,
    \label{eq:second-term-filtering-bound}
\end{align}
where the final inequality follows from
Assumption~{\refAssumpBoundedMeasurement}. The analogous ancestral
log-weight difference satisfies the same bound. Therefore,
Equation~(\ref{eq:second-term-rmse-decomposition}) gives
\begin{align}
    &\left(
    \E\big\|
    \nabla_\theta\hat s_k^1(\theta)
...
\begin{align}
    ...
    &\leq
    N^2p\,G'(\theta)^2k^2.
    \label{eq:second-term-mse-bound}
\end{align}

\kevin{TODO}

\paragraph{Third term.}
We now bound the third term in
Equation~(\ref{eq:mse-decomp}),
\begin{equation}
    \E\left\|
    \nabla_\theta\hat s_k^\alpha(\theta)
    -
...
\begin{align}
    ...
    \log\left(
    \sum_{j=1}^J
    w_{n-1,j}^{A,F,\theta,\alpha}
    \right)
    -
    \log\left(
    \sum_{j=1}^J
    w_{n-1,j}^{A,F,\theta,\alpha,k}
    \right)
    \right]
    \right\|_2^2
    \right)^{1/2}.
    \label{eq:third-term-rmse-decomposition}
\end{align}
At $\theta=\phi$, all the weights in the logarithms are equal to one.
Therefore,
\begin{align}
    &\nabla_\theta
    \left[
    ...
    \alpha^{n-i}
    \nabla_\theta
    \log\left(g_{i,j}^{A,F,\theta}\right).
    \label{eq:third-term-filtering-tail}
\end{align}
Note that the full weight contains the score contributions from
$i=1,\ldots,n$, whereas the truncated weight retains
$i=n-k,\ldots,n$. The
$i=1,\ldots,n-k-1$ terms are therefore omitted.

Using Jensen's inequality, Cauchy-Schwarz, and
Assumption~{\refAssumpBoundedMeasurement},
\begin{align}
    &\left\|
    \nabla_\theta
    \left[
    ...
    &\leq
    p\,G'(\theta)^2
    \frac{\alpha^{2k+2}}{(1-\alpha)^2}.
    \label{eq:third-term-filtering-tail-bound}
\end{align}
The corresponding ancestral log-weight difference satisfies the same
bound. Substituting
Equation~(\ref{eq:third-term-filtering-tail-bound}) into
Equation~(\ref{eq:third-term-rmse-decomposition}) gives
\begin{align}
    &\left(
    \E\left\|
    \nabla_\theta\hat\ell^\alpha(\theta)
...
\begin{equation}
    ...
    \lesssim
    \frac{
    N^2p\,G'(\theta)^2\alpha^{2k+2}
    }{
    (1-\alpha)^2
    }.
    \label{eq:third-term-final}
\end{equation}
...
Combining the bounds for the three terms in
Equation~(\ref{eq:mse-decomp}), we obtain, for every $k\leq N$,
\begin{align}
    &\E\left\|
    \nabla_\theta\ell(\theta)
    -
    ...
    \label{eq:dmop-alpha-final-mse-fixed-k}
\end{align}
Since this bound holds for every $k\leq N$, it follows that
\begin{align}
    &\E\left\|
    \nabla_\theta\ell(\theta)
    -
    ...
    \right)^2
    +
    \frac{N^2\alpha^{2k+2}}{(1-\alpha)^2}
    \right\}.
    \label{eq:dmop-alpha-final-mse}
\end{align}
...
\end{align}
\kevin{Fix expression}
\end{proof}



\begin{cor}[MSE of DMOP-$0$]
    The DMOP-$0$ estimator satisfies
    \begin{align}
        &\E\left\|
        \nabla_\theta\ell(\theta)
        -
        ...
        \label{eq:dmop-zero-mse}
    \end{align}
\end{cor}

\begin{proof}[Proof.]
When $\alpha=0$, the DMOP-$0$ gradient estimator is exactly the
one-step truncated DMOP-$1$ estimator:
\begin{equation}
    \nabla_\theta\hat\ell^0(\theta)
    =
    \nabla_\theta\hat s_1^1(\theta).
    \label{eq:cor-s1}
\end{equation}
Indeed, from Lemma~\ref{lem:mop-0-formula},
\begin{equation}
    \nabla_\theta\hat\ell^0(\theta)
    =
    \frac{1}{J}
    ...
    \right),
\end{equation}
which is precisely the gradient obtained by retaining only the current
measurement-score contribution in the truncated DMOP-$1$ estimator.
...
\begin{align}
    ...
    \nabla_\theta\hat s_1^1(\theta),
\end{align}
there are no analogues of the second and third terms in
Equation~(\ref{eq:mse-decomp}). Applying
Equation~(\ref{eq:first-mse-term}) with $k=1$ gives
\begin{align}
    &\E\left\|
    \nabla_\theta\ell(\theta)
    -
...
\paragraph{Interpretation and choice of the truncation level.}
Write
\begin{equation}
    A_k(\alpha)
    :=
    \sum_{r=1}^k(1-\alpha^r)
    =
    k-\frac{\alpha(1-\alpha^k)}{1-\alpha}.
    \label{eq:alpha-window-discrepancy}
\end{equation}
The preceding argument gives the bound
\begin{align}
    &\E\left\|
    \nabla_\theta\ell(\theta)
    -
    ...
    \frac{N^2\alpha^{2k+2}}{(1-\alpha)^2}
    \right\}.
    \label{eq:dmop-alpha-crude-mse}
\end{align}

The last two terms exhibit the principal tradeoff in the truncation
level. The quantity $A_k(\alpha)$ is increasing in $k$, whereas
$\alpha^{k+1}/(1-\alpha)$ is decreasing in $k$. Increasing $k$
therefore retains more of the MOP-$\alpha$ history and reduces the
truncation tail, but increases the discrepancy between the truncated
MOP-$1$ and truncated MOP-$\alpha$ estimators.

For a fixed $k$, increasing $\alpha$ decreases
$A_k(\alpha)$, because $ 1-\alpha^r$
decreases for every $r$. At the same time, increasing $\alpha$ makes
the tail $\frac{\alpha^{k+1}}{1-\alpha}$
larger and more persistent. Thus, larger values of $\alpha$ require
larger truncation levels $k$.

To determine the effective-memory scale when $\alpha$ is close to one,
write $\delta:=1-\alpha$
and set $ k=\frac{x}{\delta}.$
For fixed $x>0$, as $\delta\downarrow0$,
\begin{equation}
    \alpha^k
    =
    (1-\delta)^{x/\delta}
...
\begin{align}
    ...
    -
    \frac{(1-\delta)
    \left\{1-(1-\delta)^{x/\delta}\right\}}{\delta}
    \nonumber \\
    &=
    \frac{x-1+e^{-x}}{\delta}
    +
    O(1).
    \label{eq:alpha-window-asymptotics}
\end{align}
Consequently, the last two terms in
Equation~(\ref{eq:dmop-alpha-crude-mse}) are asymptotically
proportional to
\begin{equation}
    \frac{N^2pG'(\theta)^2}{(1-\alpha)^2}
    \left\{
    (x-1+e^{-x})^2+e^{-2x}
    \right\}.
    \label{eq:alpha-tradeoff-objective}
\end{equation}
The minimizing constant $x_\star$ satisfies
\begin{equation}
    (x_\star-1+e^{-x_\star})
    (1-e^{-x_\star})
    =
...
\begin{equation}
    ...
    \min\left\{
    N,\,
    \ceil{\frac{0.879}{1-\alpha}}
    \right\}.
    \label{eq:crude-optimal-k}
\end{equation}
For rate calculations, the simpler choice
\begin{equation}
    k_\alpha
    \asymp
    \min\left\{
...
and
\begin{equation}
    \frac{\alpha^{k_\alpha+1}}{1-\alpha}
    \asymp
    \frac{1}{1-\alpha}.
\end{equation}
Substituting this choice into
Equation~(\ref{eq:dmop-alpha-crude-mse}) gives
\begin{align}
    &\E\left\|
    \nabla_\theta\ell(\theta)
    -
    ...
    \right\},
    \label{eq:crude-bound-substituted}
\end{align}
where $C>0$ is an absolute constant. Since the forgetting factor is
at most one, this implies
\begin{equation}
    \E\left\|
    \nabla_\theta\ell(\theta)
    -
    ...
    N^2+\frac{N\log(J)}{J}
    \right\}.
    \label{eq:crude-alpha-rate}
\end{equation}

This conclusion should be interpreted cautiously. The divergence as
$\alpha\uparrow1$ is an artifact of separately bounding the
within-window discrepancy and the truncation tail. The estimator
itself converges to MOP-$1$ as $\alpha\uparrow1$, but
Equation~(\ref{eq:dmop-alpha-crude-mse}) discards the cancellation
required to recover this limit.
...
% \paragraph{Sharper bias--variance analysis.}
% The preceding truncation argument is useful for bounding stochastic
% fluctuations, but it is too crude for bounding the systematic
% dependence on $\alpha$. In particular, separately bounding
% \begin{equation}
%     \nabla_\theta\hat s_k^1(\theta)
%     -
%     \nabla_\theta\hat s_k^\alpha(\theta)
...
%     \nabla_\theta\hat\ell^\alpha(\theta)
% \end{equation}
% discards cancellation between the contribution inside the truncation
% window and the omitted tail. This leads to a bound that diverges as
% $\alpha\uparrow1$, even though the MOP-$\alpha$ estimator converges to
% the MOP-$1$ estimator.

% A sharper analysis begins with the exact bias--variance decomposition.
% Write
% \begin{equation}
%     E_n^\alpha(\theta)
%     :=
...
%     \sum_{n=1}^N
%     \left\{
%     E_n^\alpha(\theta)-\E E_n^\alpha(\theta)
%     \right\}
%     \right\|_2^2
%     +
%     \left\|
...
%     \E E_n^\alpha(\theta)
%     \right\|_2^2.
%     \label{eq:exact-mse-bias-variance}
% \end{align}

% The first term in
% Equation~(\ref{eq:exact-mse-bias-variance}) is a centered fluctuation.
% It can be controlled by the same particle-filter mixing and covariance
% summation argument used in the variance theorem. In particular, one
% expects a bound of the form
% \begin{equation}
%     \E\left\|
%     \sum_{n=1}^N
%     \left\{
...
%     \right\|_2^2
%     \lesssim
%     \frac{NpG'(\theta)^2\tau_J}{J}
%     V_\alpha,
%     \label{eq:centered-alpha-error}
% \end{equation}
% where $\tau_J\asymp\log(J)$ is the particle-filter mixing time and
% $V_\alpha$ records the effective memory of the score functional.

% The second term must be bounded directly. The appropriate argument is
% lagwise forgetting. Let the effect at time $n$ of a score increment
% from time $n-r$ be denoted schematically by
% \begin{equation}
%     \mathcal{H}_{n,r}.
% \end{equation}
% The MOP-$1$ estimator gives this lag full coefficient one, whereas
% MOP-$\alpha$ gives it coefficient $\alpha^r$. Their difference is
% therefore weighted by
% \begin{equation}
%     1-\alpha^r.
% \end{equation}
% If the effect of a perturbation at lag $r$ contracts geometrically,
% then the required lagwise estimate is
% \begin{equation}
%     \left\|
%     \E\mathcal{H}_{n,r}
%     \right\|_2
%     \lesssim
%     \sqrt p\,G'(\theta)\rho_J^r,
%     \label{eq:lagwise-forgetting-lemma}
% \end{equation}
% where $\rho_J\in(0,1)$ is determined by the particle-filter forgetting
% rate. Theorem~3 of \cite{karjalainen23} suggests
% \begin{equation}
%     \rho_J
%     :=
%     (1-\epsilon)^{1/(c\log(J))}
% \end{equation}
% up to constants and the treatment of the floor function.

% Under Equation~(\ref{eq:lagwise-forgetting-lemma}), the per-time bias
% induced by discounting satisfies
% \begin{align}
%     \left\|
%     \E E_n^\alpha(\theta)
%     \right\|_2
...
%     \sqrt p\,G'(\theta)
%     \frac{\rho_J(1-\alpha)}
%     {(1-\rho_J)(1-\alpha\rho_J)}
%     +
%     \frac{\sqrt p\,G'(\theta)}{J}.
%     \label{eq:per-time-alpha-bias}
% \end{align}
% The first term is the approximation bias due to $\alpha<1$, while the
% second is the finite-particle bias.
...
%     \frac{1}{J}
%     \right\}^2.
%     \label{eq:cumulative-alpha-bias}
% \end{align}
% Combining
% Equations~(\ref{eq:centered-alpha-error}) and
% (\ref{eq:cumulative-alpha-bias}) gives the refined form
% \begin{align}
%     &\E\left\|
%     \nabla_\theta\hat\ell^\alpha(\theta)
%     -
...
%     \frac{1}{J}
%     \right\}^2.
%     \label{eq:refined-alpha-mse}
% \end{align}

% Unlike the separate truncation bound, the approximation-bias
% coefficient in Equation~(\ref{eq:refined-alpha-mse}) satisfies
% \begin{equation}
%     \frac{\rho_J(1-\alpha)}
%     {(1-\rho_J)(1-\alpha\rho_J)}
%     \longrightarrow0
...
%     \alpha\uparrow1.
% \end{equation}
% Thus, the quadratic factor $N^2$ does not represent an unavoidable
% $N^2$ error. It is the square of the cumulative systematic bias, whose
% coefficient vanishes as $\alpha$ approaches one and as $J$ increases.

% For the average score, Equation~(\ref{eq:refined-alpha-mse}) becomes
% \begin{align}
%     &\E\left\|
%     \frac{1}{N}
%     \left\{
...
%     \frac{1}{J}
%     \right\}^2.
%     \label{eq:refined-average-score-mse}
% \end{align}

% To obtain an $O(N)$ MSE for the unnormalized score, it is sufficient
% that the per-time bias be at most of order $N^{-1/2}$. Thus it is
% sufficient to require
% \begin{equation}
%     \frac{\rho_J(1-\alpha_N)}
%     {(1-\rho_J)(1-\alpha_N\rho_J)}
%     \lesssim
%     \frac{1}{\sqrt N}
%     \label{eq:alpha-bias-rate-condition}
% \end{equation}
% and
% \begin{equation}
%     J_N\gtrsim\sqrt N.
%     \label{eq:particle-bias-rate-condition}
% \end{equation}
% Since
% \begin{equation}
%     1-\rho_J
...
%     \sqrt N\,\log(J_N)^2
%     }.
%     \label{eq:sufficient-alpha-sequence}
% \end{equation}
% Under
% Equations~(\ref{eq:particle-bias-rate-condition}) and
% (\ref{eq:sufficient-alpha-sequence}), the squared-bias contribution is
% of order at most $N$, rather than $N^2$. We formalize the argument below.

% \subsection{A refined MSE bound via lagwise score forgetting}

% The preceding truncation argument loses the cancellation between the
% within-window discrepancy and the omitted tail. We instead decompose
% the DMOP-$\alpha$ error into its centered fluctuation and systematic
% bias, and control the latter directly using lagwise forgetting.

% For $i\leq n$, let
% \begin{equation}
%     H_{n,i}(\theta)
% \end{equation}
% denote the contribution of the measurement-score increment at time
% $i$ to the conditional score increment at time $n$. Equivalently,
% writing $r=n-i$, we use the notation
% \begin{equation}
%     H_{n,r}(\theta)
%     :=
%     H_{n,n-r}(\theta).
...
% For \(i \geq 2\), define the local complete-data score increment
% \begin{equation}
%     \zeta_i^\theta(x_{i-1},x_i)
%     :=
%     \nabla_\theta
%     \log f_{X_i \mid X_{i-1}}
%     (x_i \mid x_{i-1};\theta)
%     +
%     \nabla_\theta
%     \log f_{Y_i \mid X_i}
%     (y_i^\ast \mid x_i;\theta),
% \end{equation}
% and define
% \begin{equation}
%     \zeta_1^\theta(x_1)
%     :=
%     \nabla_\theta \log f_{X_1}(x_1;\theta)
%     +
%     \nabla_\theta
%     \log f_{Y_1 \mid X_1}
%     (y_1^\ast \mid x_1;\theta).
% \end{equation}
...
%         \zeta_i^\theta(X_{i-1},X_i)
%         \,\middle|\,
%         Y_{1:n-1}=y_{1:n-1}^\ast
%     \right],
% \end{equation}
% where, for \(i=1\), the argument \(X_0\) is omitted, and for
% \(i=n\), the second conditional expectation is defined to be zero.
% Thus,
% \begin{equation}
%     H_{n,n}(\theta)
%     =
...
% \begin{equation}
%     \nabla_\theta
%     \log f_{Y_n \mid Y_{1:n-1}}
%     (y_n^\ast \mid y_{1:n-1}^\ast;\theta)
%     =
%     \sum_{i=1}^n H_{n,i}(\theta).
% \end{equation}
% The differentiated MOP recursion gives the lagwise representations
% \begin{equation}
%     \nabla_\theta\ell_n(\theta)
%     =
%     \sum_{r=0}^{n-1} H_{n,r}(\theta)
%     \label{eq:exact-score-lag-decomposition}
% \end{equation}
% and
% \begin{equation}
...
%     \sum_{r=0}^{n-1}
%     \alpha^r H_{n,r}(\theta),
%     \label{eq:population-alpha-score}
% \end{equation}
% where $\nabla_\theta\ell_n^\alpha(\theta)$ denotes the
% infinite-particle target of the $n$th DMOP-$\alpha$ score increment.
% Thus,
% \begin{equation}
%     \nabla_\theta\ell_n(\theta)
%     -
...
%     =
%     \sum_{r=1}^{n-1}
%     (1-\alpha^r)H_{n,r}(\theta).
%     \label{eq:direct-alpha-score-difference}
% \end{equation}

% We first establish the required lagwise forgetting estimate.

% \begin{lem}[Lagwise forgetting of conditional score contributions]
% \label{lem:lagwise-score-forgetting}
% Suppose Assumptions~{\refAssumpBoundedProcess} and
% {\refAssumpBoundedMeasurement} hold. Then there exist constants
% $C<\infty$ and $\rho\in(0,1)$, depending only on
% $M,\bar M,G,\bar G$, such that, uniformly in $n$ and
% $0\leq r<n$,
% \begin{equation}
%     \left\|
%     H_{n,r}(\theta)
%     \right\|_2
%     \leq
%     C\sqrt p\,G'(\theta)\rho^r.
%     \label{eq:lagwise-score-forgetting}
% \end{equation}
% \end{lem}

% \begin{proof}
% Under Assumptions~{\refAssumpBoundedProcess} and
% {\refAssumpBoundedMeasurement}, every filtering and backward
% smoothing kernel admits a uniform minorization. In particular, there
% exists $\varepsilon_0>0$, depending only on
% $M,\bar M,G,\bar G$, such that the corresponding Dobrushin
...
%     1
% \end{equation}
% uniformly in $m$. By submultiplicativity of the Dobrushin
% coefficient,
% \begin{equation}
%     \beta_{\mathsf{TV}}
%     \left(
...
%     \leq
%     \rho^{n-i}.
%     \label{eq:backward-kernel-contraction}
% \end{equation}

% The lag-$r$ score contribution is the difference between two
% expectations of the bounded score increment at time $n-r$, propagated
% through $r$ filtering or backward-smoothing steps. Hence, for each
% coordinate $q=1,\ldots,p$,
% \begin{align}
%     \left|
%     H_{n,r}^{(q)}(\theta)
%     \right|
...
%     C G'(\theta)\rho^r,
% \end{align}
% where the first inequality is the standard contraction inequality
% \begin{equation}
%     \left|
%     \mu K(h)-\nu K(h)
%     \right|
...
%     \beta_{\mathsf{TV}}(K)
% \end{equation}
% for probability measures $\mu,\nu$, a Markov kernel $K$, and a
% bounded function $h$. Summing the coordinatewise bounds gives
% \begin{equation}
%     \left\|
%     H_{n,r}(\theta)
%     \right\|_2
...
% The preceding lemma is not an additional model assumption. It is a
% consequence of the uniform lower and upper bounds already imposed in
% Assumptions~{\refAssumpBoundedProcess} and
% {\refAssumpBoundedMeasurement}.

% We next record the particle approximation bounds needed below.

% \begin{lem}[Lagwise particle approximation]
% \label{lem:lagwise-particle-approximation}
% Under Assumptions~{\refAssumpBoundedProcess} and
% {\refAssumpBoundedMeasurement}, let
% $\widehat H_{n,r}^J(\theta)$ denote the particle approximation of
% $H_{n,r}(\theta)$. There exists $C<\infty$, independent of
% $n,r,N,J$, such that
% \begin{align}
%     \left\|
%     \E\widehat H_{n,r}^J(\theta)
%     -
%     H_{n,r}(\theta)
%     \right\|_2
%     &\leq
%     \frac{C\sqrt p\,G'(\theta)}{J},
%     \label{eq:lagwise-particle-bias}
%     \\
%     \left(
%     \E\left\|
%     \widehat H_{n,r}^J(\theta)
%     -
%     \E\widehat H_{n,r}^J(\theta)
%     \right\|_2^4
%     \right)^{1/4}
%     &\leq
%     \frac{C\sqrt p\,G'(\theta)}{\sqrt J}.
%     \label{eq:lagwise-particle-l4}
% \end{align}
% \end{lem}

% \begin{proof}
% Each $\widehat H_{n,r}^J(\theta)$ is a normalized
% Feynman--Kac particle approximation of a bounded score functional.
% Assumption~{\refAssumpBoundedMeasurement} gives
% \begin{equation}
%     \left\|
%     \nabla_\theta\log g_i^\theta
%     \right\|_2
%     \leq
%     \sqrt p\,G'(\theta),
% \end{equation}
% while Assumptions~{\refAssumpBoundedProcess} and
% {\refAssumpBoundedMeasurement} give uniform stability of the
% Feynman--Kac semigroup.

% The standard first-order expansion of a normalized particle measure
% around its deterministic Feynman--Kac flow gives
% \begin{equation}
%     \left|
%     \E\left[
%     \eta_n^J(h)
...
%     \right|
%     \leq
%     \frac{C\operatorname{osc}(h)}{J},
% \end{equation}
% uniformly in time. Applying this coordinatewise to the bounded
% lagwise score functional gives
% Equation~(\ref{eq:lagwise-particle-bias}).

% Likewise, the standard $L^4$ particle approximation inequality gives
% \begin{equation}
%     \left(
%     \E\left|
%     \eta_n^J(h)-\eta_n(h)
...
%     \right)^{1/4}
%     \leq
%     \frac{C\operatorname{osc}(h)}{\sqrt J}.
% \end{equation}
% Applying this coordinatewise and summing over the $p$ coordinates
% gives Equation~(\ref{eq:lagwise-particle-l4}).
% \end{proof}

% We may now state the refined MSE result.

% \begin{thm}[Refined MSE bound for DMOP-$\alpha$]
% \label{thm:refined-dmop-alpha-mse}
% Suppose Assumptions~{\refAssumpBoundedProcess} and
% {\refAssumpBoundedMeasurement} hold, let $\alpha\in[0,1)$, and set
% $\theta=\phi$. Then there exist constants $C<\infty$,
% $\rho\in(0,1)$, and a particle mixing time
% \begin{equation}
%     \tau_J
%     \lesssim
%     \log(J),
...
%     \right\}^2
%     \right].
%     \label{eq:refined-dmop-alpha-mse-final}
% \end{align}
% In particular, since $\tau_J\lesssim\log(J)$,
% \begin{align}
%     &\E\left\|
%     \nabla_\theta\hat\ell^\alpha(\theta)
%     -
...
%     \right\}^2
%     \right].
%     \label{eq:refined-dmop-alpha-logj}
% \end{align}
% \end{thm}
...
%     \E E_n^\alpha(\theta)
%     \right\|_2^2.
%     \label{eq:refined-exact-bias-variance}
% \end{align}

% We first bound the bias. From
% Equations~(\ref{eq:population-alpha-score}) and
% (\ref{eq:direct-alpha-score-difference}),
% \begin{align}
%     \E E_n^\alpha(\theta)
%     &=
%     \sum_{r=0}^{n-1}
...
%     \sum_{r=1}^{n-1}
%     (1-\alpha^r)H_{n,r}(\theta).
%     \label{eq:per-time-bias-decomposition}
% \end{align}
% Therefore, by Lemmas~\ref{lem:lagwise-score-forgetting} and
% \ref{lem:lagwise-particle-approximation},
% \begin{align}
%     \left\|
%     \E E_n^\alpha(\theta)
%     \right\|_2
...
%     (1-\alpha^r)\rho^r
%     \right\}.
%     \label{eq:per-time-bias-before-sum}
% \end{align}
...
%     (1-\alpha^r)\rho^r
%     &=
%     \sum_{r=1}^{\infty}\rho^r
%     -
%     \sum_{r=1}^{\infty}(\alpha\rho)^r
%     \nonumber \\
%     &=
%     \frac{\rho}{1-\rho}
%     -
%     \frac{\alpha\rho}{1-\alpha\rho}
%     \nonumber \\
%     &=
%     \frac{
%     \rho(1-\alpha)
...
%     (1-\rho)(1-\alpha\rho)
%     }.
%     \label{eq:exact-lagwise-alpha-series}
% \end{align}
% Consequently,
% \begin{equation}
%     \left\|
...
%     }
%     \right\}.
%     \label{eq:refined-per-time-bias}
% \end{equation}
% It follows that
% \begin{align}
%     \left\|
...
%     }
%     \right\}^2.
%     \label{eq:refined-cumulative-bias}
% \end{align}

% We next bound the centered fluctuation. The lagwise representation is
% \begin{equation}
%     E_n^\alpha(\theta)
%     -
%     \E E_n^\alpha(\theta)
...
%     \E\widehat H_{n,r}^J(\theta)
%     \right\}.
%     \label{eq:centered-lagwise-representation}
% \end{equation}
% For fixed lags $r,s$, Lemma~S9 and the particle-filter contraction
% bound imply that
% \begin{align}
%     &\left|
%     \E\left[
...
%     \left|n-m\right|-r-s
%     \right)^{1/2},
%     \label{eq:lagwise-covariance-bound}
% \end{align}
% where $a_J(l)=1$ for $l\leq0$ and, for $l>0$,
% \begin{equation}
%     a_J(l)
%     \leq
%     (1-\epsilon)^{
...
% Moreover,
% \begin{equation}
%     \sum_{l=0}^{\infty}a_J(l)^{1/2}
%     \lesssim
%     \tau_J
%     \lesssim
%     \log(J).
%     \label{eq:particle-mixing-sum}
% \end{equation}
% Equation~(\ref{eq:lagwise-covariance-bound}) follows from
% Davydov's inequality together with the $L^4$ estimate in
% Equation~(\ref{eq:lagwise-particle-l4}).

% Expanding the centered second moment and using
% Equations~(\ref{eq:centered-lagwise-representation})--%
% (\ref{eq:particle-mixing-sum}), we obtain
% \begin{align}
%     &\E\left\|
%     \sum_{n=1}^N
%     \left\{
...
%     J(1-\alpha)^2
%     }.
%     \label{eq:refined-centered-variance}
% \end{align}
% Combining
% Equations~(\ref{eq:refined-exact-bias-variance}),
% (\ref{eq:refined-cumulative-bias}), and
% (\ref{eq:refined-centered-variance}) proves
% Equation~(\ref{eq:refined-dmop-alpha-mse-final}).
% \end{proof}

% \begin{cor}[Average-score MSE]
% \label{cor:refined-average-score-mse}
...
%     \right\}^2
%     \right].
%     \label{eq:refined-average-score-mse-final}
% \end{align}
% \end{cor}

% \begin{cor}[Conditions for an $O(N)$ unnormalized MSE]
% \label{cor:linear-n-mse}
% Suppose $\alpha=\alpha_N$ and $J=J_N$ satisfy
% \begin{equation}
%     1-\alpha_N
%     \lesssim
%     N^{-1/2},
%     \label{eq:alpha-linear-n-condition}
% \end{equation}
% \begin{equation}
%     J_N(1-\alpha_N)
%     \gtrsim
%     \sqrt N,
%     \label{eq:particle-bias-linear-n-condition}
% \end{equation}
% and
% \begin{equation}
%     \frac{
...
%     \lesssim
%     1.
%     \label{eq:variance-linear-n-condition}
% \end{equation}
% Then
% \begin{equation}
%     \E\left\|
...
% \begin{proof}
% Because $\rho<1$ is independent of $N$ and $J$,
% \begin{equation}
%     \frac{
%     \rho(1-\alpha_N)
%     }{
...
% \end{equation}
% Thus,
% Equations~(\ref{eq:alpha-linear-n-condition}) and
% (\ref{eq:particle-bias-linear-n-condition}) imply that the quantity
% inside braces in
% Equation~(\ref{eq:refined-dmop-alpha-mse-final}) is
% $O(N^{-1/2})$. Its squared contribution is therefore $O(N)$.
% Equation~(\ref{eq:variance-linear-n-condition}) makes the centered
% variance term $O(N)$ as well.
% \end{proof}
